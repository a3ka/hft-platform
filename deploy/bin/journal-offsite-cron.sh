...
#          (мутация внедряет `--delete` В ПЕЧАТАЕМЫЙ rsync-блок через
#          `perl -0pi -e 's/--archive/--archive --delete/' …`, и проверка
#          читает ИМЕННО этот блок). Без (b) канарейка не доказывает, что она
#          видит нарушение — отсутствует анти-плацебо (`testing.md` §«Мутационный
#          контроль — обязателен»).
