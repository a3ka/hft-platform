...
**Соседняя половина НЕ механизирована — и здесь это не изображается** (`R-097` Б-2). Кандидат:
документ-фактура несёт `<!-- FACTS: audited_head=… -->`, и `verify_design_claims.sh` печатает
`NOTE` по фактуре без него (`A-010` §H). Статус на 21.08: построен на ветке
`feat/harness-facts-marker` (PR #49), вердикт `C-116` **REJECT** — четыре обманных стаба
проходят пробу; в `main` детектора нет:
`git show origin/main:scripts/verify_design_claims.sh | grep -c FACTS` → `0`. Форма важна —
`git grep -c` при нуле совпадений НЕ печатает «0», а молчит с `exit=1` (`R-097` N-7): в
разделе, требующем показать команду, показанный вывод обязан ею производиться. Пока PR #49 не влит, у этого правила механизма нет
ВООБЩЕ. Первая редакция раздела писала «Механизирована соседняя половина» в настоящем времени
— ровно класс `TD-155`, против которого написан раздел о предъявлении FA ниже: ложное
«механизировано» хуже честного «нет», потому что создаёт уверенность.
