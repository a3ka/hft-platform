...
1bis. **СПИСОК ПРОВЕРОК БЕРЁТСЯ ИЗ МАНДАТА ДОСЛОВНО — ссылка списком не является
   ...
   **Замер, из-за которого пункт существует (`R-130` B-1/B-2/B-3, 2026-08-26).** Мандат назвал
   проверку ссылкой на `A-023` §4.3; tester прогнал список из ТЕЛА КОММИТА — **восемь команд
   вместо десяти**. Не прогонялись две, включая `verify_M-68.sh` — единственную, снимающую
   базовую линию милестоуна. Отдельно: одна команда выполнена на неверном диапазоне
   (`B-2`), а пять запретов подпёрты пересказом вместо сырого stdout (`B-3` — прямое нарушение
