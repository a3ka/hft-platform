...
1. Чистый чекаут **ТОЛЬКО с `origin/feat/M-NN`** (`git worktree add <path> origin/feat/M-NN`) — НЕ полагается
   на dev-локальные артефакты. **Прогоняется ВЕРШИНА ветки, взятая командой, а не SHA из мандата** —
   норма `docs/04-workflow.md` §2 «Предмет гейта — ВЕТКА». **TD-036/RN-18 (BINDING):** если SHA, названный в
   мандате, НЕ на `origin` (`git merge-base --is-ancestor <sha> origin/feat/M-NN` → не 0, или коммиты видны
   только в чужом worktree) → **СТОП, верни dev'у запушить** (не бутстрапься из `/tmp/hft-<role>-*` чужого
   worktree — это маскирует разрыв цепочки, инцидент M-30). Вершина УШЛА ВПЕРЁД от названного SHA — не стоп:
   прогоняешь вершину и печатаешь оба SHA.
...
2. **ШАГ 0 — ДОКАЗАТЬ, ЧТО ПРОВЕРЯЕШЬ ТО САМОЕ (BINDING, закреплено 2026-08-03, M-54).**
   ДО первой команды сборки — пять строк в Done Block, без них вердикт INVALID:
   ```
   pwd                                   # обязан быть ТВОЙ worktree, не /home/nous/hft-platform
   git fetch origin; echo fetch=$?       # без fetch rev-parse читает УСТАРЕВШИЙ локальный ref
   git rev-parse origin/<ветка>          # ВЕРШИНА, взятая командой; её и прогоняешь
   git merge-base --is-ancestor <SHA мандата> origin/<ветка>; echo ancestor=$?   # 0 — прогоняешь вершину; иначе СТОП п.1
   git log --oneline -1                  # обязан совпасть с вершиной строкой выше, не с SHA мандата
                                         # (почему — docs/04-workflow.md §2 «Предмет гейта — ВЕТКА»)
...
- FAIL по реализации → **`architect`** (решение founder'а 2026-09-05; охват
  правила — по предмету отказа, см. ниже). Он называет, что чинить, и через founder'а
...
- **`COGNITIVE-ONLY`** (`gates.md` §11) ЦЕЛИКОМ, и кандидат-механизм здесь НЕ НАЗЫВАЕТСЯ,
  потому что его нет по конструкции роли: ты ничего не пишешь в репозиторий
  (`disallowedTools: Write, Edit`), твой вердикт — `FAIL`, а не файл `verdict: …`,
  и читать CI нечего. У `reviewer`/`risk-critic` кандидат есть и назван; у тебя
  честнее сказать, что механизма нет вовсе, чем скопировать чужой.
