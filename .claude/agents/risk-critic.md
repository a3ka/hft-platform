...
- **ЯВНОЕ ИСКЛЮЧЕНИЕ из решения founder'а 2026-09-05** (записано, а не умолчано): строка выше
  маршрутизирует отказ НЕ к architect'у, и это не недосмотр. Отказ по backtest-отчёту —
  исследовательский цикл Границы A (`docs/DESIGN.md` §6/§7, `docs/02-quant-desk.md`), а не
  находка гейта по набору артефактов милестоуна: предмет — гипотеза и код сигнала, зоны в
  которых у architect'а НЕТ (`scope-guard.md`: `crates/signals/**` + `research/**` — квант-деск).
  Маршрут «через architect'а» поставил бы его звеном в петле, где он ничего не решает.
  Найдено грепом КЛАССА при исполнении `R-173` Б-1; вынесено founder'у на подтверждение.
