...
## Responsibilities
0. **Предмет — ВЕТКА, вершину берёшь командой** (`git rev-parse origin/<ветка>`) и пишешь её в `audited_head`; SHA из мандата — справка. Правило и развилки «ушла вперёд / не предок» — `docs/04-workflow.md` §2 «Предмет гейта — ВЕТКА».
