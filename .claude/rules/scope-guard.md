...
| **engine-dev** (journal/book/oms/sim/runner/alpha/portfolio/strategy/**ops**/**gateway**) | `crates/{journal,book,oms,sim,runner,alpha,portfolio,strategy,ops,gateway}/src/**` (impl, НЕ типы) + `deploy/**` (ops/деплой-механика: cron, Dockerfile, compose, метрики-scrape — НЕ секреты). `crates/gateway` = viz-backend Read Gateway (Слой 8, M-22): read-only консюмер журнала через `journal::stream`, БЕЗ journal-writer/order-path (VB-I-3). | `crates/risk/`, `crates/killswitch/`, `crates/contracts/` (T1), другие крейты вне своего списка |
