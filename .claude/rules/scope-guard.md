...
| **engine-dev** (journal/book/oms/sim/runner/alpha/portfolio/strategy/**ops**/**gateway**/**recorder**) | `crates/{journal,book,oms,sim,runner,alpha,portfolio,strategy,ops,gateway,recorder}/src/**` (impl, НЕ типы) + `deploy/**` (ops/деплой-механика: cron, Dockerfile, compose, метрики-scrape — НЕ секреты). `crates/recorder` = вход даталеера (venue→journal; де-факто engine-dev: M-05 run_writer, M-08 heartbeat, M-09 recon/метрики — закреплено 2026-07-25, M-35). `crates/gateway` = viz-backend Read Gateway (Слой 8, M-22): read-only консюмер журнала через `journal::stream`, БЕЗ journal-writer/order-path (VB-I-3). | `crates/risk/`, `crates/killswitch/`, `crates/contracts/` (T1), другие крейты вне своего списка |
