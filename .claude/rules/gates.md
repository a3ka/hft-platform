...
Непусто → вернуть агента доделать push (`SendMessage`/повторный мандат), НЕ пушить его коммиты
за него: push-scope (`.claude/rules/branch-hygiene.md` п.4) требует, чтобы каждый пушил своё, и
подмена ломает аудит-трейл. Отсутствие worktree → работа не выполнена вовсе, отчёт агента об
успехе — ложный.
