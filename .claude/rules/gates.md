...
- CI-job **`protected-artifacts`** (`scripts/check_protected_artifacts.sh`) валит сборку, если
  диф удаляет эти файлы (`git diff --name-status origin/main...HEAD`, статус `D`).
- Правка и переименование разрешены; запрещено УДАЛЕНИЕ.
- Осознанное удаление — только явной строкой в теле коммита: `ALLOW-ARTIFACT-DELETE: <причина>`.
- Проверено: удаление вердикта → CI FAIL (exit 1); с `ALLOW-ARTIFACT-DELETE` → проходит.
