...
- Проверка **ПОКОММИТНАЯ** от `merge-base(origin/main, HEAD)` до HEAD — не по нетто-дифу
  (иначе `add → delete` внутри ветки «схлопывается» и проходит незамеченным; находка C-006 rev3).
- **Переименование из защищённого пути в незащищённый = удаление** (тихий увод из-под защиты).
  Разрешён только переезд защищённый → защищённый (напр. `docs/contract-rfc/` → `docs/rfc/`).
- Критерий нарушения: артефакт удалён/уведён И **отсутствует на HEAD**. Сценарий «снёс по ошибке →
  восстановил» (инцидент `139b399` → `352b1db`) не наказывается: на HEAD артефакт есть.
- **Override действует только в ТОМ ЖЕ коммите**, который удаляет: `ALLOW-ARTIFACT-DELETE: <причина>`
  в теле. Приписать разрешение задним числом отдельным коммитом — нельзя.
- Канонический путь RFC — **`docs/rfc/`**; `docs/contract-rfc/` остаётся защищённым как исторический.
- **Merge-коммиты проверяются отдельно** (находка C-006 rev4): merge может САМ выбросить файл,
  присутствующий во ВСЕХ родителях («злой мерж») — прежняя версия с `--no-merges` это пропускала.
  Критерий: путь удалён относительно первого родителя И существует во всех родителях ⇒ его выбросил
  сам merge. (Файл, удалённый на одной из веток, в этом родителе отсутствует — это не «злой мерж»,
  и то удаление уже разобрано на своём коммите.)
- Проверено на 7 сценариях: add→delete внутри ветки → FAIL; увод переименованием → FAIL; override
  в другом коммите → FAIL; **злой мерж → FAIL**; override в том же коммите → PASS; **честный merge
  origin/main → PASS**; переезд protected→protected → PASS.
