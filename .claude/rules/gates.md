...
3. **Trials-ledger** (`research/trials-ledger.json`, append-only, `INTG-I-6`) — каждая
   ...
   **Эпохи ledger'а НЕСОПОСТАВИМЫ (TD-015, закреплено 2026-07-13 после M-07).** Ledger
   ...
   brain»). Причина: до M-07 бэктест гонял ad-hoc harness (`qty=1.0`, taker-выход по
   horizon) — он мерил **логику, которой больше не существует** (пре-M-07 записи:
   4 шт., `code_hash f7f4761`, Sharpe −1.73..−2.21). Отдельно невалидны любые записи из
   окна equity-бага (`37753a6..5141fd9`, фантомные точки кривой → **завышенный** Sharpe);
   в репо-ledger'е их нет (проверено reviewer'ом), но всплывшие из локальных прогонов
   отбрасываются. Отчёт обязан НАЗЫВАТЬ свою эпоху (`code_hash` + диапазон записей);
