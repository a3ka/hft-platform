...
- Проверено на 17 сценариях (проба `scripts/tests/red_protected_artifacts.sh`, setup каждого сценария fail-closed): `git mv` в merge → FAIL; `merge -s ours` (side-only вердикт) → FAIL;
