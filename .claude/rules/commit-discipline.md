...
5. **Architect НЕ пушит архитектурные документы сам** (уточнено 2026-07-14, doc-гейт
   ...
   Self-push architect'у разрешён ТОЛЬКО для класса B — **механически несемантические** правки:
   статус-колонки, close-out-пруфы, форматирование, орфография В ПРОЗЕ. **Класс A даже в одну
   строку:** изменение команды, пути, порога, имени роли, ID инварианта, текста §Tasks/Acceptance
   или перенацеливание cross-reference (`gates.md` §9). Сомневаешься — класс A. Причина: документ — источник, из которого агенты исполняют;
