...
5. **Architect приземляет сам ТОЛЬКО чисто-процессные правки** (`.claude/rules/*`,
   `docs/04-workflow.md` и т.п.), не тронувшие код/контракты/риск — **открывая и мержа
   СВОЙ PR** (`gh pr create` → зелёные чеки → `gh pr merge --merge --delete-branch`).
   Слово «пушит сам» из прежней редакции с 2026-08-15 предписывало невозможное: прямой push
   в `main` отклоняется защитой. Это carve-out к строке «не мержит PR» в профиле architect'а:
   она про ЧУЖУЮ работу (milestone-цепочку), а не про собственную процессную правку —
   иначе у такой правки не остаётся ни одного разрешённого исполнителя (`R-085` B-1). Правка, меняющая ФОРМУ
