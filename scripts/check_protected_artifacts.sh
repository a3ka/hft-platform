...
for path in ${existed}; do
  ...
  #
  # Evil merge этим НЕ прощается: если путь жил лишь на ветке и его выбросил merge, удаления нет
  # НИГДЕ в истории — `removed_by` остаётся пустым, и ветка «исчез, и ни один коммит его не
  # удалял» срабатывает по-прежнему.
