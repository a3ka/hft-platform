...
visible_body() {
  git show "HEAD:$1" 2>/dev/null | awk '
    # C-175 B-8: забор опознаётся ТРЕМЯ свойствами, а не одним символом. По CommonMark
    # закрывающий забор обязан быть (1) того же символа, (2) НЕ КОРОЧЕ открывающего и
    # (3) не нести после себя ничего, кроме пробелов. Круг 3 хранил только символ, и оба
    # входа ниже давали ЖИВОЕ ложно-зелёное: ```` закрывался ```, а ``` с хвостом
    # `not-a-closing-fence` считался закрытием. В обоих случаях `## Allowed paths`
    # оставался ВНУТРИ блока, а барьер печатал OK.
    BEGIN { fence = 0; fchar = ""; flen = 0; comment = 0 }
    {
      line = $0
      body = line
      indent = 0
