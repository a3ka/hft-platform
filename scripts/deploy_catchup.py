...
# НАБЛЮДАЕМОЕ: красный ран виден в `gh run list --workflow=deploy.yml` — это ЯРУС S карты
# чтения, `docs/workflow/reading-map.md:128-141` («СОСТОЯНИЕ МИРА»: три команды в начале
# сессии и перед каждым «готово»; вторая из них — ровно эта). Путь и строки названы
# намеренно: `C-096` B-6 объявил ссылку несуществующей, прогнав греп на дереве ветки,
# отстававшей от `main` на 22 коммита, — ярус S внесён коммитом `24dbc1e` и на дереве
# слияния присутствует. Доставляемый алерт остаётся за `П-003`.
HOLD_RC = 2
...
# ═══════════════════════════════════════════════════════════════════════════════════════════
#
# ЧТО ЭТОТ БАРЬЕР ЛОВИТ: НЕОСТОРОЖНОСТЬ — типовые правки манифеста, которые делают, не думая
# о последствиях («временно отключу», «добавлю continue-on-error, чтобы не мешало»).
#
# ЧЕГО ОН НЕ ЛОВИТ: НАМЕРЕННЫЙ ОБХОД. И это не «пока не доделали» — это класс задачи.
# Барьер статически разбирает YAML; способов обезвредить шаг в GitHub Actions открытое
# множество, и каждый круг проверки приносил новые. Замер трёх кругов (`C-093`, `C-096`,
# `C-097`): пропущено 7 из 12 стабов → 12 из 16. Ряд расходится, а не сходится.
#
# ПОИМЁННО НЕ ЛОВИТСЯ (проверено исполнением, `C-097` §1.1; девять вредоносных классов):
#   · инверсия гейта CI          `jobs.catchup.if` → `conclusion != 'success'`
#   · инверсия решения           `deploy.if` → `decision != 'DEPLOY'`
#   · `if`-ВЫРАЖЕНИЕ на шаге     `if: github.event_name == 'schedule'` (ловится только `false`)
#   · `set +e` в теле команды
#   · глушение через `|| echo <текст>` (ловятся только пять литеральных суффиксов)
#   · вызов через переменную     `CATCHUP=…; $CATCHUP decide`
#   · невызов соседних гейтов    удалён шаг `check-wiring`, `--battery` или самой пробы
#
# ЧТО ЭТО ЗНАЧИТ ПРАКТИЧЕСКИ. Красное этого барьера — сигнал «кто-то правил проводку, посмотри
# глазами». Зелёное НЕ является доказательством, что сторож работает: доказательством был бы
# прогон самого workflow с подставным входом, а он локально невоспроизводим.
#
# ПОЧЕМУ НЕ ДОВЕДЕНО ДО КОНЦА — решение владельца, а не оценка «сойдёт»: класс «деплой не
# доехал» случился трижды за месяц и каждый раз чинился вручную за пять минут, тогда как
# барьер стоил трёх кругов гейта и продолжал течь. `harness-track.md` §6 называет это прямо:
# «если доля харнесс-работы остаётся высокой, вопрос не в маршруте, а в том, зачем столько
# инструментов». Вердикт `C-097` остаётся REJECT и НЕ объявляется снятым — влитие идёт
# founder-override'ом (`gates.md` §5), с этим текстом как названным обоснованием.
# ═══════════════════════════════════════════════════════════════════════════════════════════
...
def _agg_env_bindings(job, results):
    """`env:` шагов, чьи значения ссылаются на `needs.*.result` (`C-097` H-6).

    Стандартный и ЧЕСТНЫЙ рефакторинг — вынести `${{ needs.X.result }}` в переменную
    окружения шага. Прежняя редакция читала только тела `run:`, поэтому такая форма давала
    `rc_red == rc_green` и барьер объявлял корректный fail-closed агрегат «падающим всегда».
    Ложное красное на честной форме — та же болезнь, за которую чинился второй барьер.
    """
    env = {}
    for st in _steps(job):
        for k, v in (st.get("env") or {}).items():
            if isinstance(v, str):
                env[str(k)] = _agg_expand(v, results)
    return env


def _agg_exec(script, results, root, env_extra=None):
    """ИСПОЛНИТЬ условие агрегата при заданной модели результатов. Возвращает (rc, stdout).

    Прод гоняет шаг `bash --noprofile --norc -eo pipefail` (форма раннера GitHub). Гейт,
    проверенный не тем вызовом, каким его зовёт прод, не проверен (`testing.md`, целостность
    гейта, свойство 1) — поэтому та же форма здесь.
    """
...
def cmd_check_aggregate():
    ...
    if not gate_script:
        ...
        rc_red, out_red = _agg_exec(gate_script, {AGG_JOB: "failure"}, root,
                                    _agg_env_bindings(gate, {AGG_JOB: "failure"}))
        if rc_red == 0:
            bad(
                "A3",
        ...
        rc_green, out_green = _agg_exec(gate_script, {}, root, _agg_env_bindings(gate, {}))
        if rc_green != 0:
            bad(
                "A3",
