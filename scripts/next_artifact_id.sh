...
for ref in $(all_refs); do
  case "${CLS}" in
    ...
    M|R|C|A)
      ...
      # КАНАЛ -z + read -r -d '' (закрывает ось 4 «имя, требующее квотирования»,
      # тот же ход, что закрыл M-60a в b3ba11a): текстовый `git ls-tree --name-only`
      # квотирует имена с не-ASCII (`"…\320\260.md"`), и grep по `(^|/)R-NNN` ловит
      # только те, что НЕПОСРЕДСТВЕННО под деревом (где есть `/` перед классом) — файл
      # под `R-NNN-…md` в корне реф'а через `(^|/)` не виден. В батарее (red_artifact_ids)
      # все фикстуры вкладывают файлы на глубину, так что баг НЕ ВИДЕН пробой, но это та же
      # дыра, что у `check_artifact_ids.sh`, и закрывать её здесь — обязательно (та же ось,
      # та же причина). После перехода на `-z` имена приходят сырым UTF-8, разделитель — NUL,
      # и grep `(^|/)` ловит и корень, и вложения.
      n=$(git ls-tree -r --name-only -z "${ref}" 2>/dev/null \
          | tr '\0' '\n' \
          | grep -oE "(^|/)(${CLS})-[0-9]+" \
          | grep -oE '[0-9]+' || true)
      ;;
