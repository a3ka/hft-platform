...
#   DEFAULT-режим (переменная НЕ задана) обязан делать ТУ ЖЕ работу — оба ресурса меряются
#   и в нём (DB-11/DB-12, `F-089-3`): барьер, реагирующий только на форму вызова проб,
#   аттестуется набором, ничего не проверяя в проде;
#   exit 0  — свободно ≥ порога И (`CARGO_TARGET_DIR` не задан ИЛИ ФИЗИЧЕСКИ внутри текущего
#             дерева — симлинк наружу есть побег, DB-9/DB-10, `F-089-2`);
...
echo "── Диск-преамбула verify (M-60b G6.2): сценарии DB-1..DB-12 ──"
echo "барьер: ${BARRIER}"
echo
positive_control
...
# ── DB-9 — СИМЛИНК ВНУТРИ ДЕРЕВА, разрешающийся НАРУЖУ (F-089-2).
# Текстовая нормализация видит путь внутри дерева и пропускает его; физически сборка уходит
# в чужой каталог — ровно та подмена бинаря (аудит §2.6), ради которой проверка и заведена.
# Инвариант обходится ФИЗИЧЕСКИ, а не формально, поэтому и судить его надо по физическому пути.
SYMLINK_ESCAPE="${D}/target-escape"
ln -s "${OUTSIDE}" "${SYMLINK_ESCAPE}" || die "DB-9 ln -s"
[ -L "${SYMLINK_ESCAPE}" ] || die "DB-9: симлинк не создан — сценарий тестировал бы обычный каталог"
[ "$(readlink -f "${SYMLINK_ESCAPE}")" = "$(readlink -f "${OUTSIDE}")" ] \
  || die "DB-9: симлинк не разрешается в ${OUTSIDE} — фикстура не про побег из дерева"
case "$(readlink -f "${SYMLINK_ESCAPE}")/" in
  "$(readlink -f "${D}")"/*) die "DB-9: цель симлинка оказалась ВНУТРИ дерева — фикстура не про побег" ;;
esac
if run_barrier "1" "${SYMLINK_ESCAPE}"; then
  fail "DB-9 симлинк внутри дерева, ведущий НАРУЖУ, ПРОШЁЛ — инвариант «target внутри дерева» обходится физически (F-089-2)"
else
  if head -1 "${OUT}" | grep -q "CARGO_TARGET_DIR"; then
    pass "DB-9 symlink-побег пойман по физическому пути, первая строка называет CARGO_TARGET_DIR"
  else
    fail "DB-9 гейт красный, но первая строка не называет CARGO_TARGET_DIR: «$(head -1 "${OUT}")»"
  fi
fi

# ── DB-10 — симлинк ВНУТРИ дерева на каталог ВНУТРИ дерева ⇒ ПРОХОД (анти-ложноположительный).
# Зеркало DB-9: набор обязан падать и против СЛИШКОМ СТРОГОЙ починки («любой симлинк — отказ»),
# иначе фикс DB-9 покупается ложным красным на законной раскладке (`testing.md`: анти-плацебо
# в ОБЕ стороны).
mkdir -p "${D}/target-real" || die "DB-10 mkdir"
ln -s "${D}/target-real" "${D}/target-alias" || die "DB-10 ln -s"
[ -L "${D}/target-alias" ] || die "DB-10: симлинк не создан — сценарий проверял бы обычный каталог"
case "$(readlink -f "${D}/target-alias")/" in
  "$(readlink -f "${D}")"/*) : ;;
  *) die "DB-10: цель симлинка вне дерева — фикстура не про законный симлинк" ;;
esac
run_barrier "1" "${D}/target-alias" \
  && pass "DB-10 симлинк на каталог ВНУТРИ дерева — проход (починка DB-9 не запретила симлинки как класс)" \
  || fail "DB-10 законный симлинк внутрь дерева отвергнут — починка F-089-2 куплена ложным красным"

# ── DB-11 — DEFAULT-режим ОБЯЗАН МЕРИТЬ ДИСК (F-089-3).
# Все сценарии выше подают MIN_FREE_KB, поэтому барьер, делающий работу ТОЛЬКО когда
# переменная задана, а в дефолте возвращающий 0, брал 13/13 — «проб-образный» стаб. Здесь
# переменная НЕ передаётся вовсе, а `df`-shim рапортует 4242 KB — заведомо ниже дефолтного
# порога (5 GiB). Честный барьер обязан отказать; проб-образный — пройдёт и будет пойман.
# Конфаундер (реальное свободное место хоста) исключён: величина приходит из shim'а.
DF_SHIM_DIR="${D}/df-shim-default"
mkdir -p "${DF_SHIM_DIR}" || die "DB-11 df-shim mkdir"
cat > "${DF_SHIM_DIR}/df" <<'SHIM' || die "DB-11 df-shim write"
#!/usr/bin/env bash
echo "Filesystem 1024-blocks Used Available Capacity Mounted on"
echo "fixture 100000 95758 4242 96% ."
SHIM
chmod +x "${DF_SHIM_DIR}/df" || die "DB-11 df-shim chmod"
if run_barrier "UNSET" "UNSET"; then
  fail "DB-11 DEFAULT-режим при подставных 4242 KB свободного ПРОШЁЛ — в дефолте диск не меряется (барьер реагирует лишь на форму вызова проб, F-089-3)"
else
  if head -1 "${OUT}" | grep -q "диск:"; then
    pass "DB-11 default-режим реально мерит диск: 4242 KB против дефолтного порога названы диском"
  else
    fail "DB-11 гейт красный, но первая строка не называет диск: «$(head -1 "${OUT}")» — отказ пришёл не от дисковой проверки"
  fi
fi
unset DF_SHIM_DIR

# ── DB-12 — DEFAULT-режим ОБЯЗАН ПРОВЕРЯТЬ target (F-089-3, вторая половина).
# Тот же адверсарий по второму ресурсу: MIN_FREE_KB не задан, диск заведомо годен (shim даёт
# ~1 TB свободного — дефолтный порог перекрыт), поэтому единственная возможная причина
# отказа — `CARGO_TARGET_DIR` вне дерева. Стаб, валидирующий target только при заданном
# MIN_FREE_KB, здесь проходит и краснеет этим сценарием.
DF_SHIM_DIR="${D}/df-shim-roomy"
mkdir -p "${DF_SHIM_DIR}" || die "DB-12 df-shim mkdir"
cat > "${DF_SHIM_DIR}/df" <<'SHIM' || die "DB-12 df-shim write"
#!/usr/bin/env bash
echo "Filesystem 1024-blocks Used Available Capacity Mounted on"
echo "fixture 2000000000 1000000000 999999999 50% ."
SHIM
chmod +x "${DF_SHIM_DIR}/df" || die "DB-12 df-shim chmod"
if run_barrier "UNSET" "${OUTSIDE}"; then
  fail "DB-12 DEFAULT-режим с target ВНЕ дерева ПРОШЁЛ — в дефолте CARGO_TARGET_DIR не проверяется (F-089-3)"
else
  if head -1 "${OUT}" | grep -q "CARGO_TARGET_DIR"; then
    pass "DB-12 default-режим реально проверяет target: место есть, а target вне дерева назван"
  else
    fail "DB-12 гейт красный, но первая строка не называет CARGO_TARGET_DIR: «$(head -1 "${OUT}")» — отказ пришёл от диска, а не от target"
  fi
fi
unset DF_SHIM_DIR
