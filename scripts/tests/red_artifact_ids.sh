...
#
# ПРАВИЛО ОСТАНОВКИ (§4.4, переписано кругом 5 по `A-007` §6). Прежнее правило («назови ось и
# значение») арбитр признал НЕИСПОЛНИМЫМ: находка о форме данных филируется в любую ось, и за
# пять кругов оно не ограничило ни одного. Действует другое:
#   1. блокирует только ЛОЖНОЕ ЗЕЛЁНОЕ, предъявленное ИСПОЛНЕНИЕМ (фикстура + прогон + контроль);
#   2. ложное КРАСНОЕ блокирует, лишь если достижимо на форме прода, снятой ЗАМЕРОМ;
#   3. всё прочее — категория (iii): NOTE/TD, merge не блокирует.
#
...
MVHIJACK|5|V|новая множественность внесена диапазоном
B1R|1|V|R
B1R|2|V|имя файла
B1R|4|V|новый файл
...
# АНТИ-ПЛАЦЕБО ВТОРОГО РОДА к послаблению (`C-127` Б-1, §2.5 п.2). MVDUP держит случай
# «старый путь ОСТАЛСЯ»; здесь старый путь УДАЛЁН — и именно поэтому первая редакция
# послабления пропускала захват. Предмет НЕ переехал: он уничтожен, а его номер занят
# СОВСЕМ ДРУГИМ предметом (другой слаг). `gates.md` §12 запрещает это прямо: «два разных
# файла под одним идентификатором — нарушение, об одном они предмете или о разных».
#
# Без этого сценария сужение послабления не пиннится НИЧЕМ — ровно как не пиннилось само
# послабление до `MVOK` прод-формы. Тот же дефект, второй раз на том же механизме.
R="$(mk_repo mvhijack)"; art "$R" docs/archive/M-906-synthetic.md ""; commit_all "$R" base-hijack
B="$(cd "$R" && git rev-parse HEAD)"
# ПРОД-ФОРМА: старый носитель ЖИВ в другом ref'е (origin/main до merge'а) — как в MVOK.
( cd "$R" && git branch keeper ) || die "mvhijack: второй ref не создан"
( cd "$R" && git rm -q docs/archive/M-906-synthetic.md ) || die "mvhijack: rm"
art "$R" milestones/M-906-hijack.md ""
commit_all "$R" "старый предмет удалён, номер занят ДРУГИМ"
setup_assert MVHIJACK "$R" "старый носитель обязан ИСЧЕЗНУТЬ с HEAD, остаться в keeper, а новый — иметь ДРУГОЙ слаг (иначе это переезд, а не захват)" \
  '[ ! -e docs/archive/M-906-synthetic.md ] && [ -e milestones/M-906-hijack.md ] \
   && git cat-file -e keeper:docs/archive/M-906-synthetic.md'
expect_block MVHIJACK "$R" "$B" "номер M-906 ЗАХВАЧЕН новым предметом: старый носитель удалён, послабление переезда на это не распространяется"
...
if [ "${FAILED}" -gt 0 ]; then
  ...
  echo "блокирует ложное ЗЕЛЁНОЕ, предъявленное ИСПОЛНЕНИЕМ; ложное КРАСНОЕ — только"
  echo "на замеренной форме прода; прочее — категория (iii), NOTE/TD (§4.4)."
  exit 1
fi
