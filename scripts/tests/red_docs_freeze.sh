...
# Объявленное покрытие. Сверяется в конце с фактически исполненным.
...
echo "── Замок процессного слоя (M-60a): ${EXPECTED_SCENARIOS} сценариев ──"
echo "барьер: ${BARRIER}"
echo
...
# FR-13 — запертый файл УДАЛЁН без токена ⇒ блок.
# Найдено критиком (C-066). Реализация, смотрящая только ИЗМЕНЁННЫЕ файлы
# (`--diff-filter=M`), проходила пробу целиком: сценария на исчезновение не было ни
# одного. Урок соседнего барьера `check_protected_artifacts.sh` («инвариант от
# РЕЗУЛЬТАТА, а не от способа») к замку применён не был — исправляется здесь.
R="$(mk_repo fr13)"; B="$(cd "$R" && git rev-parse HEAD)"
( cd "$R" && git rm -q .claude/rules/gates.md && git commit -q -m "снос правила без разрешения" ) || die "фикстура FR-13"
run_barrier "$R" push "$B" && fail "FR-13 УДАЛЕНИЕ запертого файла прошло — замок слеп к исчезновению" \
                           || pass "FR-13 удаление запертого файла заблокировано"

# FR-14 — запертый файл ПЕРЕИМЕНОВАН из зоны без токена ⇒ блок.
# Вторая половина той же слепоты: файл не изменён и не удалён, он «уехал» за зону.
R="$(mk_repo fr14)"; B="$(cd "$R" && git rev-parse HEAD)"
( cd "$R" && git mv .claude/rules/gates.md docs/gates-moved.md \
  && git commit -q -m "увод правила из зоны без разрешения" ) || die "фикстура FR-14"
run_barrier "$R" push "$B" && fail "FR-14 УВОД запертого файла из зоны прошёл — замок обходится переименованием" \
                           || pass "FR-14 переименование из зоны заблокировано"

# ── Самопроверка покрытия ────────────────────────────────────────────────────────────
# Число сценариев сверяется ТАМ, ГДЕ ОНО ПОРОЖДАЕТСЯ. До этого оно жило в трёх местах
# (шапка пробы «12», фактических 13, порог гейта «>=11») и разошлось во всех трёх —
# C-066 зафиксировал рассинхрон. Литерал, живущий отдельно от предмета, врёт всегда;
# проверка на месте порождения не даёт сценарию потеряться молча.
TOTAL=$((PASSED + FAILED))
if [ "${TOTAL}" -ne "${EXPECTED_SCENARIOS}" ]; then
  echo "FAIL  ПОКРЫТИЕ: исполнено ${TOTAL} сценариев при объявленных ${EXPECTED_SCENARIOS} — сценарий потерян или не объявлен"
  FAILED=$((FAILED + 1))
fi
