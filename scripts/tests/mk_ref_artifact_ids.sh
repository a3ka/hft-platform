...
while IFS="$SEP" read -r cls num lt car; do
  ...
  while IFS="$SEP" read -r c2 n2 l2 car2; do
    ...
    if [ "$c2" = "$cls" ] && [ "$n2" = "$num" ] && [ "$l2" = "$lt" ] && [ "$car2" != "$car" ]; then
      ...
      # `C-127` Б-1: равенство BASENAME отделяет ПЕРЕЕЗД предмета от ЗАХВАТА его номера
      # новым предметом. Зеркалит прод-барьер целиком — расхождение эталона с проводом уже
      # дважды стреляло в этой же батарее.
      if [ "$cls" != TD ] \
         && [ "$(basename "$car")" = "$(basename "$car2")" ] \
