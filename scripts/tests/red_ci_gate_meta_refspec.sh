...
#   CR-9  `C-183` Б-2: на шаге `if: ${{ false }}` ⇒ отвергнут (безусловность)
...
extract_salvage_fetch() { # $1=workflow → команда на stdout; 1, если шаг отсутствует/недейственен
  ...
  # (б) БЕЗУСЛОВНОСТЬ: между `- name:` шага дотягивания и его `run:` не должно быть `if:`.
  #     `if: ${{ false }}` пропускает шаг молча, оставляя его текст на месте.
  name_ln="$(printf '%s\n' "${slice}" | awk -F'\t' -v f="${fetch_ln}" '$1 <= f && $2 ~ /^[[:space:]]*- name:/ { ln = $1 } END { print ln }')"
  [ -n "${name_ln}" ] || return 3
  if printf '%s\n' "${slice}" | awk -F'\t' -v a="${name_ln}" -v b="${fetch_ln}" \
