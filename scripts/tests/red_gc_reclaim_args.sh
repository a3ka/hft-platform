...
#
# `GC_PROC_ROOT` ПИННИТСЯ, и это не удобство, а требование `testing.md` (целостность гейта,
# свойство 2: оракул меряет СВОЙ инвариант, а не окружение). Предмет этой пробы — разбор
# аргументов и достижимость путей удаления; сторож живого процесса (условие 4) к нему
# отношения не имеет, но при живом `/proc` РЕШАЕТ исход: на runner'е GitHub нашлись процессы
# нашего же uid с нечитаемым `cwd` (PID 909/925, прогон 2026-08-24T23:04Z), сторож законно
# удержал ВСЕ деревья, и сценарий CONTROL-REACHABLE объявил механизм мёртвым. Проба мерила
# таблицу процессов раннера. Фикстура даёт один заведомо безобидный PID с ЧИТАЕМЫМ cwd вне
# песочницы: `/proc` признаётся перечислимым (страж `readable` не срабатывает), держателей нет.
_mk_proc_fixture() {
  local s="$1"
  [ -d "$s/fakeproc/1" ] && return 0
  mkdir -p "$s/fakeproc/1" || return 1
  ln -sfn /  "$s/fakeproc/1/cwd" || return 1
}
run_gc_with() {
  local script="$1" s="$2"; shift 2
  _mk_proc_fixture "$s"
  ( cd "$s/repo" && PATH="$s/bin:$PATH" GC_PROC_ROOT="$s/fakeproc" timeout 60 bash "$script" "$@" 2>&1 )
}
