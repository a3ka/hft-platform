...
EOF
  ...
  if [ "${rc}" -ne 0 ] && echo "${out}" | grep -q 'FAIL  \[6-RFC-SHA\].*`1111111`'; then
    pass "сценарий RFC-SHA-no-inapplicable/второй токен: 1111111 в СОСЕДНЕМ параграфе тоже проверен → FAIL, exit=${rc}"
  else
    fail "сценарий RFC-SHA-no-inapplicable/второй токен: ОЖИДАЛСЯ FAIL [6-RFC-SHA] и на 1111111 (соседний параграф — не оправдание пропуска), получено (exit=${rc}):"
    echo "${out}" | sed 's/^/      /'
  fi
}
