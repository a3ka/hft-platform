...
SUT_HOLDER="${ROOT}/scripts/gc_holder.sh"
SUT_ACTIVE="${SUT}"
...
g-reclaim-порог-5|--reclaim 5|RECLAIMED
h-reclaim-dry-порог-5|--reclaim-dry 5|WOULD-RECLAIM
FORMS

# --- L17 ВРАЖДЕБНАЯ ВЫЗЫВАЮЩАЯ СРЕДА не меняет решения (A-020 п.4) ------------------------
#
# L16 (текстовая канарейка над телом holder_pids) УДАЛЁН предписанием арбитра A-020: решение
# о держателе вынесено в ОТДЕЛЬНЫЙ ПРОЦЕСС (scripts/gc_holder.sh), и аргв-независимость
# теперь держится границей процесса, а не проверкой текста. Канарейка не сходилась — семь
# кругов, и последние два ушли на дефекты её самой (ложное красное на `declare honest=0`,
# ложное зелёное на `local IDLE_H="$IDLE_H"`).
#
# ЧТО ПИННИТ ЭТОТ СЦЕНАРИЙ. Замер арбитра, case 2: атрибут `export` ПЕРЕЖИВАЕТ
# переприсваивание, поэтому ГОЛЫЙ дочерний вызов дал бы изоляцию, зависящую от чужого
# профиля: один `export DRY` в окружении — и parser-state уехал бы ребёнку. Ровно из-за
# этого `env -i` с закрытым белым списком обязателен, а не декоративен. Сценарий подаёт
# ВРАЖДЕБНУЮ среду (MODE/DRY/IDLE_H экспортированы со значениями, которые прежние мутанты
# использовали как триггер) и требует, чтобы решение совпало с решением в чистой среде.
read -r main wt < <(mk_case l17) || { sfail "L17-враждебная-среда" "фикстура"; return; }
mkdir -p "${wt}/target"; echo x > "${wt}/target/x"; touch -d '2020-01-01' "${wt}/target"
mk_pgrep "${main}" 1 || { sfail "L17-враждебная-среда" "поддельный pgrep"; return; }
fp="${main}/fakeproc"; mkdir -p "${fp}"; register "${fp}"
: > "${fp}/4242"          # мёртвый PID: перечислим, но не каталог процесса
hostile() {  # <аргументы gc> → вывод при ЭКСПОРТИРОВАННЫХ parser-переменных
  # `export` именно здесь, а не префиксом команды: замер арбитра (case 2) показал, что
  # опасен АТРИБУТ export, переживающий переприсваивание внутри gc_worktrees.sh.
  ( export MODE=reclaim DRY=1 IDLE_H=5
    cd "${main}" || exit 1
    PATH="${main}/bin:${PATH}" GC_PROC_ROOT="${fp}" bash "${SUT_ACTIVE}" "$@" 2>&1 )
}
out_dry="$(hostile --dry-run)"; rc_dry=$?
out_rec="$(hostile --reclaim 5)"; rc_rec=$?
if [ "${rc_dry}" -ne 0 ] || [ "${rc_rec}" -ne 0 ]; then
  nok "L17-враждебная-среда" "exit dry=${rc_dry} reclaim=${rc_rec}, ожидались нули"
elif grep -qF "ЖИВОЙ процесс держит cwd" <<<"${out_dry}${out_rec}"; then
  nok "L17-враждебная-среда" "экспортированные MODE/DRY/IDLE_H ПРОСОЧИЛИСЬ в решение — env -i не держит"
elif ! grep -qF "WOULD-REMOVE  wt" <<<"${out_dry}"; then
  nok "L17-враждебная-среда" "сухой прогон не дошёл до своего пути при враждебной среде"
elif ! grep -qF "RECLAIMED" <<<"${out_rec}"; then
  nok "L17-враждебная-среда" "reclaim не забрал кэш при враждебной среде"
else ok "L17-враждебная-среда" "решение совпало с чистой средой"; fi

# --- L18 ЗАПУСК ЧЕРЕЗ СИМЛИНК ИЗ ДРУГОГО КАТАЛОГА (C-153) --------------------------------
# Находка круга 8, и внесена она МОИМ ЖЕ выносом решения в процесс: `dirname "$0"` при
# запуске через симлинк давал каталог СИМЛИНКА, ребёнок не находился, `bash` отдавал 127, а
# родитель читал ненулевой код как «держателей нет» — fail-open на разрушительном пути.
# Сценарий требует, чтобы решение через симлинк совпало с решением по прямому пути.
read -r main wt < <(mk_case l18) || { sfail "L18-запуск-через-симлинк" "фикстура"; return; }
mk_pgrep "${main}" 1 || { sfail "L18-запуск-через-симлинк" "поддельный pgrep"; return; }
fp="${main}/fakeproc"; mkdir -p "${fp}"; register "${fp}"
ln -s "${wt}" "${fp}/link-target-unused" 2>/dev/null
q="$(spawn_in "${wt}")"
lnkdir="${main}/elsewhere"; mkdir -p "${lnkdir}"; register "${lnkdir}"
ln -s "${SUT_ACTIVE}" "${lnkdir}/gc-link.sh"
GC_PROC_ROOT="$(proc_of "${main}" "${q}")"; export GC_PROC_ROOT
OUT="$(cd "${main}" && PATH="${main}/bin:${PATH}" bash "${lnkdir}/gc-link.sh" --dry-run 2>&1)"; RC=$?
unset GC_PROC_ROOT; kill "${q}" 2>/dev/null
if [ "${RC}" -ne 0 ]; then nok "L18-запуск-через-симлинк" "exit=${RC}"
elif grep -qF "WOULD-REMOVE  wt" <<<"${OUT}"; then
  nok "L18-запуск-через-симлинк" "через симлинк дерево объявлено сносимым ПРИ ЖИВОМ процессе — fail-open"
elif ! grep -qF "ЖИВОЙ процесс держит cwd" <<<"${OUT}"; then
  nok "L18-запуск-через-симлинк" "решение через симлинк не совпало с прямым: держателя не видно"
else ok "L18-запуск-через-симлинк" "exit=0"; fi

# --- L19 РЕБЁНОК НЕДОСТУПЕН — отказ ДО удаления, а не «никто не держит» (C-153) ----------
# Вторая половина той же находки: любой ненулевой код ребёнка, кроме 1, означает «спросить
# не смогли». Сворачивать это в «никто» значит открывать rm -rf отказом инструмента.
read -r main wt < <(mk_case l19) || { sfail "L19-ребёнок-недоступен" "фикстура"; return; }
cpdir="${main}/copy"; mkdir -p "${cpdir}/scripts"; register "${cpdir}"
cp "${SUT_ACTIVE}" "${cpdir}/scripts/gc_worktrees.sh"      # ребёнка НЕ копируем
OUT="$(cd "${main}" && bash "${cpdir}/scripts/gc_worktrees.sh" --dry-run 2>&1)"; RC=$?
if [ "${RC}" -eq 0 ]; then nok "L19-ребёнок-недоступен" "прогон без gc_holder.sh завершился успехом — отказа нет"
elif grep -qF "WOULD-REMOVE" <<<"${OUT}"; then nok "L19-ребёнок-недоступен" "без ребёнка дерево объявлено сносимым — FAIL-OPEN"
elif ! grep -qF "GC REFUSED" <<<"${OUT}"; then nok "L19-ребёнок-недоступен" "отказ не назван: ${OUT}"
else ok "L19-ребёнок-недоступен" "exit=${RC}, отказ ДО удалений"; fi

# --- L20 РЕБЁНОК ЕСТЬ, НО УПАЛ НА ХОДУ — тоже «не знаю», а не «никто» (C-153) -----------
# L19 ловит ОТСУТСТВИЕ ребёнка и до разбора кода возврата не доходит: его отсекает проверка
# `[ -r "$HOLDER" ]`. Значит ветка «иной код ⇒ fail-closed» осталась бы НЕ ЗАПИННЕННОЙ —
# ровно тот случай, когда мутационный контроль показывает пустой kill-set, а строка живёт
# на вере. Здесь ребёнок СУЩЕСТВУЕТ и падает на ходу (rc=3: убит, нет прав, внутренняя
# ошибка) — родитель обязан держать дерево и НАЗВАТЬ причину, а не молча снести.
read -r main wt < <(mk_case l20) || { sfail "L20-ребёнок-упал-на-ходу" "фикстура"; return; }
cpdir="${main}/copy20"; mkdir -p "${cpdir}/scripts"; register "${cpdir}"
cp "${SUT_ACTIVE}" "${cpdir}/scripts/gc_worktrees.sh"
printf '#!/usr/bin/env bash\nexit 3\n' > "${cpdir}/scripts/gc_holder.sh"
chmod +x "${cpdir}/scripts/gc_holder.sh"
OUT="$(cd "${main}" && bash "${cpdir}/scripts/gc_worktrees.sh" --dry-run 2>&1)"; RC=$?
if [ "${RC}" -ne 0 ]; then nok "L20-ребёнок-упал-на-ходу" "exit=${RC}, ожидался 0"
elif grep -qF "WOULD-REMOVE" <<<"${OUT}"; then
  nok "L20-ребёнок-упал-на-ходу" "отказ ребёнка свёрнут в «никто не держит» — FAIL-OPEN"
elif ! grep -qF "gc_holder rc=3" <<<"${OUT}"; then
  nok "L20-ребёнок-упал-на-ходу" "причина не названа в выводе: ${OUT}"
else ok "L20-ребёнок-упал-на-ходу" "держит и называет rc=3"; fi
...
battery() {
  local mutants=(
    # ОДНОЧАСТНЫЕ — целятся в ребёнка (H:), где теперь живёт решение о держателе.
    'G1-сторож-молчит~|~H:[ "${found}" = "1" ]~|~false~|~~|~~|~L11-нечитаемый-cwd-свой L12-алиас-того-же-каталога L2-живой-в-дереве L2-каталог-уцелел L3-живой-в-подкаталоге L18-запуск-через-симлинк L5-cwd-deleted L8-reclaim-живой L8-target-уцелел'
    'G3-не-снят-deleted~|~H:  t="${t% (deleted)}"~|~  :~|~~|~~|~L5-cwd-deleted'
    'G5-нет-readable-стража~|~H:[ "${readable}" = "1" ] || { echo "?"; exit 0; }~|~:~|~~|~~|~L6-proc-нет-fail-closed L7-proc-пуст-fail-closed'
    'G6-скип-нечитаемого-cwd~|~H:      echo "${pid}?"; found=1~|~      :~|~~|~~|~L11-нечитаемый-cwd-свой'
    'G7-нет-сравнения-инода~|~H:[ -n "${cwdid}" ] && [ "${cwdid}" = "${wtid}" ]~|~false~|~~|~~|~L12-алиас-того-же-каталога'
    'G8-мёртвый-PID-держит~|~H:    [ -d "${p}" ] || continue~|~    [ -d "${p}" ] || { echo "${pid}?"; found=1; continue; }~|~~|~~|~L14a-gc-боевой L14b-gc-сухой L14c-reclaim-боевой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5 L17-враждебная-среда'
    # G4 целится в ОБВЯЗКУ родителя — сторож в цикле reclaim; он остался в gc_worktrees.sh.
    ...
    # G12/G13 — восстанавливают РОВНО две дыры C-153, обе внесённые выносом решения в процесс.
    'G12-SCRIPT_DIR-без-разрешения~|~SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" && pwd)"~|~SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"~|~~|~~|~L18-запуск-через-симлинк'
    'G13-ненулевое-как-никто~|~    *) echo "?(gc_holder rc=$rc)"; return 0 ;;~|~    *) return 1 ;;~|~~|~~|~L20-ребёнок-упал-на-ходу'
    # СОСТАВНЫЕ (A-020): прежние G9/G10/G11 в ребёнке НЕЙТРАЛЬНЫ — переменная всегда UNSET,
    # условие мертво. Честная мутация обязана показать ЕДИНСТВЕННЫЙ оставшийся путь:
    # ЛЕТОК В БЕЛОМ СПИСКЕ env -i ПЛЮС условие в ребёнке. Убивают их существующие L14-формы
    # и L17 — то есть поверхность закрыта не канарейкой, а конструкцией.
    'G9-леток-MODE-и-условие~|~    PATH="$PATH" \~|~    PATH="$PATH" MODE="$MODE" \~|~H:    [ -d "${p}" ] || continue~|~    if [ "${MODE:-gc}" = "reclaim" ] && [ ! -d "${p}" ]; then echo "${pid}?"; found=1; continue; fi\n    [ -d "${p}" ] || continue~|~L14c-reclaim-боевой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5 L17-враждебная-среда'
    'G10-леток-DRY-и-условие~|~    PATH="$PATH" \~|~    PATH="$PATH" DRY="$DRY" \~|~H:    [ -d "${p}" ] || continue~|~    if [ "${DRY:-0}" = "1" ] && [ ! -d "${p}" ]; then echo "${pid}?"; found=1; continue; fi\n    [ -d "${p}" ] || continue~|~L14b-gc-сухой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14h-reclaim-dry-порог-5 L17-враждебная-среда'
    'G11-леток-IDLE_H-и-условие~|~    PATH="$PATH" \~|~    PATH="$PATH" IDLE_H="$IDLE_H" \~|~H:    [ -d "${p}" ] || continue~|~    if [ "${IDLE_H:-2}" = "5" ] && [ ! -d "${p}" ]; then echo "${pid}?"; found=1; continue; fi\n    [ -d "${p}" ] || continue~|~L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5 L17-враждебная-среда'
  )
  local bfail=0 spec
  for spec in "${mutants[@]}"; do
    # ШЕСТЬ ПОЛЕЙ: имя ~|~ needleA ~|~ replA ~|~ needleB ~|~ replB ~|~ kill-set.
    # Пара B пуста у одночастных мутантов. Префикс «H:» у needle означает gc_holder.sh.
    local name="${spec%%~|~*}" rest="${spec#*~|~}"
    local nA="${rest%%~|~*}"; rest="${rest#*~|~}"
    local rA="${rest%%~|~*}"; rest="${rest#*~|~}"
    local nB="${rest%%~|~*}"; rest="${rest#*~|~}"
    local rB="${rest%%~|~*}"; local declared="${rest#*~|~}"
    # МУТАНТ — ПАРА ФАЙЛОВ (A-020): решение о держателе живёт в gc_holder.sh, обвязка и
    # белый список env -i — в gc_worktrees.sh. Копируются ОБА; SCRIPT_DIR ребёнка
    # разрешается от $0, поэтому копия зовёт копию, а не оригинал.
    # Префикс needle выбирает файл: «H:» — gc_holder.sh, иначе — gc_worktrees.sh.
    # Составной мутант (леток в белом списке + условие в ребёнке) записывается как две
    # пары, разделённые «;;»: это ЕДИНСТВЕННЫЙ оставшийся путь протащить parser-state.
    local mdir="${WORK}/mut-${name}"; mkdir -p "${mdir}/scripts"; register "${mdir}"
    cp "${SUT}" "${mdir}/scripts/gc_worktrees.sh"
    cp "${SUT_HOLDER}" "${mdir}/scripts/gc_holder.sh"
    local mut="${mdir}/scripts/gc_worktrees.sh"
    local applied=0
    apply_patch() {  # <needle> <repl> → 0 если применено
      [ -n "$1" ] || return 0
      local pn="$1" pr="$2" tgt="${mut}"
      case "${pn}" in H:*) tgt="${mdir}/scripts/gc_holder.sh"; pn="${pn#H:}" ;; esac
      python3 - "${tgt}" "${pn}" "${pr}" <<'PYEOF'
import io,sys
dst,needle,repl=sys.argv[1],sys.argv[2],sys.argv[3]
needle=needle.replace('\\n','\n'); repl=repl.replace('\\n','\n')
s=io.open(dst,encoding='utf-8').read()
if s.count(needle)!=1: raise SystemExit(3)
io.open(dst,'w',encoding='utf-8').write(s.replace(needle,repl,1))
PYEOF
    }
    if apply_patch "${nA}" "${rA}" && apply_patch "${nB}" "${rB}"; then applied=1; fi
    if [ "${applied}" -le 0 ]; then
      printf 'SETUP-FAIL %-28s мутант не построен: needle не найдено или не уникально\n' "${name}"
      bfail=$((bfail + 1)); continue
    fi
    if ! bash -n "${mut}" 2>/dev/null || ! bash -n "${mdir}/scripts/gc_holder.sh" 2>/dev/null; then
