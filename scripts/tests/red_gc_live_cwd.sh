...
# --- L16 СТРУКТУРНО: решение о держателе не ЧИТАЕТ параметров вызова (C-147 R5-1) --------
#
# ПЯТЬ КРУГОВ, ПЯТЬ ПЕРЕМЕННЫХ — И ПОЭТОМУ ЗДЕСЬ НЕ ШЕСТОЙ СЦЕНАРИЙ, А ЗАПРЕТ.
#   C-141 нет пиннинга вовсе · C-145 условность по MODE · C-146 условность по DRY
#   C-147 условность по IDLE_H — пороге, которого перебор не варьировал
# Каждый раз я перечислял ЗНАЧЕНИЯ ещё одной переменной, а критик находил следующую. Перебор
# по построению не может доказать ОТСУТСТВИЕ зависимости: он покрывает то, о чём автор
# вспомнил. Доказать отсутствие можно только запретом на само чтение.
#
# ИНВАРИАНТ: решение «держит / не держит» — функция ДЕРЕВА и состояния `/proc`, и ничего
# больше. Ни один параметр вызова (`MODE`, `DRY`, `IDLE_H`) не имеет права участвовать.
# Канарейка читает ТЕЛО `holder_pids` и падает на любом упоминании — то есть убивает и
# мутанта критика, и всякого будущего, условного по переменной, которой ещё нет.
#
# ПРЕДЕЛ НАЗВАН, И ОН РЕАЛЕН: канарейка ТЕКСТОВАЯ. Чтение через псевдоним
# (`x=$IDLE_H` снаружи функции, затем `$x` внутри) она не поймает. Планка поднята с
# «перечисли все переменные» до «обойди намеренно» — но не до невозможности. Поэтому
# поведенческий перебор ниже СОХРАНЁН, а не заменён: две проверки закрывают разные
# подмножества, и ни одна не объявляется достаточной.
body="$(awk '/^holder_pids\(\) \{/,/^\}/' "${SUT_ACTIVE}")"
if [ -z "${body}" ]; then nok "L16-решение-не-читает-argv" "тело holder_pids не извлечено — сценарий недействителен"
else
  # БЕЛЫЙ СПИСОК, А НЕ ЧЁРНЫЙ. Перечислять запрещённые имена (`MODE|DRY|IDLE_H`) значит
  # повторить ту же ошибку уровнем выше: список закрывает то, о чём автор вспомнил, и
  # молчит о переменной, которой ещё нет. Разрешено РОВНО то, из чего решение и состоит:
  # аргумент, локальные переменные цикла и два модульных значения, не зависящих от argv.
  # Всё остальное — включая любую будущую ручку разбора аргументов — роняет сценарий.
  allow=' 1 wt p pid t found readable wtid cwdid PROC_ROOT MY_UID UNINSPECT_FILE '
  leaked=''
  while IFS= read -r v; do
    [ -n "${v}" ] || continue
    case "${allow}" in *" ${v} "*) ;; *) leaked="${leaked}${v} " ;; esac
  done < <(grep -oE '\$\{?[A-Za-z_0-9][A-Za-z0-9_]*' <<<"${body}" | sed 's/^[$]{\?//' | sort -u)
  if [ -n "${leaked}" ]; then
    nok "L16-решение-не-читает-argv" "решение о держателе читает постороннее: ${leaked}— значит может зависеть от формы запуска"
  else ok "L16-решение-не-читает-argv" "тело читает только своё: ${allow}"; fi
fi
...
battery() {
  local mutants=(
    ...
    'G8-мёртвый-PID-держит~|~      [ -d "$p" ] || continue~|~      [ -d "$p" ] || { echo "${pid}?"; found=1; continue; }~|~L14a-gc-боевой L14b-gc-сухой L14c-reclaim-боевой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5'
    ...
    # G11 — мутант критика из C-147 R5-1 ДОСЛОВНО: условен по ТРЕТЬЕЙ переменной, порогу
    # IDLE_H. Пережил все 24 сценария прежней пробы и девять мутантов. Ловится ДВАЖДЫ:
    # поведенчески (формы с порогом 5) и структурно (канарейка L16) — и второе важнее,
    # потому что работает и против переменной, которой ещё не существует.
    'G11-условен-по-порогу~|~      [ -d "$p" ] || continue~|~      if [ "${IDLE_H:-2}" = "5" ] && [ ! -d "$p" ]; then printf '%s?\n' "$pid"; found=1; continue; fi\n      [ -d "$p" ] || continue~|~L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5 L16-решение-не-читает-argv'
    'G10-мёртвый-PID-держит-в-dry~|~      [ -d "$p" ] || continue~|~      if [ "${DRY:-0}" = "1" ] && [ ! -d "$p" ]; then echo "${pid}?"; found=1; continue; fi\n      [ -d "$p" ] || continue~|~L14b-gc-сухой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14h-reclaim-dry-порог-5 L16-решение-не-читает-argv'
    'G9-мёртвый-PID-держит-в-reclaim~|~      [ -d "$p" ] || continue~|~      if [ "${MODE:-gc}" = "reclaim" ] && [ ! -d "$p" ]; then echo "${pid}?"; found=1; continue; fi\n      [ -d "$p" ] || continue~|~L14c-reclaim-боевой L14d-reclaim-сухой L14e-dry-затем-reclaim L14f-reclaim-затем-dry L14g-reclaim-порог-5 L14h-reclaim-dry-порог-5 L16-решение-не-читает-argv'
  )
  local bfail=0 spec
