...
# ИЗВЛЕЧЕНИЕ — ПОДСЧЁТОМ СКОБОК, А НЕ ДИАПАЗОНОМ ДО `^}` (C-150 R6-1, второй дефект).
# Прежняя редакция брала `awk '/^holder_pids\(\) \{/,/^\}/'` и обрывалась на ПЕРВОЙ
# неотступленной `}`. Критик вставил вложенный помощник с закрывающей скобкой в нулевой
# колонке, а запрещённое чтение — ПОСЛЕ него: тело обрезалось, канарейка зеленела, проба
# давала 27/27. Ложный НЕГАТИВ у стража правдивости хуже, чем его отсутствие: он создаёт
# уверенность. Счётчик глубины вложенность переживает.
# ОСТАТОЧНЫЙ ПРЕДЕЛ НАЗВАН: скобки внутри строк и комментариев считаются наравне с кодом —
# лексера здесь нет и не будет. Обойти можно намеренно; случайно — нет.
body="$(awk '
  !inf && /^holder_pids\(\) \{/ { inf = 1 }
  inf {
    line = $0
    o = gsub(/\{/, "{"); c = gsub(/\}/, "}")
    print line
...
if [ -z "${body}" ]; then nok "L16-решение-не-читает-argv" "тело holder_pids не извлечено — сценарий недействителен"
  ...
  # БЕЛЫЙ СПИСОК ВЫВОДИТСЯ ИЗ САМОЙ ФУНКЦИИ, А НЕ ЗАШИТ (C-150 R6-1, первый дефект).
  # Зашитый перечень локальных давал ЛОЖНОЕ КРАСНОЕ на честной новой переменной: критик
  # добавил `honest=0`, использовал внутри, и страж объявил дефектом законную реализацию.
  # Страж, мешающий честной работе, выключат первым же «он мешает» — и мы вернёмся к нулю.
  # Поэтому разрешено: аргумент, ВСЁ, что функция объявила через `local`, и три модульных
  # значения, от argv не зависящих. Всё остальное — постороннее чтение.
  locals="$(grep -oE '^[[:space:]]*local[[:space:]]+.*' <<<"${body}" \
            | sed -E 's/^[[:space:]]*local[[:space:]]+//' | tr ' ' '\n' \
            | sed -E 's/=.*$//' | grep -E '^[A-Za-z_][A-Za-z0-9_]*$' | sort -u | tr '\n' ' ')"
  allow=" 1 PROC_ROOT MY_UID UNINSPECT_FILE ${locals}"
  leaked=''
  ...
  else ok "L16-решение-не-читает-argv" "читает только объявленное своё: ${allow}"; fi
fi
