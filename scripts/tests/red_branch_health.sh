...
# expect_th — то же, что `expect`, но с ЯВНЫМ порогом висяка. Нужен потому, что `expect`
# гоняет предмет при `STALE_DAYS=0`, где ЛЮБОЙ зелёный PR старше-или-равен порогу и попадает
# в висяки. Дефект 2026-08-24 жил ровно в зазоре «зелёный, но моложе порога», и на нулевом
# пороге он недостижим: сценарий, не доходящий до проверяемой ветки кода, покрытие лишь
# ЗАЯВЛЯЕТ (`testing.md`, ловушка «сценарий утверждает покрытие, не покрывая»).
expect_th() {
  local name="$1" root="$2" prs="$3" th="$4" wantrc="$5" must="$6" mustnot="$7"
  OUT="$(BRANCH_HEALTH_ROOT="${root}" BRANCH_HEALTH_PRS="${prs}" BRANCH_HEALTH_STALE_DAYS="${th}" bash "${SUT_ACTIVE}" 2>&1)"; RC=$?
...
# --- ЗЕЛЁНЫЙ МОЛОЖЕ ПОРОГА: агрегат обязан утверждать РОВНО то, что замерил ---------------
# Дефект 2026-08-24: колонка печатала `#89/green`, а агрегат — «веток с зелёным PR и без
# merge'а не найдено». Отбор шёл по `age >= STALE_DAYS`, а фраза утверждала БЕЗУСЛОВНОЕ
# отсутствие. Наблюдатель противоречил сам себе в одном прогоне на одних данных.
d="$(mk_repo young feat/M-88-fresh)" || { sfail "S22-зелёный-моложе-порога" "фикстура"; return; }
printf 'feat/M-88-fresh\t88\tgreen\n' > "${d}/prs.tsv"
expect_th "S22-зелёный-моложе-порога" "${d}" "${d}/prs.tsv" 1 0 "ЗЕЛЁНЫЙ-СВЕЖИЙ: feat/M-88-fresh" "NOTE  ВИСЯК"

# Та же фикстура, второе утверждение: фраза агрегата ОБЯЗАНА быть привязана к порогу.
# Без этого сценария правка «назвать свежие» прошла бы, а ложная фраза осталась бы жить.
expect_th "S23-фраза-названа-порогом" "${d}" "${d}/prs.tsv" 1 0 "старше 1 сут не найдено" "без merge'а не найдено"

# Анти-плацебо: КРАСНЫЙ моложе порога не «свежий зелёный» и не висяк.
d="$(mk_repo youngred feat/M-89-broken)" || { sfail "S24-красный-моложе-порога" "фикстура"; return; }
printf 'feat/M-89-broken\t89\tred\n' > "${d}/prs.tsv"
expect_th "S24-красный-моложе-порога" "${d}" "${d}/prs.tsv" 1 0 "-" "ЗЕЛЁНЫЙ-СВЕЖИЙ"
...
battery() {
  ...
  local mutants=(
    ...
    # M5 целится в накопитель «зелёный моложе порога». Роняет ОБА новых сценария: S22 не
    # находит строки ЗЕЛЁНЫЙ-СВЕЖИЙ, S24 остаётся зелёным (ему запрещённой строки и не надо).
    "M5-СВЕЖИЙ-ЗЕЛЁНЫЙ:GREEN_YOUNG+=:S22-зелёный-моложе-порога"
  )
  local bfail=0
