...
# | T4   | негативный путь и регистр живы           | `ok. [1-9]` + ДВА `^fn ИМЯ(` (не упоминания)|
...
for crate in venue-binance venue-binance-futures; do
  ...
  #
  # НОСИТЕЛЬ №9 (`C-205`): первая редакция этого пина грепала ИМЯ ГДЕ УГОДНО в файле и
  # обманывалась комментарием — реальный `fn` переименован, старое имя оставлено строкой
  # `// снят: o2_…`, шаг зелёный, гейт `VERDICT: PASS`. Тот же класс, что носитель №6, и —
  # хуже — введён В ТОМ ЖЕ КОММИТЕ, где №6 закрывался якорем `^const`: я исправил форму
  # строкой выше и тут же повторил дефект строкой ниже. Отсюда якорь `^\s*fn ИМЯ(`:
  # пиннится ОБЪЯВЛЕНИЕ функции, а не упоминание имени.
  if cargo test -p "$crate" --test red_l2delta_allowlist >/tmp/m45-allow-$crate.log 2>&1 \
     ...
     && grep -qE '^[[:space:]]*fn o2_symbol_outside_allowlist_is_not_captured\(' "$(oracle_of "$crate")" \
     && grep -qE '^[[:space:]]*fn o4_config_case_does_not_silently_disable_capture\(' "$(oracle_of "$crate")"; then
