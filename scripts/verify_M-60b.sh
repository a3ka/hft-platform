...
# `C-130` Б-3: якорь `^` считал ТОЛЬКО вызовы в начале строки, а сценарии, обёрнутые в
# `if`-ветку setup-guard'а, стоят с отступом — счёт declared/executed расходился (37 против 41)
# и ронял acceptance МОЛЧА, при зелёных джобах CI. Отступ допускается явно.
declared_of() { grep -cE '^[[:space:]]*(if )?run_barrier ' "$1"; }
...
# `C-130` Б-3: паттерн БЕЗ границы после «16» ловил и суффиксованные `GM-16b..h` — сценарии
# СОСЕДНИХ предметов (движущаяся база, sync-вершина, утечка токена), к сожжённому номеру
# отношения не имеющие. Сторож обязан красить `GM-16` и только его, поэтому за цифрами
# требуется НЕ буква и НЕ цифра.
if grep -qE '(pass|fail) +"GM-16[^0-9a-z]' "${GM_PROBE}"; then
