...
# Норма пересмотрена founder'ом 2026-08-15: 725 → 950. Прежняя цифра взята из `A-003`, когда
# корпуса сегодняшних норм ещё не существовало (защита `main` и PR-флоу, харнесс-трек,
# предъявление FA, GATE-META — всё это добавлено позже и режется только вместе с самими
# нормами). Замер 15.08: rules = 1120; чистка ИСТОРИИ по плану даёт 917, дальше пришлось бы
# резать требования, по которым работают агенты. Новая норма 950 достижима вырезанием одной
# лишь истории и оставляет 33 строки запаса — не на рост, а на то, что корпус живёт, пока
# чистка исполняется. Решение founder'а: «резать не надо, подыми чуток норму».
RULES_BUDGET=950
CLAUDE_BUDGET=70
