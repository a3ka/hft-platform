...
if [ -n "$SRC_FILES" ]; then
  READ_ALL_HITS=$(sed 's://.*::' $SRC_FILES | grep -nE 'read_all|Vec[[:space:]]*<[[:space:]]*Event[[:space:]]*>' || true)
  ...
  WRITER_HITS=$(sed 's://.*::' $SRC_FILES | grep -nE 'Journal::open|open_with|WriterConfig|\.append\(|\.flush\(' || true)
  [ -z "$WRITER_HITS" ]; check "no journal-writer symbols in gateway/src (GW-I-1 read-only)" $?
  [ -n "$WRITER_HITS" ] && note "  ↳ $WRITER_HITS"

  # --- Прод-путь чтения назван явно: gateway/src ссылается на journal::stream (NOTE-2 positive) ---
  grep -rqE 'journal::stream|::stream\(' $SRC_FILES; check "gateway/src uses journal::stream (bounded read path)" $?
else
