...
# ПРЕДЕЛ НАЗВАН: сторож видит `cwd`, а не открытые файлы. Процесс, работающий в дереве по
# АБСОЛЮТНЫМ путям из другого `cwd`, им не ловится. Закрыть это можно только обходом
# `/proc/*/fd`, что стоит на порядок дороже и на больших `target/` даёт секунды на прогон;
# сознательный выбор — дешёвый сторож на доминирующий случай, а не полный и неподъёмный.
#
...
holder_pids() {
  local wt="$1" p pid t found=0 readable=0
  ...
  # нулём, и ответ всё равно «не знаю» ⇒ «держит». Ни один сценарий пробы её не ронял
  # (kill-set мутанта пришёл пустым), то есть защищена она не была. Строка, которую нечем
  # уронить, — не барьер, а вид барьера; лучше одна проверенная, чем две, из которых одна
  # держится на вере.
  for p in "$PROC_ROOT"/[0-9]*; do
    ...
    t="${t% (deleted)}"
    case "$t" in "$wt"|"$wt"/*) echo "$pid"; found=1 ;; esac
  done
