...
# «Введён» = появился в диапазоне И ДОЖИЛ ДО HEAD: инвариант §4.1 сформулирован ОТ
# РЕЗУЛЬТАТА, а покоммитный обход без сверки с HEAD судит промежуточные состояния. Цена
# конкретна: ветка, исправившая СВОЮ ЖЕ ошибку перенумерацией (реальный коммит `f0e915b`:
# R-038-M-60a → R-042 — снятие блокера F-2 `R-041`), блокировалась, хотя в результате
# коллизии нет. Проверки `git cat-file -e HEAD:$f` и `git show HEAD:TECH-DEBT.md` снимают
# ложный красный, НЕ ослабляя ни одного нарушения: и переименование В занятый номер, и
# коллизия в не-вершинном коммите, и третий файл под существующей коллизией оставляют свой
# файл/запись в HEAD. Предмет тоже читается из HEAD — в результате живёт именно его
# (финальная) редакция, а не та, что существовала в момент промежуточного коммита.
#
