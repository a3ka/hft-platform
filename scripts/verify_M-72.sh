...
chk_named_test() { # $1=имя шага, далее — команда cargo
  ...
  if [ "${ran:-0}" -eq 0 ]; then
    # ТРИ ИСХОДА, А НЕ ДВА, и различать их обязательно: «оракула нет» и «оракул есть,
    # но не собрался» — разные состояния задачи, и одинаковый текст отправил бы читателя
    # искать не то. COMPILE-RED — ЗАЯВЛЕННОЕ состояние оракула задачи 2: он ссылается на
    # шов, который вносит engine-dev (спека §«Сигнатура шва»), и до тех пор не собирается.
    if printf '%s\n' "${out}" | grep -qE 'could not compile|^error\[E[0-9]'; then
      echo "FAIL: ${name} — оракул ЕСТЬ, но НЕ СОБРАЛСЯ (COMPILE-RED): $(printf '%s\n' "${out}" | grep -m1 -E '^error' | cut -c1-100)"
    else
