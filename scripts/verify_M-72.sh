...
# Мутация идёт в ИЗОЛИРОВАННОЙ копии — гейт не смеет править рабочее дерево. Кэш сборки
# общий (CARGO_TARGET_DIR), иначе шаг стоит десяти минут и его выключат как «медленный».
...
if [ "${HFT_M72_MUTATION:-1}" = "0" ]; then
  ...
  if cmp -s "${SERVE}" "${MUT}/${SERVE}"; then
    ...
    ( cd "${MUT}" && CARGO_TARGET_DIR="${ROOT}/target" \
