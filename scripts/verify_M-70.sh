...
if [ -z "${BASE}" ]; then
  ...
  FAIL=$((FAIL + 1))
elif [ -n "${HEAD_SHA}" ] && [ "${BASE}" = "${HEAD_SHA}" ]; then
  ...
  echo "SKIP: task #6 — диапазон ПУСТ (merge-base == HEAD == ${BASE}): милестоун влит, сравнивать версию не с чем. НЕ проверено; при непустом диапазоне шаг снова обязателен."
  SKIPPED=$((SKIPPED + 1))
else
