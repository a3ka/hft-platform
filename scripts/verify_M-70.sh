...
# |             | работает, а прод-дефолт РОВНО узкий         | РАЗОБРАННЫЙ дефолт из `${GATEWAY_BANDS:-…}` равен ровно `0.001` + `DB-I-7` ДВУХ уровней (парсер И кадр на проводе). Согласие ВТОРОГО блока (`gateway-checkpoint`) сторожит поведенческий `c3ter`, не греп |
...
if ! printf '%s\n' "${BANDS_BLOCK}" | grep -qE '^[[:space:]]+GATEWAY_[A-Z_]+:'; then
  ...
  # Зашитый литерал без `${GATEWAY_BANDS:-…}` отнял бы у оператора включение вовсе.
    ...
    # СТРАЖ ОТЛОЖЕННОГО РЕШЕНИЯ: дефолт обязан быть РОВНО `0.001` — ни шире, ни длиннее.
    #
    # ПЕРВАЯ РЕДАКЦИЯ ЭТОГО ПРЕДИКАТА БЫЛА ДЫРЯВОЙ, и дыру нашёл `C-226` B-1: она
    # запрещала СЕМЬ КАНОНИЧЕСКИХ значений и требовала присутствия `0.001`, поэтому
    # `${GATEWAY_BANDS:-0.001,0.02,0.04}` ПРОХОДИЛ — две дальние полосы включены до замера,
    # а гейт печатал «прод-дефолт узкий». Это ровно тот класс, который этот милестоун ловит
    # у себя третий раз: запрещён НАЗВАННЫЙ экземпляр (канонический набор), а не КЛАСС
    # (любой широкий дефолт). Поэтому теперь дефолт РАЗБИРАЕТСЯ и сравнивается целиком.
    DEFAULT_VAL="$(printf '%s\n' "${BANDS_LINE}" | sed -n 's/.*${GATEWAY_BANDS:-\([^}]*\)}.*/\1/p')"
    if [ -z "${DEFAULT_VAL}" ]; then
      echo "FAIL: task #7 — дефолт не разобран из \${GATEWAY_BANDS:-…} (строка: ${BANDS_LINE}): вывод о его ширине был бы ложным при любом значении" >&2
      FAIL=$((FAIL + 1))
    elif [ "${DEFAULT_VAL}" != "0.001" ]; then
      echo "FAIL: task #7 — прод-дефолт НЕ РОВНО узкий: разобрано '${DEFAULT_VAL}', требуется ровно '0.001' (строка: ${BANDS_LINE}). До замера длительности полого окна (R-185 §1) ЛЮБАЯ полоса шире 0.001 в дефолте запрещена: после ресинка метка ставит liveness=confirmed на заниженные числа. Включение — строка оператора в host .env, не смена дефолта" >&2
      FAIL=$((FAIL + 1))
    else
      echo "PASS: task #7 доставка состава под оракулом DB-I-7, прод-дефолт РОВНО 0.001, ручка переопределяема"
    fi
  fi
fi
