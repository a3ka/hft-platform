...
if [ -z "${BASE}" ]; then
  ...
  FAIL=$((FAIL + 1))
elif [ -n "${HEAD_SHA}" ] && { [ "${BASE}" = "${HEAD_SHA}" ] || [ -z "$(git diff --name-only "${BASE}..HEAD" -- "${LIB}" 2>/dev/null)" ]; }; then
  ...
  if [ "${BASE}" = "${HEAD_SHA}" ]; then
    _why="диапазон ПУСТ (merge-base == HEAD == ${BASE}): милестоун влит, сравнивать версию не с чем"
  else
    _why="диапазон НЕ ТРОГАЕТ ${LIB}: форма выдачи не менялась, объявлять бампом нечего"
  fi
  echo "SKIP: task #6 — ${_why}. НЕ проверено; как только диапазон тронет ${LIB}, шаг снова обязателен."
  SKIPPED=$((SKIPPED + 1))
else
