...
# | task #7     | ~~канонический состав ДОЕХАЛ до сервиса~~ **СНЯТО rev12** — теперь: доставка
# |             | работает, а прод-дефолт РОВНО узкий         | **ОТРЕНДЕРЕННОЕ** `docker compose config` значение `GATEWAY_BANDS` равно ровно `0.001` (не текстовый разбор — `C-227`: суффикс за скобками в разбор не попадал) + `DB-I-7` ДВУХ уровней. Согласие ВТОРОГО блока (`gateway-checkpoint`) сторожит поведенческий `c3ter`, не греп. Рендерер недоступен ⇒ FAIL-closed |
...
FAIL=0
# SKIP — ТРЕТИЙ исход, введён R-191 Б-1 (путь B). Шаг диапазона после вливания судить
# НЕЧЕГО: это не успех и не провал. Считается отдельно и печатается в итоговой строке —
# пропуск, невидимый в вердикте, есть ложный PASS (`testing.md`, анти-плацебо).
SKIPPED=0
step() { printf '\n── %s\n' "$*"; }
chk() {
...
# НОСИТЕЛИ ФОРМЫ ВЫДАЧИ — множество, а не один файл (R-192 Н-2).
# В `${LIB}` живут константа версии и типы кадра (`Selector`, `OhlcvRow`, `DepthRow`,
# `SeriesBundle`, `Snapshot`). Но WS-конверт `ServeMsg` (`crates/gateway-serve/src/lib.rs:86`)
# версионируется ТЕМ ЖЕ `GATEWAY_SCHEMA_VERSION` (`gateway-serve/src/lib.rs:56`): новый вариант
# конверта или переименование меняют ПРОВОД, не трогая `${LIB}`. Пока носителем считался один
# файл, такая смена получала SKIP с текстом "форма не менялась" — утверждение шире проверенного.
FORM_CARRIERS="crates/gateway/src/lib.rs crates/gateway-serve/src/lib.rs"
...
# СРАВНЕНИЕ ИДЁТ С РАБОЧИМ ДЕРЕВОМ, А НЕ С `HEAD` (R-192 Н-1): `git diff <base> -- <пути>`
# без `..HEAD`. Иначе возникало слепое пятно — форма сменена, но не закоммичена: условие по
# диапазону КОММИТОВ её не видит и печатает SKIP "форма не менялась", тогда как `V_HEAD`
# читает РАБОЧИЙ файл. Теперь все три чтения согласованы: `V_BASE` — из коммита базы,
# `V_HEAD` — из файла, диапазон — база против файла. На чистом дереве исход тот же.
...
if [ -z "${BASE}" ]; then
  ...
  FAIL=$((FAIL + 1))
elif [ -n "${HEAD_SHA}" ] && [ -z "$(git diff --name-only "${BASE}" -- ${FORM_CARRIERS} 2>/dev/null)" ]; then
  # ДИАПАЗОН СХЛОПНУЛСЯ (R-191 Б-1, путь B). Милестоун влит: merge-base совпал с HEAD, и
  # "версия в базе" стала той же строкой, что "версия на HEAD". Шаг сравнивает правку с её
  # же результатом и краснеет при ЛЮБОМ состоянии дерева — ловил бы несуществующий дефект
  # у каждого, кто запустит гейт после merge'а (замер R-190 §6: 41 PASS / 1 FAIL на чистом
  # дереве, бамп 9->10 при этом СОСТОЯЛСЯ коммитом 09a7fdf).
  #
  # ПОЧЕМУ SKIP, А НЕ PASS. PASS означал бы "проверено и хорошо" — шаг не проверил НИЧЕГО.
  # Ложный PASS дороже ложного FAIL: он тихо снимает требование бампа с будущих правок формы.
  # SKIP печатает причину, считается отдельно и виден в итоговой строке.
  if [ "${BASE}" = "${HEAD_SHA}" ]; then
    _why="диапазон ПУСТ (merge-base == HEAD == ${BASE}) и носители формы в рабочем дереве не тронуты: милестоун влит, сравнивать версию не с чем"
  else
    _why="между базой ${BASE} и РАБОЧИМ ДЕРЕВОМ не тронут ни один носитель формы (${FORM_CARRIERS}): объявлять бампом нечего"
  fi
  echo "SKIP: task #6 — ${_why}. НЕ проверено; шаг снова обязателен, как только тронут любой носитель формы."
  SKIPPED=$((SKIPPED + 1))
else
...
step "task #7 — доставка состава РАБОТАЕТ, а прод-дефолт УЗКИЙ (сторож отложенного включения)"
...
# ТРЕБОВАНИЕ СМЕНИЛОСЬ rev12, и шаг обязан смениться вместе с ним, иначе он сторожит
# отменённое. Было: «канонический набор записан в прод-дефолт». Стало: «доставка состава
# работает, а прод-дефолт УЗКИЙ до замера полого окна» (`R-185` §1, §4; спека §3sexies).
#
# Почему шаг теперь сторожит УЗКИЙ дефолт, а не широкий. `R-185` §1 предъявил прогоном:
# после ресинка книга плотна только до REST-охвата, но ОДНО дальнее обновление поднимает
# `book::max_reach_pct` (`crates/book/src/lib.rs:287-297` — расстояние до КРАЙНЕГО уровня,
# не мера полноты), и промежуточные полосы получают `liveness=confirmed` на ЗАНИЖЕННЫХ
# числах (`crates/gateway/src/lib.rs:2248` — ветка `not-observed` только при `band > reach`).
# Пока это так, флип дефолта на канонический = включение известной лжи, и шаг обязан
# КРАСНЕТЬ на таком флипе. Это страж РЕШЕНИЯ, а не проверка прогресса.
#
# ЧЕГО ШАГ НЕ ДЕЛАЕТ, названо явно:
#  · он НЕ пиннит, что канонический набор где-то поимённо перечислен. Его пиннят оракул
#    `DB-I-7` своими константами и `docs/fa/viz-backend.md` §2; греп по КОММЕНТАРИЮ был бы
#    наблюдением текста вместо поведения (`A-031` носитель №6, тот же класс);
#  · он НЕ сверяет два блока compose между собой. Согласованность «писатель чекпоинта ↔
#    читатель» судит ПОВЕДЕНЧЕСКИЙ оракул `c3ter` (`red_checkpoint_bin_prod_argv`), который
#    выводит читателя ИЗ блока `gateway-serve`; дубль грепом отвергнут `R-145` Б-2.
...
if ! printf '%s\n' "${BANDS_BLOCK}" | grep -qE '^[[:space:]]+GATEWAY_[A-Z_]+:'; then
  ...
  if [ -z "${BANDS_LINE}" ]; then
    ...
    FAIL=$((FAIL + 1))
  # Ручка обязана оставаться ПЕРЕОПРЕДЕЛЯЕМОЙ: включение — одна строка оператора в host `.env`.
  # Зашитый литерал без `${GATEWAY_BANDS:-…}` отнял бы у оператора включение вовсе.
  elif ! printf '%s\n' "${BANDS_LINE}" | grep -q '${GATEWAY_BANDS:-'; then
    echo "FAIL: task #7 — GATEWAY_BANDS не переопределяем: запись обязана идти через \${GATEWAY_BANDS:-…}, иначе оператор не может включить состав строкой в host .env (строка: ${BANDS_LINE})" >&2
    FAIL=$((FAIL + 1))
  else
    # СТРАЖ ОТЛОЖЕННОГО РЕШЕНИЯ — спрашиваем РЕНДЕРЕР, а не разбираем текст руками.
    #
    # ТРИ РЕДАКЦИИ ЭТОГО ПРЕДИКАТА БЫЛИ ДЫРЯВЫ, и каждую нашёл критик. Общая причина у всех
    # трёх ОДНА: я судил ФРАГМЕНТ строки вместо ЗНАЧЕНИЯ, которое увидит потребитель.
    #   ред. 1 — запрещала семь канонических значений ⇒ `${GATEWAY_BANDS:-0.001,0.02,0.04}`
    #            проходил (`C-226` B-1): запрет названного экземпляра вместо класса;
    #   ред. 2 — разбирала дефолт из `${…:-…}` ⇒ `${GATEWAY_BANDS:-0.001},0.02` проходил
    #            (`C-227`): `sed` брал `0.001` ИЗ СКОБОК, а Compose рендерит `0.001,0.02` —
    #            суффикс ЗА скобками в разбор не попадал вовсе.
    # Мораль записана здесь, а не в третий раз выучена: текстовый разбор чужого формата —
    # суррогат. `testing.md` §«Целостность гейта» свойство 1: гейт обязан прогонять ПРОД-ФОРМУ.
    # Прод-форма состава — то, что отдаёт `docker compose config`, и это единственный
    # источник, знающий ВСЕ способы получить значение (скобки, суффикс, якоря, `.env`).
    #
    # `GATEWAY_BANDS` СНИМАЕТСЯ из окружения намеренно: судим ДЕФОЛТ репозитория, а не то,
    # что случайно выставлено у запускающего. Секрет подставляется пустышкой — `config`
    # только рендерит YAML и контейнеров не поднимает, но без обязательной переменной
    # интерполяция падает целиком.
    # РЕГРЕССИОННАЯ БАТАРЕЯ (`C-227` §D: «добавь mutation proof в architect-owned artifact»).
    # Исполнена 2026-09-18, `cargo` подменён заглушкой; в колонке «рендер» — то, что вернул
    # `docker compose config`, то есть ровно то, что увидит прод:
    #
    #   запись в compose                       рендер            исход шага
    #   ${GATEWAY_BANDS:-0.001},0.02           0.001,0.02        FAIL  ← обход `C-227`
    #   ${GATEWAY_BANDS:-0.001,0.02,0.04}      0.001,0.02,0.04   FAIL  ← обход `C-226`
    #   0.02,${GATEWAY_BANDS:-0.001}           0.02,0.001        FAIL  ← префикс, мой довесок
    #   ${GATEWAY_BANDS:-0.001}                0.001             PASS  ← положительный контроль
    #   (docker недоступен, exit=127)          —                 FAIL  ← fail-closed
    #
    # Первые две редакции предиката проходили строки 1 и 2 соответственно. Батарея записана
    # ЗДЕСЬ, рядом с предикатом: вердикт живёт в `research/critiques/`, а сторож — тут, и
    # следующий правящий обязан видеть, против чего он обязан краснеть.
    RENDER_ERR="$(env -u GATEWAY_BANDS GATEWAY_JWT_SECRET=verify-m70-dummy \
                    docker compose config 2>&1 >/tmp/m70-compose-render.$$ )"
    RENDER_RC=$?
    if [ ${RENDER_RC} -ne 0 ]; then
      # FAIL-CLOSED: невозможность ИЗМЕРИТЬ не равна «измерено и хорошо». Шаг, молча
      # пропускающий себя при отсутствии рендерера, был бы плацебо самого себя.
      echo "FAIL: task #7 — прод-форма НЕ ИЗМЕРЕНА: 'docker compose config' вернул ${RENDER_RC} (${RENDER_ERR}). Гейт не вправе печатать вывод о составе, которого не видел" >&2
      FAIL=$((FAIL + 1))
      rm -f /tmp/m70-compose-render.$$
    else
      RENDERED="$(sed -n 's/^[[:space:]]*GATEWAY_BANDS:[[:space:]]*"\{0,1\}\([^"]*\)"\{0,1\}[[:space:]]*$/\1/p' /tmp/m70-compose-render.$$ | head -1)"
      rm -f /tmp/m70-compose-render.$$
      if [ -z "${RENDERED}" ]; then
        echo "FAIL: task #7 — в отрендеренном compose нет ключа GATEWAY_BANDS: судить нечего, и зелёное здесь было бы вакуумом" >&2
        FAIL=$((FAIL + 1))
      elif [ "${RENDERED}" != "0.001" ]; then
        echo "FAIL: task #7 — ОТРЕНДЕРЕННЫЙ прод-дефолт НЕ РОВНО узкий: '${RENDERED}', требуется ровно '0.001'. До замера длительности полого окна (R-185 §1) ЛЮБАЯ полоса шире 0.001 в дефолте запрещена: после ресинка метка ставит liveness=confirmed на заниженные числа. Включение — строка оператора в host .env, не смена дефолта" >&2
        FAIL=$((FAIL + 1))
      else
        echo "PASS: task #7 доставка под оракулом DB-I-7, ОТРЕНДЕРЕННЫЙ прод-дефолт ровно 0.001, ручка переопределяема"
      fi
    fi
  fi
fi
...
if [ "${SKIPPED}" -gt 0 ]; then
  echo "ПРОПУЩЕНО ШАГОВ: ${SKIPPED} — см. строки SKIP выше; это НЕ проверенное."
fi
if [ "${FAIL}" -eq 0 ]; then echo "VERDICT: PASS (пропущено: ${SKIPPED})"; exit 0; fi
echo "VERDICT: FAIL (${FAIL}; пропущено: ${SKIPPED})"; exit 1
