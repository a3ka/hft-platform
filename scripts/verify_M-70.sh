...
if ! printf '%s\n' "${BANDS_BLOCK}" | grep -qE '^[[:space:]]+GATEWAY_[A-Z_]+:'; then
  ...
  # Зашитый литерал без `${GATEWAY_BANDS:-…}` отнял бы у оператора включение вовсе.
    ...
    # СТРАЖ ОТЛОЖЕННОГО РЕШЕНИЯ: дефолт обязан быть УЗКИМ, пока полое окно не замерено.
    # Проверяется ОТСУТСТВИЕ каждой из шести дальних полос канона — наблюдение отсутствия
    # (`testing.md`, целостность гейта, свойство 4), а не наличие узкой: `0.001` могли бы
    # оставить рядом с каноническими и получить широкий состав при зелёном шаге.
    LEAKED=""
    for b in 0.015 0.03 0.05 0.08 0.15 0.3 0.6; do
      printf '%s\n' "${BANDS_LINE}" | grep -qE "(^|[^0-9.])${b}([^0-9]|$)" && LEAKED="${LEAKED} ${b}"
    done
    if [ -n "${LEAKED}" ]; then
      echo "FAIL: task #7 — прод-дефолт содержит дальние полосы:${LEAKED} (строка: ${BANDS_LINE}). До замера длительности полого окна (R-185 §1) включение запрещено: после ресинка метка ставит liveness=confirmed на заниженные числа. Включение = строка оператора в host .env, не смена дефолта" >&2
      FAIL=$((FAIL + 1))
    elif ! printf '%s\n' "${BANDS_LINE}" | grep -qE '(^|[^0-9.])0\.001([^0-9]|$)'; then
      echo "FAIL: task #7 — прод-дефолт не узкий: 0.001 в записи не найден (строка: ${BANDS_LINE})" >&2
      FAIL=$((FAIL + 1))
    else
      echo "PASS: task #7 доставка состава под оракулом DB-I-7, прод-дефолт узкий (0.001), ручка переопределяема"
    fi
  fi
fi
