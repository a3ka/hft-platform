  ...
  if [ -z "${BASE}" ]; then
    ...
    if git clone -q --no-hardlinks . "$T" 2>/dev/null; then
      ...
      if [ -n "${EXIST}" ]; then
        ...
        # ВТОРАЯ ПОЛОВИНА негативного контроля — класс TD (`R-054` Б-6, п.4 условия APPROVED).
        # Прежняя редакция строила дубль ТОЛЬКО файлом (класс C), а `TD` живёт ЗАПИСЬЮ и
        # обслуживается отдельной веткой кода — ровно там и жил fail-open, при котором барьер
        # печатал affirmative-«OK» на настоящей коллизии. Слаг берётся С МЕТАСИМВОЛОМ намеренно:
        # форма «счастливого пути» этот класс не ловит по построению, а 6 слагов из 111 в
        # `origin/main` (5.4 %) метасимволы уже несут.
        TDNUM="$(git show HEAD:TECH-DEBT.md 2>/dev/null \
                 | sed -nE 's/^- \*\*TD-0*([0-9]+)\*\*.*/\1/p' | head -1)"
        if [ -z "${TDNUM}" ]; then
          fail "S негативный контроль TD невозможен: в TECH-DEBT.md нет ни одной записи"
        else
          ...
          if git clone -q --no-hardlinks . "$T2" 2>/dev/null; then
            ...
            if ( cd "$T2" && EVENT_NAME=push PUSH_BEFORE="${BASE}" PR_BASE_SHA="${BASE}" \
              ...
              fail "S негативный контроль TD: дубль записи TD-${TDNUM} со слагом-метасимволом ПРОШЁЛ. \
Это fail-OPEN в fail-closed-барьере: половина TD обслуживается отдельной веткой кода, и \
проверка только класса C её не накрывает (класс Б-6)"
            else
              pass "S негативный контроль TD: дубль записи TD-${TDNUM} со слагом-метасимволом заблокирован"
            fi
