...
echo "--- S: САМОРЕФЕРЕНЦИЯ + негативный контроль (дубль = ВТОРОЙ НОСИТЕЛЬ под тем же ID) ---"
...
#
# ПЕРЕПИСАН ПОД РЕШЕНИЕ Б (founder, 2026-08-12; спека §6, план миграции §6). Предмет артефакта
# барьером БОЛЬШЕ НЕ ЧИТАЕТСЯ, поэтому синтетический дубль строится как ВТОРОЙ НОСИТЕЛЬ ПОД ТЕМ
# ЖЕ ИДЕНТИФИКАТОРОМ, а не как расхождение предметов. Прежняя редакция писала файл
# `C-NNN-совсем-другой-предмет.md` и доказывала свойство, которого у барьера под Б нет вовсе:
# после миграции она осталась бы зелёной по совпадению (два файла под одним ID — нарушение и
# так), но проверяла бы НЕ ТО, что заявляет, а на летер-аварной паре (`C-058b`) могла бы
# промахнуться мимо идентификатора целиком.
#
# КОНСТРУКЦИЯ ДУБЛЯ: к слагу СУЩЕСТВУЮЩЕГО носителя приписывается суффикс
# (`research/critiques/C-058b-critic2.md` → `…-critic2-дубль.md`). Класс, номер и БУКВА при этом
# сохраняются побайтово, а путь различается — то есть конструкция верна независимо от ведущих
# нулей и наличия буквы и НЕ повторяет разбор барьера (иначе оракул мерил бы сам себя:
# `testing.md` — «зависимый эталон мутация ловит плохо»). Не-ASCII в суффиксе намеренно: тот же
# путь чтения имён, что у `B4Q`.
if [ -f "${BARRIER}" ]; then
  # Карта каталогов-носителей — та же, что в §3.1 (включая ВТОРОЙ реестр класса R,
  # `research/reports/`, и обязательную привязку к `.md`).
  CARRIER_RE='^(milestones/M|research/reviews/R|research/reports/R|research/critiques/C|research/arbitration/A)-[0-9]+[a-z]?(-[^/]*)?\.md$'
  BASE=$(git merge-base origin/main HEAD 2>/dev/null || echo "")
  ART_N=0
  if [ -n "${BASE}" ]; then
    ART_N=$(git diff --name-only --diff-filter=AR "${BASE}..HEAD" 2>/dev/null | grep -cE "${CARRIER_RE}")
  fi
  if [ -z "${BASE}" ]; then
    fail "S база не установлена (origin/main недоступен)"
  elif [ "${ART_N}" -eq 0 ]; then
    fail "S SETUP НЕ СОСТОЯЛСЯ: в диапазоне ${BASE}..HEAD нет ни одного нового носителя — судить не о чем"
  elif EVENT_NAME=push PUSH_BEFORE="${BASE}" PR_BASE_SHA="${BASE}" bash "${BARRIER}" >/dev/null 2>&1; then
    pass "S собственный диапазон проходит барьер (${ART_N} носителей введено)"
    # ── негативный контроль 1: ФАЙЛ — второй носитель под тем же идентификатором ──────
    T="$(mktemp -d /tmp/m61-selfneg-XXXXXX)"
    if git clone -q --no-hardlinks . "$T" 2>/dev/null; then
      # Донор выбирается детерминированно и с проверкой, что дубль ещё не существует:
      # setup, молча выбравший занятый путь, тестировал бы не тот сценарий.
      DONOR=""; DUP=""
      while IFS= read -r cand; do
        [ -n "${cand}" ] || continue
        try="${cand%.md}-дубль.md"
        git cat-file -e "HEAD:${try}" 2>/dev/null && continue
        DONOR="${cand}"; DUP="${try}"; break
      ...
      if [ -n "${DONOR}" ]; then
        ...
        # ── негативный контроль 2: КЛАСС TD (`R-054` Б-6, п.4 условия APPROVED) ────────
        # `TD` живёт ЗАПИСЬЮ и обслуживается отдельной веткой кода — там и жил fail-open, при
        # котором барьер печатал affirmative-«OK» на настоящей коллизии. Слаг берётся С
        # МЕТАСИМВОЛОМ намеренно: форма «счастливого пути» этот класс не ловит по построению,
        # а 6 слагов из 111 в `origin/main` (5.4 %) метасимволы уже несут.
        #
        # НОМЕР ВЫБИРАЕТСЯ ВТОРЫМ ПУТЁМ, независимым от разбора барьера, и с ПРЕДПОЧТЕНИЕМ
        # номера, который держит ТОЛЬКО НЕканоническая строка (закрытая карточка / «(исходная
        # формулировка)» / мультиномерный пункт). Под Б занятость широка именно ради этих форм:
        # замер плана — 5 из 5 таких номеров давали exit 0 на настоящем переиспользовании.
        # Каноническая карточка — законный запасной путь (не «поблажка»): при её выборе шаг
        # проверяет ту же половину, но в форме, которую видел и прежний барьер.
        TD_ALL="$(git show HEAD:TECH-DEBT.md 2>/dev/null \
                  | grep -oE '^- \*\*TD-[0-9]+' | grep -oE '[0-9]+' | sed 's/^0*//' | sort -u)"
        TD_CANON="$(git show HEAD:TECH-DEBT.md 2>/dev/null \
                    | sed -nE 's/^- \*\*TD-0*([0-9]+)\*\* `[^`]+`.*/\1/p' | sort -u)"
        TDNUM="$(awk 'NR==FNR{c[$0];next} ($0 != "") && !($0 in c)' \
                 <(printf '%s\n' "${TD_CANON}") <(printf '%s\n' "${TD_ALL}") | head -1)"
        TDKIND="НЕканоническая строка (прод-форма занятости)"
        if [ -z "${TDNUM}" ]; then
          TDNUM="$(printf '%s\n' "${TD_CANON}" | head -1)"
          TDKIND="каноническая карточка (запасной путь: неканонических форм в дереве нет)"
        fi
        if [ -z "${TDNUM}" ]; then
          fail "S SETUP НЕ СОСТОЯЛСЯ: в TECH-DEBT.md нет ни одной строки-записи — занятость TD не с чем столкнуть"
        else
          T2="$(mktemp -d /tmp/m61-selfneg-td-XXXXXX)"
...
Это fail-OPEN в fail-closed-барьере: занятость TD обязана быть ШИРОКОЙ (любая строка-запись), \
введение — узким (только заводящая карточка); §3.1 (в)"
            else
              pass "S негативный контроль TD: дубль карточки TD-${TDNUM} заблокирован; номер держит ${TDKIND}"
            fi
          else fail "S негативный контроль TD: клон не удался"; fi
          rm -rf "$T2"
        fi
      else fail "S SETUP НЕ СОСТОЯЛСЯ: в дереве нет носителя, для которого дубль ещё не создан"; fi
...
# R-052 Н-1 / A-006 §5: механизм опирается на норму, и задача 6 §Tasks своего оракула не имела
# вовсе (шаг S проверяет БАРЬЕР и о существовании нормы не знает ничего).
#
# ПЕРЕПИСАН ПОД РЕШЕНИЕ Б (founder, 2026-08-12; спека §6, план миграции §6). Прежний шаг требовал,
# чтобы §12 называла ВСЕ формы шапки `**Предмет:**`/`**Контекст**`, принимаемые барьером. Под Б
# шапка не читается: `subject_from_body` удаляется вместе с правилами 1/3/4/5 §3.1 — значит
# прежнее требование стало требованием НАЗЫВАТЬ НЕСУЩЕСТВУЮЩИЙ МЕХАНИЗМ, а сам шаг после
# миграции умер бы собственным setup-guard'ом (из кода нечего извлекать). Проверяются четыре
# объявленных свойства задачи 6:
#   G1 раздел существует, тело ≤8 строк;
#   G2 аллокатор назван ПОИМЁННО;
#   G3 §12 НЕ обещает механизма чтения шапки (норма, обещающая несуществующий механизм, хуже
#      её отсутствия — формулировка самой спеки §6);
#   G4 ФОРМА идентификатора, названная в §12, разбирается регекспом `id_of`, ИЗВЛЕЧЁННЫМ ИЗ
#      КОДА барьера (`C-071` NOTE-1: перечень берётся из кода, а не из литерала в гейте).
...
if [ ! -f "${RULES}" ]; then
  ...
  # G3 — норма НЕ обещает чтения шапки. Ищутся ровно те две формы, которые требовала прежняя
  # редакция нормы (`**Предмет:**` и `**Контекст**`): их присутствие означает, что от автора
  # артефакта по-прежнему требуют поле, которого не читает никто.
  G_GHOST="$(printf '%s\n' "${G_SEC}" | grep -oE 'Предмет:|Контекст' | sort -u | tr '\n' ' ')"
  if [ -z "${G_GHOST// /}" ]; then
    pass "G §12 не обещает механизма чтения шапки (под Б его не существует)"
  else
    fail "G §12 всё ещё требует шапку (${G_GHOST}) — механизма чтения под Б нет: норма, обещающая несуществующий механизм, хуже её отсутствия"
  fi

  # G4 — форма идентификатора сверяется с КОДОМ. Регексп извлекается из барьера; обе живые
  # записи класса покрыты (`[MRCA]` и `M|R|C|A`). Делимитр `#` исключён из захвата, поэтому
  # альтернация внутри хвоста регекспа шаг не ломает.
  G_RE="$(grep -hoE '\^\((\[MRCA\]|M\|R\|C\|A)\)[^'"'"'/#]*\$' "${BARRIER}" | head -1)"
  g_id() { printf '%s\n' "$1" | sed -nE "s#${G_RE}#\1|\2|\3#p"; }
  if [ -z "${G_RE}" ]; then
    fail "G SETUP НЕ СОСТОЯЛСЯ: из ${BARRIER} не извлечён регексп идентификатора — сверять форму §12 не с чем"
  elif [ -z "$(g_id 'M-60a')" ] || [ -n "$(g_id 'README')" ]; then
    # Страж НЕвакуумности: извлечённое обязано быть разборщиком идентификатора, а не любой
    # строкой, случайно похожей на регексп. Разбор, принимающий всё или ничего, сверку
    # обессмысливает — и это FAIL setup'а, а не тихое зелёное.
    fail "G SETUP НЕ СОСТОЯЛСЯ: извлечённый регексп «${G_RE}» не разбирает 'M-60a' либо принимает 'README' — сверка была бы вакуумной"
  else
    # Примеры идентификаторов берутся ИЗ ТЕКСТА §12: backtick-токен, начинающийся с
    # `КЛАСС-ЦИФРА`, целиком — и БЕЗ сужения формы. Сужение здесь стоило бы ровно того дефекта,
    # ради которого заведён милестоун: первая редакция шага брала `[MRCA]-[0-9]+[a-z]?`, и
    # пример `M-60ab` (двухбуквенный — код такого НЕ разбирает) МОЛЧА выпадал из сверки,
    # оставляя её зелёной. Единственное исключение — перечислительная форма через `/`
    # (`M-60a/b/c`): это сокращение для трёх идентификаторов, а не идентификатор. Исключение
    # ПЕЧАТАЕТСЯ в вердикте, чтобы оно было видно, а не подразумевалось.
    G_ALL="$(printf '%s\n' "${G_SEC}" | grep -oE '`[MRCA]-[0-9][^`]*`' | tr -d '`' | sort -u)"
    G_EX="$(printf '%s\n' "${G_ALL}" | grep -v '/')"
    G_SKIP="$(printf '%s\n' "${G_ALL}" | grep '/' | tr '\n' ' ')"
    G_NEX="$(printf '%s\n' "${G_EX}" | grep -c .)"
    G_BAD=""
    for ex in ${G_EX}; do
      [ -n "$(g_id "${ex}")" ] || G_BAD="${G_BAD}«${ex}» "
    done
    if [ "${G_NEX}" -eq 0 ]; then
      fail "G SETUP НЕ СОСТОЯЛСЯ: §12 не приводит ни одного примера идентификатора — сверять форму нечем"
    elif [ -n "${G_BAD// /}" ]; then
      fail "G §12 называет форму, которую код НЕ разбирает: ${G_BAD}— норма разошлась с кодом (регексп барьера: ${G_RE})"
    else
      # Различитель ДЕФИС — то самое свойство, ради которого §12 пишет букву ВПЛОТНУЮ (`M-60a`).
      # Код обязан читать букву у `M-60a-slug` и НЕ читать её у `M-90-a` (слаг из одной буквы).
      # Без этой половины G зеленел бы и против letter-blind разбора, при котором утверждение
      # §12 «буква — часть идентификатора» механизмом не поддержано (мутанты `letterblind`,
      # `slugletter`, `dashoptional` — §4.5 спеки).
      G_L="$(g_id 'M-60a-docs-freeze' | awk -F'|' '{print $3}')"
      G_S="$(g_id 'M-90-a' | awk -F'|' '{print $3}')"
      if [ "${G_L}" = "a" ] && [ -z "${G_S}" ]; then
        pass "G форма §12 совпадает с кодом: ${G_NEX} примера разобраны регекспом ${G_RE}; буква вплотную — часть ID, слаг из одной буквы — нет${G_SKIP:+; перечислительная форма не разбиралась: ${G_SKIP}}"
      else
        fail "G §12 объявляет букву частью идентификатора, а код её так не читает: 'M-60a-docs-freeze'→буква «${G_L}», 'M-90-a'→буква «${G_S}» (регексп ${G_RE})"
      fi
    fi
  fi
fi
