...
#
# Поэтому на merge-ref (существует `HEAD^2`) исключается ИМЕННО первый родитель. На событии
# `push` второго родителя нет, множество пусто, и семантика прежняя.
EXCL_MAIN=()
...
own_touched() { # $1 = дополнительное исключение (`^<sha>`) либо пусто
  if [ -n "${1:-}" ]; then
    git log --format='' --name-only --diff-merges=cc "${BASE}..HEAD" "$1" \
      ${EXCL_MAIN[@]+"${EXCL_MAIN[@]}"} 2>/dev/null | sort -u
  else
    git log --format='' --name-only --diff-merges=cc "${BASE}..HEAD" \
      ${EXCL_MAIN[@]+"${EXCL_MAIN[@]}"} 2>/dev/null | sort -u
  fi
}
own_bodies() { # тела коммитов ТОГО ЖЕ диапазона — токен ищется там же, где найдено нарушение
  git log --format='%B' "${BASE}..HEAD" "$1" ${EXCL_MAIN[@]+"${EXCL_MAIN[@]}"} 2>/dev/null || true
}
