...
# | task #2b | не-книжные серии не задваиваются            | ИСПОЛНЕНИЕ `m85_6` (`C-223` N-1)                 |
# | task #3  | цена затравки не растёт с ДЛИНОЙ ИСТОРИИ    | ИСПОЛНЕНИЕ `m85_4` + страж оси `m85_5`           |
...
#   (2) ПЯТЬ шагов из восьми ЗЕЛЕНЫ СЕГОДНЯ (#2, #2b, #3, SETUP, SEEK) — это НЕ нарушение правила
#       «шаг не зеленеет раньше своей задачи»: красным по ПРОГРЕССУ обязан быть один — #1.
#       Остальные сторожат ГРАНИЦЫ, за которые развязка не смеет выйти, и их работа
#       начинается в момент фикса. Различающая сила каждого предъявлена мутацией в
#       `M-85` §4.1, а не заявлена;
#   (3) шаг `SEEK-ВАРИАНТ` наблюдает ОТСУТСТВИЕ (`testing.md`, целостность гейта, св-во 4):
#       `frames_since_with_stats` объявлен вне предмета (§2bis.2) ровно потому, что его никто
#       не зовёт из прод-кода. Заведут — шаг покраснеет, и решение придётся принять заново.
...
TC=crates/gateway/tests/red_m85_bootstrap_cost.rs
...
step "task #2b — НЕ-книжные серии не задваиваются (C-223 N-1)"
chk "grep -q '^fn m85_6_non_book_series_do_not_double_count_under_bootstrap' ${T}"
chk_named_test "сторож m85_6 (запрет §2.1 получил сторожа, а не прозу)" \
  cargo test -p gateway --test red_m85_frames_book_continuity --quiet \
  m85_6_non_book_series_do_not_double_count_under_bootstrap

step "task #3 — цена затравки не растёт с ДЛИНОЙ ИСТОРИИ (граница ресурса)"
chk "grep -q '^fn m85_4_bootstrap_cost_does_not_grow_with_history_length' ${TC}"
chk "grep -q '^fn m85_5_axis_guard_tail_work_is_constant_across_both_measurements' ${TC}"
# Оба сценария ОДНИМ прогоном: страж оси обязан исполняться вместе с измерением, иначе
# «ось чиста» осталось бы утверждением о прошлом запуске.
chk_named_test "оракул m85_4 + страж оси m85_5 (VB-I-10 на пути frames_since)" \
