...
#     восемь дырявых реализаций — красными. Без этого F проверяет лишь, что проба не падает.
...
echo "--- F2: АНТИ-ПЛАЦЕБО — батарея эталон + восемь дырявых (спека §4.1) ---"
...
# Шесть из восьми мутантов — те, против которых rev4 была ЗЕЛЁНОЙ (`A-005` §10).
