...
- **TD-052** `пол-защиты-ограничен-по-памяти-но-не-по-времени` (найдено reviewer'ом на PR-гейте
  ...
  **Почему не блокер:** (а) память — класс TD-011 — закрыта и запиннена; (б) до rev5 в этой же
  ...
  обхода) проектируется **architect'ом** RED-first. Severity: MINOR (операторский путь, не
  горячий; данные не теряются — задержка, а не порча).
- **TD-053** `событие-крупнее-64KiB-невидимо-для-скана-пола` (найдено reviewer'ом на PR-гейте
  M-49 rev5, 2026-07-31; **НЕ блокер**). `READABLE_SCAN_MAX_CARRY = 64 * 1024` в
  `crates/journal/src/segments.rs` против собственного санити-капа ридера крейта
  (`segments.rs:443` — `if len > 64 * 1024 * 1024`, то есть 64 **MiB**). `resync_max_seq`
  трактует фрейм, чей `frame_end - i` превышает 64 KiB, как порчу и ресинкает на 1 байт ⇒
  событие с сериализованным телом > ~64 KiB в пол НЕ попадёт, хотя штатный ридер его принимает.
  Направление ошибки — **занижение** пола, то есть fail-OPEN для валидации декларации.
  **Почему не блокер:** пол занижается, только если ПОСЛЕДНИЕ читаемые события префикса все
  крупнее 64 KiB; реальные события на два порядка меньше (фикстура `snap` — ~2.4 KiB на 100
  уровней; 64 KiB — это ~2700 уровней книги). Кап при этом ОБЯЗАТЕЛЕН для свойства (3) `op_8`
  (независимость памяти от размера): без него мусорная длина `0x5A5A5A5A` копила бы carry
  неограниченно — то есть это несущая конструкция границы памяти, а не магическое число.
  **Слепая зона оракула:** эталон `common::max_seq_in` капа не имеет, поэтому расхождение
  реализации и эталона на фикстурах не проявляется — поймать его оракул не может.
  Что нужно (описание, не проект): назвать предел явно — замером максимального события на проде
  либо согласованием константы с санити-капом ридера. Severity: MINOR.
