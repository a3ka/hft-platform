...
- **TD-083** `любое-WS-подключение-навсегда-заклинивает-прод-gateway` (заведено reviewer'ом на
  ...
  **Состояние прода.** Восстановлено reviewer'ом `docker restart hft-gateway-serve` (перезапуск
  ...
  Severity: **CRITICAL**.
  **СТАТУС 2026-08-03 (reviewer, PR-гейт M-53, `R-026`): КОД-ФИКС СМЕРЖЕН, ЗАКРЫТИЕ ОЖИДАЕТ
  ПОДТВЕРЖДЕНИЯ ПРОГОНОМ ПРОТИВ ПРОДА.** Смержено в `main` (`af2ee7b`): push-цикл
  ...
  **Долг НЕ закрывается по зелёным гейтам:** дефект родом с прода и на фикстурах не
  воспроизводится в принципе. Закрытие — только по чек-листу §11 milestone'а против живого VPS
  (`frames_received > 0`, CPU в норме через 5 мин, `CLOSE_WAIT = 0`, второй клиент
  подключается). Результат прогона — `research/reviews/R-026-M-53.md` §7.
...
- **TD-093** `гонка-снапшот↔live-редьюсер-и-двойная-стоимость-connect-держатся-конструкцией-без-оракула`
  ...
  **Зона: architect** (RED-оракул на (а): продвинуть чекпоинт МЕЖДУ шагом резюме и шагом
  снапшота и потребовать отсутствия задвоения; переоценка (б)/(в) как единой стоимости коннекта),
  далее engine-dev. Severity: **MINOR** (сегодня корректность держится; долг — в отсутствии
  проверки, а не в поведении).
