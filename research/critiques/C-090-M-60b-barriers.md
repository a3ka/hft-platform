> **Перенумерован architect'ом 2026-08-15: `C-089` → `C-090`.** Под `C-089` два разных
> вердикта: этот и `C-089-M-66-rev4.md` на ветке `feat/M-66-fixture`. Оба критика взяли номер
> у аллокатора в пределах трёх минут (19:45 и 19:48) и оба получили свободный — гонка
> `max+1`, тринадцатое зафиксированное срабатывание за четверо суток. По действующему
> правилу номер удерживает тот, чей носитель раньше; здесь — вердикт M-66. Суждение критика
> не редактировалось, изменён только идентификатор.

# C-090 — M-60b barriers: adversarial harness audit
