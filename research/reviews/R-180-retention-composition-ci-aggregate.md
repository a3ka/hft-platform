...
## Дополнение (тот же круг, 2026-09-15T17:40Z) — три блока, предъявленные ЗАМЕРОМ, и PR-поверхность

Первая редакция вердикта вынесла эти три блока выводом, а не командой. По собственному правилу
гейта («каждое утверждение подкреплено командой и её exit-кодом») они добираются здесь, а не
подразумеваются.

### Block-RED — оракулы не переписаны исполнителем

```
$ git log --format='%h %s' --name-status origin/main..origin/harness/aggregate-holds -- '*/tests/*' 'crates/*/tests/**'
b568335 harness: сторож агрегата — каждый needs обязан влиять на исход [architect]
A	scripts/tests/red_ci_aggregate.sh
13c48dd harness: барьер достижимости артефакта покрытия + его проба [architect]
A	scripts/tests/red_retention_composition.sh
```

`crates/*/tests/**` не тронуты вовсе. Обе пробы — статус `A` (ДОБАВЛЕНЫ), обе от `[architect]`.
Ни один dev-коммит оракула не касался: `9ab00fa`, `bc00f99`, `569ea36` тронули по одному файлу
каждый (`docker-compose.yml`, `docker-compose.yml`, `.github/workflows/ci.yml`). Sacred-зона
`*/tests/**` цела; правки «теста под реализацию» в диапазоне нет.

### Block-atomicity — бандлов нет

```
$ for c in $(git rev-list --reverse origin/main..origin/harness/aggregate-holds); do … done
13c48dd  файлов=2   9ab00fa  файлов=1   bc00f99  файлов=1
569ea36  файлов=1   a5abec7  файлов=1   b568335  файлов=2
```

Одна задача = один коммит, 1-2 файла на коммит; бандл на несколько задач (авто-reject по
`commit-discipline.md`) отсутствует. Претензия к форме остаётся ровно одна — F-4.

### Block-FA — барьер `review-fa` действительно SKIP, и предел этим подтверждён

```
$ EVENT_NAME=pull_request PR_BASE_SHA=6f628cb04b6cbf42aacd85b94f32ad7fbd21e934 bash scripts/check_review_fa.sh; echo exit=$?
SKIP (диапазон не трогает crates/**)
exit=0
```

Это не только снимает требование `FA-WAIVER`, но и предъявляет названный предел механизма
(`.claude/agents/reviewer.md`: вне `crates/**` барьер пропускает, там требование когнитивное).
Поэтому инвариант назван вручную и по существу предмета — `JR-I-2` (`docs/fa/journal.md:112`),
см. §Block-scope выше.

### Block-risk — отсутствие risk-critic'а проверено, а не предположено

```
$ grep -rliE "retention|ретеншен|cold|холод" research/critiques/ | xargs grep -lE "aggregate-holds|check_retention_composition|/cold:ro" 
(ничего)
```

Вердикта risk-critic'а на предмет нет, и по триггеру `gates.md` §5 (пути
`crates/risk|killswitch|oms|venue-*`) он не требуется — эти пути в дифе отсутствуют. Эскалация
к founder'у на диспетч risk-critic'а (`.claude/agents/reviewer.md` §Handoff) НЕ производится по
этому основанию. По существу же предмет лежит на пути необратимого удаления данных — это
зафиксировано как F-7 (маршрут) и как `TD-203`, а не как отсутствующий риск-блок.

### PR-поверхность вердикта

Профиль reviewer'а называет формой вердикта PR-комментарий с Block-цитатами. PR на ветку
открыт мною как аудит-поверхность (merge удержан): **PR #183**, комментарий-вердикт —
`#183#issuecomment-5687508379`. Файл остаётся первичным артефактом (`gates.md` §4: вердикт в
переписке аудит-трейлом не является); комментарий — его предъявление там, где живёт работа.
Карточка долга по F-1 — `TD-203`, PR #182.

**Вердикт не меняется: REJECTED.** Три добавленных блока — ЗЕЛЁНЫЕ; блокерами остаются
F-1, F-2, F-6.
