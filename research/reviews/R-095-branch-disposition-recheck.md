...
Причина — механическая и проверена чтением `scripts/check_gate_meta.sh:246-260` (сам вызов
`grep -q 'ALLOW-SUBJECT-CHANGE:'` — `:253`): диапазон
...
Шапка барьера (`scripts/check_resource_oracles.sh:46-71`, буллеты `:53-63`) и тело `a071496`
перечисляют ровно
