...
- **Ярус A (дочитано по поправке, см. §«Поправка» в конце):** `docs/05-contract-layer.md` §4
  — governance Block-C, на который я ссылаюсь.
- **Ярус B:** `milestones/M-73-offsite-schedule.md`, `scripts/verify_M-73.sh`,
  ...
  `scripts/deploy_catchup.py`, **`docs/fa/ops.md`** (FA модуля предмета — §6 инварианты,
  §3 метрики, §7.1 матрица алертов).
...
**И это не только максима — это КОДИФИЦИРОВАННЫЙ инвариант FA того самого модуля, который
трогает M-73.** `docs/fa/ops.md` §6:
...
`OPS-I-3` — дословно предмет этой находки, и он в проекте не новый: `ops.md:405` уточняет
периодичность («раз в месяц холодная копия скачивается и читается»), `ops.md:102` заводит под
него метрику `backup_restore_drill_ok`, а `ops.md:498` — правило алерта `OPS-BKP` «бэкап не
восстанавливается» с severity **P0**. То есть проект уже объявил, что созданная-но-непроверенная
копия есть P0-состояние; M-73 создаёт копию и обязан хотя бы предъявить, что она СОЗДАЁТСЯ
работающим кодом.

Оговорка о границе, чтобы не завышать требование: полный `restore-drill` по `OPS-I-3` — это
`R1b`, отдельный предмет, и я его в круге 3 НЕ требую. Я требую меньшего и посильного —
доказательства, что новый код в принципе исполняется против живого канала (`OPS-I-2`, «второе
место» существует и наполняется). Без этого мы закрываем милестоун про расписание, не увидев
ни одного срабатывания расписания.

**Заодно поправка к теле коммита `6173534`:** дев пишет «`OPS-I-2` — канарейка теперь ПРОХОДИТ
анти-плацебо-проверку». По сути связь есть (канарейка защищает второе место от уничтожения
собственной командой копирования), но `OPS-I-2` — про существование копии в двух местах, а не
про качество теста. Формулировка привязывает живой ID к утверждению о тесте. Не находка,
замечание к точности ссылок.
...
## Поправка (2026-08-30T12:10Z) — пробел МОЕГО startup-протокола, названный founder'ом

Первая редакция этого вердикта была вынесена **без чтения двух документов, которые протокол
reviewer'а называет поимённо**: `docs/fa/ops.md` (мандат называл его прямо) и
`docs/05-contract-layer.md` §4 (ярус A, «целиком, всегда» по `CLAUDE.md`). Пробел обнаружил не
я, а founder вопросом «ты всё выполнил по протоколу». Фиксирую его здесь, а не заминаю: вердикт
гейта, скрывающий собственную неполноту, воспроизводит ровно тот класс, который этот гейт ловит
у других (`TD-138` — утверждение, за которым нет проверки).

**Что изменилось от дочитывания — и это довод против «пробел был безобидным»:**

1. **Находка `Б-4` усилилась с моего СУЖДЕНИЯ до КОДИФИЦИРОВАННОГО инварианта.** `OPS-I-3`
   (`ops.md:471`) требует, чтобы холодная копия восстанавливалась и читалась, а не только
   создавалась; `OPS-BKP` (`ops.md:498`) числит провал этого P0. Я обосновывал требование
   максимой founder'а — а оно всё это время стояло в FA модуля предмета.
2. **Появилось замечание к теле `6173534`** (привязка `OPS-I-2` к утверждению о качестве
   теста) — его в первой редакции не было, потому что я не знал формулировку `OPS-I-2`.
3. **Block-C** я объявлял N/A по факту дифа (это верно и проверено), но governance, на который
   ссылался, прочитан только сейчас: `05-contract-layer.md` §4 — «PR, трогающий `contracts/`
   вне contract-RFC → авто-REJECT». Вывод не изменился, основание стало предъявимым.

**Что НЕ изменилось:** вердикт `REJECT` и оба блокера. Дочитывание вердикт укрепило, а не
развернуло, — но узнать это можно было только дочитав, и до того мой вывод держался на удаче.

**Предъявление FA (M-66).** Барьер `check_review_fa.sh` на этом диапазоне даёт `SKIP` (нет
`crates/**`), то есть машина живого инварианта здесь не требовала — ровно тот НАЗВАННЫЙ предел
механизации, о котором говорит профиль: вне `crates/**` требование остаётся когнитивным. Оно и
не сработало. Живые инварианты тронутого модуля называю теперь явно: **`OPS-I-2`**
(`docs/fa/ops.md:470`) и **`OPS-I-3`** (`:471`) — оба существуют на проверяемой ревизии
`0dacd03`, оба относятся к предмету напрямую.
