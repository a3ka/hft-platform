...
`A-005` (арбитраж), `R-042-M-60a-arbiter-trigger.md` (перенумерован из `R-038` по §F-2)
...
> **СНЯТО (reviewer, 2026-08-07).** Перенумерован документ ЭТОЙ ветки:
> `research/reviews/R-038-M-60a-arbiter-trigger.md` → **`R-042-M-60a-arbiter-trigger.md`**.
> Выбор носителя перенумерации не произволен: `R-038-branch-hygiene` уже в `main` и его
> адрес цитирует `TECH-DEBT.md` TD-110, а `R-038-M-59` живёт на `feat/M-59-lifetime-memory`
> и в предмет M-60a не входит — двигать надо было тот, что ещё вне `main` и внутри предмета.
> Ссылки переведены поимённо в `A-005` (×4), `milestones/M-60a-docs-freeze.md` (×2, §3bis и
> §3ter) и §7 этого файла.
> **Текст находки выше НЕ переписан намеренно.** Он — замер состояния на 2026-08-07 и
> единственная запись о том, что коллизия БЫЛА; замена номеров в нём стёрла бы улику и
> сделала находку бессмысленной («номер `R-042` занят тремя документами» — ложь). По той же
> причине сохранены как есть блок замера с пятью `origin/*` и упоминание «`R-038` §G»
> строкой ниже: там речь о `R-038-branch-hygiene` в `main` — ДРУГОМ документе, который
> никуда не переименовывался. Первая попытка перенумерации (architect, откачена им же)
> сломала ровно эти три места слепой заменой — это дословно класс `TD-098`.
...
2. F-2 снят: `R-038` на этой ветке перенумерован (свободен `R-041`+, `R-039`/`R-040` заняты
   `feat/M-57-task5`). — ✅ **ВЫПОЛНЕНО** reviewer'ом 2026-08-07: `R-038-M-60a-arbiter-trigger`
   → `R-042-M-60a-arbiter-trigger`, ссылки переведены поимённо (см. врезку «СНЯТО» в §F-2).
