...
# R-154 — M-68 круг 6 (резолюция слияния + `N-1`/`N-2`): PR-time reviewer, **REJECTED**

> **ПЕРЕНУМЕРОВАН `R-148` → `R-154` (2026-08-29, `gates.md` §12).** Пока шёл этот круг, в
> `main` приземлился ДРУГОЙ носитель под тем же номером — `research/reviews/R-148-session-close-2026-08-29-recheck.md`
> (коммит `7ad870b`, 15:54:42 UTC; мой первый коммит — `a8b12b0`, 16:08:23 UTC). Правило,
> действующее операционно (`SESSION-HANDOFF` §0bis, «ГОНКА НОМЕРОВ»): **номер удерживает тот,
> чей носитель уже в `origin/main`; пришедший позже берёт следующий** — беру `R-154`
> (`scripts/reserve_artifact_id.sh R`). Резерв, взятый мной в начале круга, был снят после
> приземления на ВЕТКУ, и это его не защитило: аллокатор считает занятость по
> `refs/remotes/origin` ∪ `refs/heads`, а параллельная сессия шла к `main` быстрее. Это
> десятое зафиксированное срабатывание класса — оно же аргумент за CAS-резерв, который
> `gates.md` §12 уже описывает и который ждёт токена founder'а.
>
> **Метки `R148-PROBE` / `R148-SETUP` / `R148-EXPECT` в блоках ниже НЕ переименованы
> намеренно:** это СЫРОЙ stdout прогонов, сделанных под прежним номером. Переписать их
> значило бы подделать вывод команды — `commit-discipline.md` требует сырого, а не
> причёсанного.
