...
## Дополнение ревьюера (2026-09-07, ПОСЛЕ merge'а) — предъявление яруса C и названные пробелы

Дополнение, а не правка: вердикт APPROVED и его основания не меняются. Меняется полнота
предъявления — профиль `reviewer` требует НАЗЫВАТЬ, что именно искалось грепом в
`PROJECT-STATE.md`/`TECH-DEBT.md` (ярус C, `docs/workflow/reading-map.md` §2: два файла ≈
226 k токенов, «прочитал целиком» — заведомо ложное утверждение). В первой редакции этой
строки не было. Замалчивать собственное отступление в файле, который сам требует
предъявления, — тот же класс, что и «этап пройден, потому что роль отработала».

**Гроп-предъявление яруса C (исполнено ДО вердикта, названо здесь):**

```text
$ grep -n "TD-199" TECH-DEBT.md              → :134 (карточка-предмет) + :3850 (тело)
$ grep -n "M-77"   TECH-DEBT.md              → совпадений нет (карточки под номером милестоуна не было)
$ grep -n "M-77|VB-I-2" PROJECT-STATE.md     → :1478 :1890 :1949 :2243 — все о ПРОШЛЫХ кругах
                                               (M-71 GW-I-4/VB-I-2, M-57 каталог, M-48 провенанс,
                                               M-56 finish/finish_ref); раздела M-77 не было
$ bash scripts/next_artifact_id.sh TD        → TD-200 (номер свободен, взят механизмом)
```

Искомое: (i) есть ли уже карточка на предмет `M-77` — нет, предмет несёт `TD-199`;
(ii) не заведён ли номер повторно — нет, номера взяты `reserve_artifact_id.sh`;
(iii) не описан ли раздел `M-77` в `PROJECT-STATE.md` кем-то раньше — нет.

**Пробелы чтения, названные явно (`reading-map.md` §1 — правило цитаты):**

- `docs/fa/viz-backend.md` прочитан НЕ ЦЕЛИКОМ: §5 (инварианты `VB-I-1..11`) и начало карты
  `MD-I-8`. Инварианты, названные в вердикте (`VB-I-2` `:199`, `VB-I-10` `:207`), открыты и
  процитированы; на непрочитанные разделы вердикт не ссылается.
- `milestones/M-77-*.md` прочитан с §5 и далее целиком (§5–§13) плюс §0/§1; §2–§4 (замер,
  механизм, «почему не ловилось») — НЕ прочитаны. Вывод вердикта на них не опирается:
  предмет проверен ПРОГОНОМ и МУТАЦИЕЙ, а не пересказом спеки.
- `C-211`/`C-212`/`C-213` прочитаны в части `GATE-META` + решение + объявленный охват круга.
  Закрытие трёх условий `C-211` принято по ИСПОЛНЕННЫМ шагам гейта (`T7`/`T8` — исполнение
  оракулов, не присутствие текста) и по вердикту `C-213` NOTE, а не по собственному разбору
  находок критика. Это ослабление названо, а не выдано за полное чтение.

**Прод-пруф `§8` уточнён замером, а не оставлен догадкой.** В первой редакции цифра памяти
сервиса выдачи была снята по pid'у без проверки, что это ТОТ процесс. Проверено:

```text
$ docker top hft-gateway-serve -o pid,rss,args
PID       RSS     COMMAND
2080225   4092    /usr/local/bin/gateway-serve
$ grep RssAnon /proc/2080225/status   → RssAnon:  400 kB    (сервис ИДЛИТ: подписчиков нет)
$ grep RssAnon /proc/2080115/status   → RssAnon: 21884 kB   (recorder, пишущий путь)
```

Предел прежний и повторён здесь: 400 кБ — это память ПРОСТАИВАЮЩЕГО сервиса. Изменённый
путь (`pump` под живой подпиской) на проде сегодня не исполняется вовсе, поэтому прод-пруф
покрывает ЖИВОСТЬ, а не поведение правки. Поведение держат оракулы и мутационный контроль
выше — и `TD-201` ровно о том, что удержание нового буфера не сторожит ни один из них.
