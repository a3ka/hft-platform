...
**Маршрут:** предмет решения — граница C, и арбитру он НЕ передаётся (`gates.md` §0.1). Но АДРЕСАТ вердикта — `architect` (`.claude/agents/reviewer.md:46`): разбор находки гейта ведёт он и через founder'а диспетчеризует дальше. Вопрос founder'у несёт architect, а не я через его голову. См. поправку в конце файла.
...
Ни один пункт не возвращается engine-dev'у: его работа принята целиком. Оба пункта ведёт **`architect`** — он же выносит вопрос §3sexies (а) founder'у и он же заказывает круг критика.
...
# ПОПРАВКА REVIEWER'А К СОБСТВЕННОМУ ВЕРДИКТУ (2026-09-17T20:05Z) — маршрут §D был НЕВЕРЕН

**Нарушение названо founder'ом сразу после сдачи вердикта, и оно настоящее.** Первый Handoff
этого круга назвал следующим агентом `founder`. Живая норма говорит обратное:

```
$ sed -n '46p' .claude/agents/reviewer.md
- REJECT/CHANGES REQUESTED → **`architect`**: разбор находки — его, не dev'а (решение
$ sed -n '57,59p' .claude/agents/reviewer.md
...
Правильный маршрут: **reviewer → architect**. Architect ведёт разбор находки, правит спеку/
оракул сам либо через founder'а диспетчеризует dev, и **он же** выносит вопрос границы C
founder'у. Прямой возврат мимо architect'а отменён решением founder'а 2026-09-04
(`92aafd3`, `docs/workflow/session-handover-2026-09-04.md` §4 п.2).

**Причина — установлена замером, и она НЕ оправдание.** Профиль, впрыснутый в мою сессию,
оказался копией, устаревшей до смены маршрута:

```
$ git log --format='%h %ad %s' --date=short -1 92aafd3 -- .claude/agents/reviewer.md
92aafd3 2026-09-05 docs(process): REJECT/CONCERNS возвращается architect'у, а не dev'у [architect]
$ grep -c 'SVR-response' .claude/agents/reviewer.md        # формулировка из моего впрыска
0                                                          # на диске её нет ВООБЩЕ
```

Впрыснутая копия несла отменённую редакцию («REJECT → dev-агент, который делал impl»), я
взял из неё маршрут и, отказавшись возвращать отказ dev'у (это в ней же), адресовал вердикт
founder'у напрямую. **Но ярус A стартового протокола требует ОТКРЫТЬ `.claude/agents/<своя
роль>.md`** — `reading-map.md` §2 даже велит снимать его размер КОМАНДОЙ, потому что число
там уже протухало. Я этого не сделал и утверждал о профиле, не открыв профиль.

**Класс — `reading-map.md` §3, правило цитаты.** Тот самый, за который я в этом же вердикте
выписал `Н-1` tester'у (FA-WAIVER на основание, которого в барьере нет). Симптом совпал
дословно: утверждение о механизме без открытия механизма. Пишу это здесь, а не молча правлю
строку, потому что вердикт, скрывающий поправку к себе, аудит-трейлом быть перестаёт
(прецедент — `053b255`, `R-172`).

**Что поправка НЕ меняет.** Ни один блокер, ни одна находка, ни один прогон. Б-1 и Б-2 в силе
дословно, вердикт остаётся `REJECT`. Меняется ровно адресат.

**Отдельно — наблюдение о самом механизме, не о себе.** Маршрут объявлен `COGNITIVE-ONLY`
(`.claude/agents/reviewer.md:56-59`), барьера у него нет, и предъявленный здесь случай —
второй известный носитель класса: первый (`92aafd3`) чинил ДВА профиля с отменённой
редакцией, этот показывает, что отменённая редакция продолжает доезжать до роли ВПРЫСКОМ,
даже когда файл в `main` давно исправлен. Кандидат-механизм в профиле уже назван — проверять
`research/reviews/R-*.md` с `verdict: REJECT` на упоминание `architect` в §D, разбор
`scripts/check_gate_meta.sh` эти файлы уже делает. Заводить долг — не моя правка вслепую:
это зона architect'а (`gates.md` §9), и я передаю наблюдение ему §E Handoff'а.
