...
2. **`docs/workflow/reading-map.md` — карта чтения (ярусы A/B/C).** Корпус 2.6 MB ≈ 580 k
   токенов: «прочитать всё» невыполнимо, а невыполнимое требование заменяется догадкой.
   **Ярус A целиком, всегда:** `DESIGN.md` · `04-workflow.md` · `05-contract-layer.md` ·
   `SESSION-HANDOFF.md` §0/§0bis. **Ярус B — по предмету**, по карте там же. **Ярус C**
   (`TECH-DEBT`, `PROJECT-STATE`, `PENDING-SIGNATURE`, критики/ревью) — ТОЛЬКО грепом по
   предмету; «прочитал `TECH-DEBT`» — заведомо ложное утверждение.
2bis. **`docs/ROADMAP.md`** — ПОРЯДОК работ: с чего продолжать. Барьер `roadmap-sync`.
3. **Правило цитаты:** «документ X говорит Y» требует ОТКРЫТИЯ X. Пересказ в спеке, шапка
   чужого оракула, выписка субагента, память прошлой сессии — не основание. За одну сессию
   16.08 нарушено пятикратно (`reading-map.md` §1).

> **`docs/ORCHESTRATION-STATE.md` убран из протокола** (founder, 2026-08-04): 96 % объёма —
> журналы эксперимента с агентом-оркестратором при 128 KB обязательного чтения каждой ролью.
> Живой индекс — `SESSION-HANDOFF` §0; журналы — `docs/archive/`. Тот же класс дефекта, что
> закрывает карта чтения выше.
