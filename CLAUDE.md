...
2. `docs/SESSION-HANDOFF.md` — состояние проекта между сессиями; §0 несёт индекс пайплайна
   (что на каком гейте и каким файлом это предъявлено).
3. `docs/PENDING-SIGNATURE.md` — что ждёт founder'а (границу C не переступать).
4. `docs/DESIGN.md` → релевантный `docs/fa/<модуль>.md`.
5. `PROJECT-STATE.md` + `TECH-DEBT.md` — что сделано и какой долг открыт.

> **`docs/ORCHESTRATION-STATE.md` из протокола убран** (founder, 2026-08-04: «в рамках
> стандартного воркфлоу, когда я оркестратор, этот файл не нужен»). Он появился в период
> эксперимента с передачей оркестрации architect'у; замер аудита показал, что 96 % его
> объёма — журналы того эксперимента, а не состояние, при цене 128 KB обязательного чтения
> каждой ролью. Живой индекс переехал в `SESSION-HANDOFF` §0, журналы — в
> `docs/archive/`. При человеке-оркестраторе очередь несут handoff-блоки, ветки и артефакты
> гейтов (`gates.md` §4), а не центральный файл.
