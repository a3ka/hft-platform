...
//!
//! ```text
//! frames_since_with_stats → journal::stream_from(dir, filter, after.upto_seq)   ← SEEK ЗА курсор
//!                         → reduce_event_stream(…)
//!                         → let mut reducer = Reducer::new(selector)            ← КНИГА ПУСТА
//! ```
//!
//! Событий ДО `after` этот путь не видит вовсе (seek), а депт-серия считается ИЗ КНИГИ. На
//! дельта-хвосте без якорного снимка в окне книга неполна ⇒ точки ряда либо не рождаются, либо
//! рождаются с другим содержимым. Полный реплей книгу имеет ⇒ пути расходятся, и `VB-I-2`
//! («серия, посчитанная на live-хвосте, бит-идентична серии из replay того же окна») нарушен.
//!
    ...
    for side in ["bid", "ask"] {
        ...
        assert_eq!(
            ...
            "VB-I-2 НАРУШЕН на пути `frames_since`: ряд точек депт-серии, собранный клиентом \
             ...
             полоса={BAND}.\n  реплей: {want:?}\n  клиент: {got:?}\n\
             Причина одна и она структурная: `frames_since_with_stats` делает \
             `journal::stream_from(dir, filter, after.upto_seq)` (seek ЗА курсор) и отдаёт окно \
             свежему `Reducer::new` — книга пуста, а депт-серия считается ИЗ книги. `M-77` \
             закрыл это ТОЛЬКО для `pump` (`book_series_in`, два вызова, оба внутри `pump`)."
        );
    }
}
