...
//! (`C-138` п.3).
//!
//! # ОБЪЕКТ ЭТОГО ОРАКУЛА ОБЪЯВЛЕН: это УРОВЕНЬ 1 (`A-021`, обязательный набор п.3)
//!
//! Здесь судятся байты `Snapshot` — собственного объекта крейта `gateway`. `C-158` назвал это
//! «границей не того ресурса», и на момент того вердикта замечание было верным: спека тогда
//! обещала мерить полный исходящий текст, а мерила внутренний объект. Решение арбитра развело
//! два инварианта по двум крейтам, и внутренний объект стал **заявленным** предметом уровня 1,
//! а не подменой внешнего.
//!
//! Граница ПОЛНОГО исходящего текста — уровень 2,
//! `crates/gateway-serve/tests/red_egress_cap_wire.rs`. Бинарный поиск там не нужен и не
//! делается: достаточно одного straddle-кейса отказа сверх предела (`A-021` п.3), потому что
//! величина конверта не тождественна величине внутреннего объекта и «одна сущность» на
//! уровне 2 не определена — echo `sub`-id меняет вес сообщения, не меняя числа сущностей.
//!
