//! **ЗЕЛЁНЫЙ СТОРОЖ, не RED** (`A-033`) — кадр несёт РОВНО 14 строк `(band, side)` на
//! настоящем пути `gateway::snapshot`, стороны не перепутаны.
//!
//! Заведён закрытием `C-220` B-3: обещание жило в тексте спеки, оракула не было, и
//! транспорт, отдающий подмножество строк, проходил набор. Против СЕМИПОЛОСНОГО селектора
//! свойство верно УЖЕ сегодня — поэтому файл зелен до и после реализации, а красным станет
//! ровно тогда, когда кто-то сломает раскладку строк.
//!
//! НОВОЕ в M-84 — не «14 строк на семь полос» (это арифметика), а «14 строк НЕЗАВИСИМО от
//! того, что попросил клиент». Вторая половина наблюдаема только на пути клиента и живёт в
//! `crates/gateway-serve/tests/red_fixed_bands_entrypoint.rs`.
//!
//! Фикстура АСИММЕТРИЧНА (объёмы ask вдвое больше bid): на симметричной перепутанные
//! стороны неотличимы (`testing.md`, дегенерированный вход, п.1).
...
fn sides_are_not_swapped() {
    ...
    .expect("snapshot");
    let widest = (PRODUCT_BANDS.last().unwrap() * 1e8).round() as i64;
