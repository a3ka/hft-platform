...
//! (`docs/fa/viz-backend.md` §5). Спека — `docs/archive/M-88-liquidity-removal-contract.md`.
//!
...
// ─────────── S10 — НЕСКОЛЬКО ЖИЗНЕЙ одного уровня (`C-233` R3) ───────────

/// Обязательный пункт чек-листа `testing.md` («множественность: несколько ЖИЗНЕЙ одной
/// сущности»), которого в первой редакции набора НЕ БЫЛО — и это нашёл гейт, а не автор
/// (`C-233` R3). S9 менял в том же бакете ДРУГУЮ цену; здесь одна и та же цена рождается,
/// снимается и рождается снова ВНУТРИ ОДНОГО бакета, а дельты разрезаны на разные кадры.
///
/// **Этот тест СЕГОДНЯ ЗЕЛЁНЫЙ, и это сказано прямо, а не спрятано.** Объединение по ключу
/// с победой последнего возрождение не теряет, поэтому дефекта здесь нет. Он заведён как
/// СТОРОЖ ПРОТИВ НЕВЕРНОЙ ПОЧИНКИ: наивная реализация «удалить ключи, которых нет в новом
/// срезе» отработает первый кадр верно и сотрёт ВОЗРОЖДЁННУЮ ячейку вторым, если срез
/// колонки накапливается, а не берётся из ПОСЛЕДНЕГО наблюдения. Зелёный RED-набор ничего
/// не доказывает; зелёный сторож доказывает, что починка не купила исправление ценой
/// соседнего инварианта (`testing.md`, второй вопрос мутационного контроля).
#[test]
fn s10_level_reborn_inside_one_bucket_survives() {
    let _g = serial();
    let s = sel(vec![0.001]);
    let (dir, seqs) = journal_of(vec![
        snapshot_ev(&[(64_990.0, 5.0), (64_980.0, 3.0)], &[(65_010.0, 4.0)], T),
        delta_ev(&[(64_990.0, 0.0)], &[], 1, 2, T + 1), // жизнь 1 закончилась
        delta_ev(&[(64_990.0, 7.0)], &[], 2, 3, T + 2), // жизнь 2 началась
    ]);
    let full = full_snapshot(dir.path(), &s);
    let t0 = T / 1000;

    assert_eq!(
        heatmap_of(&full).get(&(t0, "bid".to_string(), to_fixed(64_990.0))),
        Some(&to_fixed(7.0)),
        "setup-страж: полный пересчёт обязан показать ВОЗРОЖДЁННЫЙ размер 7, а не 5 и не пусто"
    );

    // Дельты разрезаны на РАЗНЫЕ кадры: по одному событию за раз.
    let base = snapshot_at(dir.path(), &s, Cursor::at(seqs[0]));
    let mut cur = Cursor::at(seqs[0]);
    let mut frames = Vec::new();
    loop {
    ...
    assert!(
        frames.len() >= 2,
        "setup-страж: нужно ≥2 кадра (снятие и возрождение врозь), получено {}",
        frames.len()
    );
    ...
    assert_eq!(
        heatmap_of(&acc),
        heatmap_of(&full),
        "VB-I-2: цена прожила две жизни внутри одного бакета — у клиента осталась либо \
         старая жизнь, либо ни одной"
    );
}

/// ГРАНИЦЫ (`testing.md` п.4, `C-233` R3): размеры у краёв представимого диапазона.
/// Фикстура намеренно НЕ стоит ровно на границе `i64`: округление увело бы её в соседний
/// диапазон, и тест падал бы по неверной причине.
#[test]
fn s11_extreme_sizes_survive_the_round_trip() {
    let _g = serial();
    let s = sel(vec![0.001]);
    // 1e10 в единицах ×1e8 — это 1e18, на порядок ниже i64::MAX (≈9.22e18).
    const HUGE: f64 = 10_000_000_000.0;
    // Наименьшая представимая единица: 1 в фиксированной точке.
    const TINY: f64 = 0.000_000_01;
    let (dir, seqs) = journal_of(vec![
        snapshot_ev(&[(64_990.0, HUGE), (64_980.0, TINY)], &[(65_010.0, 4.0)], T),
        delta_ev(&[(64_990.0, 0.0)], &[], 1, 2, T + 1),
    ]);
    let full = full_snapshot(dir.path(), &s);
    assert_eq!(
        heatmap_of(&full).get(&(T / 1000, "bid".to_string(), to_fixed(64_980.0))),
        Some(&to_fixed(TINY)),
        "setup-страж: наименьший представимый размер обязан уцелеть в пересчёте, иначе \
         фикстура проверяет не границы, а усечение"
    );
    ...
    assert_eq!(
        heatmap_of(&acc),
        heatmap_of(&full),
        "VB-I-2 на краях диапазона: огромный размер снят, наименьший обязан уцелеть"
    );
}

// ─────────── S12 — книга стала ОДНОСТОРОННЕЙ (найдено векторным оракулом) ───────────

/// Снята последняя ячейка одной стороны: книга становится односторонней, середина по
/// свежим данным невычислима, и обе ветви — наблюдение и полный пересчёт — обязаны
/// разрешить это ОДИНАКОВО.
///
/// Сценарий найден НЕ разбором, а эталонным вектором `s6_duplicate_frame`: модель
/// потребителя сошлась в пустую карту там, где полный пересчёт отдаёт ячейку уцелевшей
/// стороны. Семь названных сценариев §9 этого не покрывали — они снимали уровни, оставляя
/// книгу двусторонней либо пустой целиком.
#[test]
fn s12_one_sided_book_after_removal_matches_recompute() {
    let _g = serial();
    let s = sel(vec![0.001]);
    let (dir, seqs) = journal_of(vec![
        snapshot_ev(&[(64_990.0, 5.0)], &[(65_010.0, 4.0)], T),
        trade_ev(65_000.0, 1.5, Side::Buy, T + 10),
        // Снимается ПОСЛЕДНИЙ bid: остаётся только ask — книга односторонняя.
        delta_ev(&[(64_990.0, 0.0)], &[], 1, 2, T + 30),
    ]);
    ...
    assert_eq!(
        heatmap_of(&acc),
        heatmap_of(&full),
        "VB-I-2 на ОДНОСТОРОННЕЙ книге: наблюдение и полный пересчёт разрешают \
         невычислимую середину по-разному. Клиент видит не то, что даёт реплей"
    );
}
