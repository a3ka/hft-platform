...
/// Якорь: широкая книга. Уровень `MID*0.999` (0.1 % от середины) лежит ВНУТРИ `PROD_BAND`,
/// уровень `MID*(1±reach)` — снаружи. Именно из-за первого реплей даёт по прод-полосе
/// НЕНУЛЕВУЮ глубину, которую книга-из-одних-дельт воспроизвести не может.
fn anchor(ts: i64, reach: f64) -> EventKind {
    EventKind::md(
        Venue::Binance,
        "BTCUSDT",
        MdPayload::L2Snapshot {
            bids: vec![lvl(MID * (1.0 - reach), 3.0), lvl(MID * 0.999, 4.0)],
            asks: vec![lvl(MID * (1.0 + reach), 3.0), lvl(MID * 1.001, 4.0)],
            ts_exch_ms: ts,
        },
    )
}
