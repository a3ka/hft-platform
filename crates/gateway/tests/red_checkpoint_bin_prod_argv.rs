...
fn d18g_garbage_cadence_is_rejected_naming_the_variable() {
    ...
    // Прод-argv несёт `--depth-cadence-ms`. Флаг имеет приоритет над env ТОЛЬКО ПРИ
    // ВАЛИДНОМ env: при мусоре в переменной бинарь отвергает старт независимо от флага
    // (замер на прод-бинаре: флаг 2000 + env `abc` → exit=2, ровно как и без флага).
    // Прежняя редакция комментария писала «ФЛАГ имеет приоритет над env» без оговорки —
    // утверждение, опровергаемое одним прогоном (`R-154` `N-2`). Чтобы судить именно
    // env-путь, флаг из argv убирается — иначе тест мерил бы не то, что обещает.
