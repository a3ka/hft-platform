...
fn stale_schema_version_checkpoint_rebuilds_silently_and_overwrites() {
    ...
    // Байты ВАЛИДНОГО чекпоинта — эталон детерминизма (см. ассерты ниже).
    let valid = std::fs::read(&path).expect("read valid");

    // Моделируем будущий штатный bump схемы: файл снят прежней версией кода.
    corrupt_gw_schema_version(&path);
    // Байты УСТАРЕВШЕГО файла — именно они не должны пережить самолечение.
    let stale = std::fs::read(&path).expect("read stale");
    assert_ne!(valid, stale, "фикстура: порча обязана изменить файл");
    ...
    // (1) Устаревший файл НЕ пережил самолечение — иначе он останется на диске навсегда,
    // и каждый следующий запуск будет платить полный реплей.
    assert_ne!(
        after, stale,
        "файл обязан быть ПЕРЕЗАПИСАН свежим чекпоинтом, а не оставлен устаревшим"
    );
    // (2) И ровно наоборот к (1): rebuild ДЕТЕРМИНИРОВАН — на том же журнале и селекторе он
    // обязан воспроизвести БАЙТ-В-БАЙТ тот же валидный чекпоинт, что был до порчи. Это не
    // придирка: недетерминизм здесь (wall-clock/счётчик в заголовке) сломал бы GW-I-9(в)
    // `advance_idempotent` и байт-идентичность реплея (DET-I-1).
    //
    // ИСПРАВЛЕНО ПО SCOPE VIOLATION REQUEST engine-dev'а (обоснованному): прежняя версия
    // сравнивала `after` с байтами ДО порчи и требовала их РАЗЛИЧИЯ — то есть требовала
    // недетерминизма и была непроходима ни для какой корректной реализации. Мой дефект:
    // я перепутал эталон (валидный файл) с тем, что должно исчезнуть (устаревший файл).
    // Dev поступил верно, отказавшись «подогнать» реализацию под противоречивый тест.
    assert_eq!(
        after, valid,
        "детерминированный rebuild обязан воспроизвести тот же валидный чекпоинт байт-в-байт \
         (расхождение здесь означает недетерминизм в заголовке/состоянии — он сломает \
         идемпотентность advance и байт-идентичность реплея)"
    );
    assert_eq!(
        &after[12..16],
