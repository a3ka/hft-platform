...
/// TD-040 (ред. 17.08, `C-095` F-4): counting-allocator ведёт учёт ПОТОКОВО (`thread_local!`
/// ниже), а НЕ процесс-глобально — прежняя редакция этого комментария противоречила коду
/// шестью строками ниже и осталась от снятой реализации. cargo гоняет тесты одного бинаря
...
fn snapshot_memory_bounded_by_window_not_history() {
    ...
    for row in &snap_many.series.depth_series {
        ...
        assert!(
            // ФОРМА АДДИТИВНАЯ (§2bis.-1): точка — кортеж. Предмет `VB-I-10` не тронут:
    ...
    // M-88 / `R-195` Б-1: СПИСОК НАБЛЮДЁННЫХ КОЛОНОК — тоже бакет-ключевое состояние.
    //
    // Почему его здесь не было и почему одного порога по объёму мало. Поле
    // `heatmap_observed_time_s` завёл M-88, и оба сторожа окна перечисляют поля ПОИМЁННО —
    // в перечень оно не попало и попасть само не могло. Абсолютный бюджет (2) находку тоже
    // не поймал: на 20 000 бакетов список весит ≈220 КБ, на порядок ниже порога 8 МиБ, —
    // а на прод-глубине (2 месяца) он съедает ВЕСЬ бюджет ответа и роняет выдачу целиком
    // (`PL-I-5`, замер ревьюера: 2 423 152 Б при лимите 2 000 000 Б, содержательные
    // счётчики нулевые). То есть порог и перечень ловят РАЗНОЕ, и перечень здесь решающий.
    //
    // Класс «новое поле невидимо перечню» отдельно закрыт канарейкой
    // `red_m88_observed_window::w4_every_bundle_field_is_classified_as_windowed_or_not`:
    // она разбирает `SeriesBundle` исчерпывающе, и следующее поле не скомпилируется, пока
    // автор не отнесёт его к оконным или к неоконным явно.
    assert!(
        !snap_many.series.heatmap_observed_time_s.is_empty(),
        "setup-страж: список наблюдённых колонок ПУСТ — L2-фикстура не дошла до пути карты, и проверка ниже судила бы пустоту"
    );
    assert!(
        snap_many.series.heatmap_observed_time_s.len() <= window_buckets
            && snap_many
                .series
                .heatmap_observed_time_s
                .iter()
                .all(|&t| t >= lo && t <= max_ts),
        "heatmap_observed_time_s вне окна (len={}, окно={window_buckets}, [{lo},{max_ts}]) — VB-I-10: состояние обязано следовать окну, а не числу бакетов истории",
        snap_many.series.heatmap_observed_time_s.len()
    );
    assert!(
