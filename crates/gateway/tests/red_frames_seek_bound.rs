...
fn td083_tick_wallclock_does_not_grow_with_history() {
    ...
    let tick_work = |events: u64| -> (u64, u32) {
        ...
        (st.events_scanned, st.segments_opened)
    };

    let (work_small, segs_small) = tick_work(1_000);
    let (work_big, segs_big) = tick_work(8_000);
    eprintln!(
        "TD-083 O-B (работа): small work={work_small} segs={segs_small} | \
         big work={work_big} segs={segs_big} (бюджет work<={}, segs<=2)",
        INCREMENT * 4
    );

    // РЕМОНТ ОРАКУЛА — решение A-004 §C.1 (независимый разбор, 2026-08-05).
    // Бюджет АБСОЛЮТНЫЙ и на ОБА плеча. Прежняя мера задавалась отношением
    // времён big/small и имела ИНВЕРТИРОВАННЫЙ знак: мутант с восстановленным
    // полным пересканированием проходил порог (ratio 2.3-2.7), а здоровый код
    // падал (4.9-6.2) — пересканирование дорожает в ОБОИХ плечах и топит
    // знаменатель. Абсолютный счётчик так обмануть нельзя: против той же мутации
    // (`segments.rs:1144`, `Ok(saved_offset)` -> `Ok(header_end)`) он даёт 260
    // прочитанных событий при приращении 3 — детерминированно, дважды подряд,
    // независимо от загрузки машины.
    for (label, work) in [("1 000", work_small), ("8 000", work_big)] {
        assert!(
            work <= INCREMENT * 4,
            "TD-083 O-B (работа): тик на журнале в {label} событий ПРОЧИТАЛ {work} событий при \
             приращении всего в {INCREMENT}. Значит журнал читается С ГОЛОВЫ, а не с курсора. \
             На проде (168M событий до курсора, активный сегмент до 1 GiB) это ~17 с на тик при \
             периоде 250 ms: live-push молчит, accept-loop мёртв."
        );
    }
    for (label, segs) in [("1 000", segs_small), ("8 000", segs_big)] {
        assert!(
            segs <= 2,
            "TD-083 O-B (сегменты): тик на журнале в {label} событий открыл {segs} сегментов при \
             приращении {INCREMENT}. Догнавший хвост тик обязан трогать активный сегмент и, при \
             ротации ровно на границе, максимум один соседний."
        );
    }
    ...
    // АССЕРТ ОТНОШЕНИЯ ВРЕМЁН СНЯТ — решение A-004 §C.1 п.1, исполняет architect
    // отдельным oracle-repair кругом (не M-57 dev/tester/reviewer и не critic).
    // Причина — не флак и не неверный порог: у меры инвертирован ЗНАК, она зеленела
    // на дефекте и краснела на исправлении (A-004 §B.3, три чередующиеся пары).
    //
    // ЧТО ПОТЕРЯНО, названо прямо (A-004 §C.1 п.2): детерминированного трипваера на
    // МЕТАДАННЫЕ-ПУТЬ после этой правки НЕТ. Замер ремонта: `segments_opened` = 1 для
    // журнала и в 1 000, и в 8 000 событий, тогда как каталог различается ~в 7 раз —
    // счётчик обхода каталога не видит по конструкции. Долг заводится reviewer'ом при
    // close-out M-57 (A-004 §C.2): «метаданные-путь `stream_from` — O(#сегментов) на
    // каждый тик; 183 сегмента и +12/день на проде; трипваера нет». Закрывается
    // отдельным milestone'ом с собственными счётчиками прибора.
    //
    // Печать времени сохранена как ДАННЫЕ для того milestone'а — без ассерта.
}
