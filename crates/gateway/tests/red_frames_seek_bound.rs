...
fn td083_tick_wallclock_does_not_grow_with_history() {
    ...
    // ВРЕМЯ ОСТАЁТСЯ ДИАГНОСТИКОЙ, А НЕ АССЕРТОМ. Wall-clock под `cargo test`, гоняющим
    // соседние тесты параллельно, меряет окружение, а не инвариант (`testing.md`,
    // «целостность гейта», свойство 2): падение 1 из 3 прогонов подряд при неизменном коде.
    // Ассерт стоит на мере 1, которая теперь честная. Число печатается, чтобы деградация
    // была ВИДНА человеку, но не роняла гейт по вине планировщика.
    eprintln!(
