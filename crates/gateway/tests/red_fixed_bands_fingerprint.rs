...
fn band_axis_no_longer_splits_fingerprint() {
    // Аня и Боря из П-029: разные наборы ДО фиксации. После фиксации оба обязаны собираться
    // с каноническим набором и давать ОДИН отпечаток.
    let anya = sel(gateway::CANONICAL_DEPTH_BANDS.to_vec(), 1_000, Some(60_000));
    let boris = sel(gateway::CANONICAL_DEPTH_BANDS.to_vec(), 1_000, Some(60_000));
    assert_eq!(
        selector_fingerprint(&anya),
        selector_fingerprint(&boris),
        "два селектора с каноническим набором обязаны давать ОДИН отпечаток — иначе слепок \
         не переиспользуется и П-010 не получает даже необходимого условия"
    );
}
