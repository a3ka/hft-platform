...
impl Reducer {
    ...
    /// M-68 rev7 (R-141 Б-2): `flush_pending_depth_interval` УДАЛЁН из `finish`. Функция
    /// сохранена в виде no-op для случая, если внешний код (например, оракул-изолятор)
    /// позже решит диагностировать открытый интервал — это уже не путь прод-цикла.
    /// До rev6 функция делала сброс незакрытого последнего каденс-интервала
    /// (§0sexies.2ter rev6), что нарушало `VB-I-2` на live-пути: live-снимок
    /// (`LiveReducer::snapshot` → `finish_ref(&self)`) сбросить не мог — `&mut self`
    /// там физически отсутствует (архитектурное решение `M-56`/`TD-097`, клонировать
    /// состояние запрещено). С rev7 оба пути (live/replay, через `finish(self)` и
    /// `finish_ref(&self)`) дают одну и ту же серию — незакрытый интервал остаётся
    /// в `depth_cadence_current_bucket` и не эмитится.
    #[allow(dead_code)]
    ...
    fn finish(self) -> SeriesBundle {
        ...
        // путь (`finish_ref(&self)`) расходится с replay (`finish(self)`): см.
        // `flush_pending_depth_interval` ниже и `docs/fa/viz-backend.md:189` `VB-I-2`.
        // Семантика выбрана «не эмитить на обоих» — единая для обоих путей, держит
        // `&self` на live-пути (M-56/TD-097), не требует клонирования состояния.
        // Альтернатива «эмитить на обоих» нарушила бы M-56.
        let me = self;
        me.finish_ref()
    }
    ...
    fn finish_with_at(self) -> (SeriesBundle, i64) {
        ...
        // и `flush_pending_depth_interval`). Здесь — только достать `at_ms`. Так обе двери —
        // `finish` и `finish_with_at` — остаются эквивалентны по семантике: после `M-56`
        // они делят одну формулу, после `M-68` rev7 — и одну политику относительно
        // незакрытого интервала (НЕ эмитить).
