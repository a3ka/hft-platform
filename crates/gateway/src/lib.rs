...
/// 3: M-88 (`milestones/M-88-*.md` §3/§4): в состояние `Reducer` добавлены проекции
...
struct Reducer {
    ...
    /// с пустым множеством, и при первом же `pump()` (если он вне LiveReducer) или при первом
    /// L2-событии в `apply()` (для `frames_since`/`replay`) проекция заполнится заново.
    #[serde(default)]
    heatmap_buckets_observed: std::collections::BTreeSet<i64>,
    /// M-88 §3/§4: `true` ⇔ в окне редьюсера было ХОТЯ БЫ одно L2-событие (`L2Snapshot` или
...
/// ЯКОРЬ МУТАЦИИ (см. milestone §шаг C; verify-скрипт милестоуна — артефакт architect'а,
/// путь к нему НЕ выносится в чужую зону, иначе переезд гейта M-71 в архив блокирован).
/// Сигнатура зафиксирована спекой; нейтрализация тела этой функции (возврат `Ok(())` без
/// проверки) ОБЯЗАНА ронять набор оракулов (`A`/`B`/`C`/`F` уровня 1 + `W3`/`W5` уровня 2)
/// и НЕ ронять контроль `E` — это и проверяет мутационный шаг.
///
