...
fn main() -> ExitCode {
    ...
    // `GATEWAY_DEPTH_CADENCE_MS` (env, общее с `gateway-serve`) → дефолт 1000.
    //
    // ОГОВОРКА, БЕЗ КОТОРОЙ СТРОКА ВЫШЕ ЛОЖНА (`R-154` `N-2`): приоритет держится ТОЛЬКО
    // на ВАЛИДНОМ значении env. На мусоре переменная отвергается ДО того, как
    // `args.depth_cadence_ms.or(cadence_from_env)` вообще спросит флаг, — и «с флагом» и
    // «без флага» неразличимы. Замер на прод-бинаре, прод-форма argv из `docker-compose.yml`:
    ...
    //   D без флага                 + `GATEWAY_DEPTH_CADENCE_MS=4000` → exit=0
    // Решает пара A≡C: на мусоре исход одинаков, то есть флаг не спрашивают вовсе.
    // ЧТО ЭТИ ЧИСЛА НЕ ПОКАЗЫВАЮТ, и это названо, а не умолчано: пара B≡D тоже одинакова,
    // значит по КОДУ ВОЗВРАТА «какое значение победило» не наблюдаемо ни в одну сторону —
    // приоритет флага на валидном env держится разбором `or(...)`, а не этим замером.
    // (`R-154` `N-2` приводил A/B/C и читал B как «флаг выиграл»; прогон D показывает, что
    // из exit-кода это не следует. Вывод вердикта о ложности безоговорочной формулировки
    // при этом верен — он держится на A≡C.)
    // Прод этим не задет и поведение защитимо: `docker-compose.yml:226,231` питает флаг и
    // переменную ОДНИМ `${GATEWAY_DEPTH_CADENCE_MS}`, расхождение недостижимо, исход
    // fail-closed. Оговорка стоит здесь потому, что самоописание, расходящееся с кодом, —
    // наш повторяющийся класс (`TD-167`), а не потому, что найден дефект поведения.
    //
    // При СОВПАДЕНИИ источников отпечаток селектора у писателя и читателя чекпоинта одинаков
