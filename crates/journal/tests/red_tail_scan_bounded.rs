//! M-57 (`TD-109`) — **активный сегмент не смеет пересканироваться на каждом тике**,
...
fn o1_scanned_counts_reads_not_yields() {
    ...
    assert!(
        ratio < 1.5,
        "O-1 (TD-109): events_scanned ведёт себя как АЛИАС events_decoded. Полный проход \
...
fn o2_tick_scans_only_increment_not_whole_segment() {
    ...
    assert!(
        ratio < 2.5,
        "O-2 (TD-109): сегмент вырос в 8 раз — работа тика выросла в {ratio:.1} раза \
