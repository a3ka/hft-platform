...
/// TD-078 (закрыто 2026-08-03): потолок МАСШТАБИРУЕТСЯ под режим сборки.
///
/// Замер reviewer'а (`R-022` F-8): один и тот же блок — **8.18 s в release** и **21.15 s в
/// debug** (×2.6), а весь блок в debug — 296.81 s, самый долгий в workspace. `verify_M-52.sh`
/// гоняет `--release`, но `ci.yml:24` — `cargo test --all`, то есть DEBUG на `ubuntu-latest`
/// (2 vCPU, shared), где раннер обычно медленнее локальной машины ещё в 2-3 раза. При таком
/// сложении фиксированный потолок 60 s пробивался бы НЕ дефектом кода, а окружением — и красил
/// `main`, который `gates.md` §8 обязывает держать зелёным.
///
/// Смысл оракула — «работа ОГРАНИЧЕНА», а не «машина быстра». Поэтому потолок в debug
/// умножается: свойство сохраняется (неограниченный скан не уложится и в ×6), а зависимость
/// от скорости раннера снимается. Множитель осознанно щедрый — оракул-измеритель окружения
/// вреднее пропущенной регрессии производительности, которую поймает release-прогон verify.
const CEILING_SCALE: u64 = if cfg!(debug_assertions) { 6 } else { 1 };
const CEILING_SECS: u64 = 60 * CEILING_SCALE;
/// Потолок операторского прогона дайджеста на подвыборке — с запасом на медленный CI.
const DIGEST_CEILING_SECS: u64 = 120 * CEILING_SCALE;
