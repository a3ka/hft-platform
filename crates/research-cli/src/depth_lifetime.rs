...
/// Уровень с `|distance_bps| >= 3000` кладётся в последнюю полосу `[1500, 3000)` как
/// `>=1500` (для реальных BTC-полос это не должно встречаться; защита от выбросов).
...
/// `|bps| >= 3000` (за пределами схемы) → последняя полоса `[1500, 3000)` как `>=1500`.
