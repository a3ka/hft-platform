...
pub fn export_to_dir(
    ...
    // `buckets.keys()` только собирает ссылки на ключи в Vec; порядок этого сбора нигде не
    // читается — следующая строка (`keys.sort_by(...)` по venue, затем symbol) немедленно и
    // полностью переупорядочивает Vec ДО первого использования (цикл `for key in keys` ниже).
    // DET-OK: хэш-сид процесса, определяющий порядок `.keys()`, не протекает наружу — итоговый порядок записи файлов есть функция только (venue, symbol), не сида (RC-I-5).
