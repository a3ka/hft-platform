...
pub mod auth {
    ...
    /// M-87 (§7.1, TD-207): ЕДИНСТВЕННАЯ точка трактовки секрета. И сервер, и зонд
    /// ОБЯЗАНЫ звать ЭТУ функцию для построения материала ключа. Прежде сервер
    /// строил ключ из СЫРЫХ байт строки (`DecodingKey::from_secret(secret.as_bytes())`),
    /// а зонд строку «только hex-символы чётной длины» ДЕКОДИРОВАЛ как hex — прод-секрет
    /// из 64 hex-символов зонд превращал в 32 байта, а сервер передавал 64 байта в
    /// `DecodingKey::from_secret` (32- vs 64-байтовая асимметрия). Дефект записан
    /// ТРЕТИЙ раз (`TD-084`/`TD-095`/`TD-207`): класс не закрылся регистрацией и
    /// закрывается механизмом — единой трактовкой.
    ///
    /// Правило: строка «ровно 64 hex-символа» (прод-форма) декодируется в 32 байта;
    /// всё прочее трактуется как сырые байты UTF-8 (dev-форма). Здесь — ОБЩАЯ функция,
    /// без новой зависимости: `jsonwebtoken` уже в зависимостях.
    pub fn key_material(secret: &str) -> Vec<u8> {
        let is_hex64 = secret.len() == 64 && secret.chars().all(|c| c.is_ascii_hexdigit());
        if is_hex64 {
...
pub mod server {
    ...
    /// M-65 (CT-RFC-09 §2.6, амендмент `A-015` 2026-08-22; фикс R-118 N-2 2026-08-24):
    /// подписанная норма `max_subscriptions_per_connection` — ЕДИНСТВЕННЫЙ источник
    /// истины для дефолта (founder 2026-08-11). Все ТРИ места, где значение используется
    /// как продуктовый дефолт, читают ОДНУ константу: (а) этот `static` ниже;
    /// (б) default-ветка `serve_config_from_env` (`None` / пустая строка — A-015 §3 п.1,
    /// возвращает `server::DEFAULT_MAX_SUBSCRIPTIONS`); (в) `tracing::warn!` в той же
    /// ветке — оператор читает лог, не исходники, поэтому ЧЕТВЁРТЫЙ литерал в тексте
    /// сообщения (R-118 N-2: «GATEWAY_MAX_SUBSCRIPTIONS=16 — подписанная норма», который
    /// при смене подписи продолжал бы говорить «=16», применяя 32) подставлен через
    /// `{server::DEFAULT_MAX_SUBSCRIPTIONS}`. Четвёртое место — test-оракул
    /// `red_max_subs_config::SIGNED_DEFAULT = 16` — литерал НАМЕРЕННЫЙ: это ВНЕШНИЙ
    /// бенчмарк подписанной нормы, и assert против константы реализации был бы
    /// тавтологией (комментарий в начале файла теста). Смена подписи требует синхронной
    /// правки бенчмарка; иначе `absent_var_yields_signed_default_16` покраснеет, и
    /// это правильно — оракул перестанет быть RED только в смысле «защита инварианта
    /// подписи», а не «подпись совпала с константой кода».
        ...
        match msg {
            wire_v1::ClientMessage::Subscribe {
                ...
                if let Some(policy) = inner.policy.clone() {
                    ...
                    // ветки ниже (а для v1-сессии — до завершения pump-цикла,
                    // отдельный путь; здесь же тест судит первую фазу).
                    // DEBUG: принудительно держим слот.
...
pub fn serve_config_from_env(
    ...
    let max_subs: usize = match trimmed {
        ...
        None | Some("") => {
            // R-118 N-2: литерал `=16` заменён подстановкой `server::DEFAULT_MAX_SUBSCRIPTIONS`,
            // иначе warn_on_default остался бы зелёным при смене подписи на 32
            // (он сверяет лог с `SIGNED_DEFAULT = 16` тем же числом). Константа и значение
            // предупреждения связаны через одно имя — оператор видит то, что процесс применяет.
            tracing::warn!(
                "GATEWAY_MAX_SUBSCRIPTIONS not set (переменная отсутствует или пуста); \
                 GATEWAY_MAX_SUBSCRIPTIONS={} — подписанная норма (founder 2026-08-11, \
