...
///
/// ЛИТЕРАЛ ЗДЕСЬ НАМЕРЕН, и привязывать его к `server::DEFAULT_MAX_SUBSCRIPTIONS` НЕЛЬЗЯ —
/// предложение engine-dev'а (handoff шага 4 `A-015`) отклонено architect'ом по той же причине,
/// по которой `A-015` §5 шаг 3 потребовал литерал в `absent_var_yields_signed_default_16`:
/// оракул, сверяющий код с константой ТОГО ЖЕ кода, тавтологичен. Смена константы на 32
/// оставила бы его зелёным, и подписанная норма уехала бы молча — а именно её этот оракул и
/// стережёт (фикстура `GATEWAY_MAX_SUBSCRIPTIONS` НЕ задаёт и опирается на дефолт процесса).
/// «Один источник истины» — правило для РЕАЛИЗАЦИИ; у оракула источник истины другой и
/// внешний: подпись founder'а.
...
// ═══════════════════════════════════════════════════════════════════════════════════════════
// O-12 — ось 4, значение «кадр в полёте приходит после смены»: ИСПОЛНЕНИЕ, а не заявка
// ═══════════════════════════════════════════════════════════════════════════════════════════
//
// Почему этот оракул существует отдельно от `o1` (R-086 §2, R-093 Б-1).
//
// `o1` ЗАЯВЛЯЕТ покрытие оси 4 (`claims("o1", 4, …)`), но гасит условие паузой
// `_settle_before_switch` в `2 * GRACE_MS + 600` мс. На tempdir-журнале из нескольких событий
// pump занимает микросекунды: к моменту второго `subscribe` он давно завершён, карта полна, и
// берётся ветка SWITCH. Гонка «switch × in-flight pump» при этом НЕ ВОСПРОИЗВОДИТСЯ ни разу.
//
// Замер ревьюера (`R-093` Б-1): нейтрализация развязки А и нейтрализация развязки Б —
// по отдельности — оставляют набор ЗЕЛЁНЫМ (12 passed на каждой). То есть защита в коде есть,
// но её не удерживает ни один оракул.
//
// На проде окно, наоборот, широкое: `CT-RFC-09` §4 фиксирует pump'ы 5096 / 6498 / 10263 мс
// против `PUSH_INTERVAL_MS = 250`. In-flight подписка там — норма, а не краевой случай.
//
// Конструкция: пауза заменена ДЕТЕРМИНИРОВАННОЙ точкой встречи (`test_sync::rendezvous`).
// Тест дожидается, пока pump ФАКТИЧЕСКИ войдёт в критическую секцию, и только тогда шлёт
// смену селектора. «Вероятностное» окно через `sleep` отвергнуто явно — это класс `TD-097`
// (флак, который потом объявляют шумом и выключают).
//
// ПОЧЕМУ ПОД ФИЧЕЙ. `test_sync` компилируется только при `cfg(any(test, feature = "testing"))`
// (задача 12а, engine-dev). Интеграционные тесты `tests/**` — отдельные крейты, линкующиеся с
// библиотекой в обычной конфигурации, поэтому без фичи модуля не существует и этот тест
// ОТСУТСТВУЕТ. Условие висит на функции, а не на файле, чтобы остальные двенадцать оракулов
// M-65 продолжали работать без фичи. Цена: без `--features testing` тест молча не исполняется —
// ровно поэтому задача 12в (шаг `verify_M-65.sh`, наблюдающий ОТСУТСТВИЕ) обязательна.
//
// КРИТЕРИЙ ПРИЁМКИ — МУТАЦИОННЫЙ, не зелёный прогон (`R-093` §5 п.1): снятие развязки А
// (изъятие `Sub` из карты на время pump'а) ЛИБО развязки Б (`generation` внутри `Sub`)
// обязано ронять ЭТОТ тест. Зелёный прогон сам по себе не доказывает ничего — это уже
// показано двумя мутациями выше.
#[cfg(feature = "testing")]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn o12_switch_during_inflight_pump_does_not_restore_old_sub() {
    use gateway_serve::test_sync::rendezvous;

    claims("o12", 4, "кадр в полёте приходит после смены");
    ...
    // Вооружаем точку встречи ДО подписки: pump обязан упереться в неё изнутри.
    rendezvous::arm("w1");
    ...
    assert_eq!(
        type_of(&first),
        Some("snapshot"),
        "SETUP: первым обязан прийти snapshot, пришло: {first}"
    );

    // Кормим журнал, чтобы push-цикл действительно запустил pump.
    append_more(dir.path(), "BTCUSDT", 8, BASE_MS + 3_000);

    // SETUP-GUARD: без фактического входа pump'а в критическую секцию тест проверял бы
    // не тот сценарий — ровно дефект `o1`. Не состоявшийся setup = FAIL сценария, а не
    // тихий пропуск (`testing.md`, целостность гейта, свойство 3).
    let in_flight = rendezvous::test_wait_for_pump("w1", BUDGET);
    assert!(
        in_flight,
        "SETUP НЕ СОСТОЯЛСЯ: pump не вошёл в точку встречи за {BUDGET:?}. Тест обязан менять \
         селектор, пока pump В ПОЛЁТЕ; без этого он вырождается в `o1` с паузой и гонку не \
         воспроизводит. Чинить фикстуру, а не реализацию."
    );

    // ГЛАВНОЕ: смена селектора, пока pump ДЕРЖИТСЯ внутри критической секции.
    send(&mut ws, subscribe("w1", "ETHUSDT")).await;

    // Отпускаем pump — он завершится СТАРЫМ селектором и попытается записать свой результат.
    rendezvous::test_release("w1");
    ...
    assert!(
        stale.is_empty(),
        "O-12 НАРУШЕН: после смены селектора, выданной во время in-flight pump'а, пришло {} \
         сообщений ПРЕЖНЕГО инструмента. Это и есть находка `R-086` §2: завершившийся pump \
         восстановил старую подписку (карта `insert`-replace по ключу), и клиент получает \
