...
//! | ~~`c2`~~ | №5 — счётчик | **УДАЛЁН** (`R-201`): проверял текст. Предмет — в `red_m87_read_volume_truth.rs`, эталон у ядра |
//! | ~~`c3`~~ | №6 — бюджет | **УДАЛЁН** (`R-201`): проверял присутствие имени. Там же |
...
// ═══════════════════════════════════════════════════════════════════════════════════════
// `c2` и `c3` УДАЛЕНЫ, а не «уточнены» (`R-201`, мутации 1 и 2).
//
// Они проверяли ТЕКСТ, а не работу, и вердикт предъявил это прогоном:
//   · `payload_bytes_read: 1` (величина заменена литералом)      → `c2` остался ЗЕЛЁН;
//   · `if true { None } else { feed_tail_within(…) }` (вызов не  → `c3` остался ЗЕЛЁН.
//     исполняется, текст сохранён)
//
// `c2` искал подстроку `snap_text.len()` — то есть ловил ОДНУ конкретную неверную величину и
// пропускал любую другую. `c3` считал строки с текстом `feed_tail_within(` — присутствие
// ИМЕНИ. Коммит, который их вводил, назывался «проверяются ПО РАБОТЕ, не по имени», и сам
// этому заголовку не соответствовал.
//
// Улучшать грепом здесь нечего: сколько бы форм он ни перечислял, следующая неверная величина
// в перечень не попадёт. Предмет переехал в `red_m87_read_volume_truth.rs`, где эталон берётся
// у ЯДРА (`/proc/self/io` `rchar`) — независимо и от нашего счётчика, и от текста исходника.
// Первый же прогон нового оракула дал число, которого прежние два дать не могли: прочитано
// 448 831 Б при каталоге 194 703 Б и хвосте 465 Б.
// ═══════════════════════════════════════════════════════════════════════════════════════
