...
fn policy() -> AdmissionPolicy {
    AdmissionPolicy {
        ...
        // Порог назван ОТНОСИТЕЛЬНО фикстуры, а не «побольше, чтобы не мешало» (`C-247` R1
        // поймал ровно эту ошибку в соседнем файле). `C9` дописывает 200 событий ПОСЛЕ
        // слепка и требует, чтобы выдача это пережила: при пороге 1 000 утверждение
        // «отставание в 200 событий — НОРМА» содержательно. При пороге в миллион оно было
        // бы тавтологией — сценарий зеленел бы, ничего не проверяя.
        // Прочие сценарии файла снимают слепок в конце (отставание ≈0) и порогом не задеты.
        max_tail_events: 1_000,
        expected_warmup_events: 500,
    }
}
