...
///
/// **ЗОВЁТСЯ ТАМ, ГДЕ РАНЬШЕ СТОЯЛ `advance` — ДО ловушек сценария (находка ТЕСТЕРА круга задачи 12).** Прежняя
        ...
        if m87_registry::registry_checkpoint(scenario) == m87_registry::CheckpointFixture::Warm {
            ...
            assert_eq!(
                ...
                "фикстура «{scenario}» создала отставание {lag} при объявленном {declared} \
                 ...
                 пересекать (находка ТЕСТЕРА круга задачи 12)",
                declared + 1
            );
        }
    }
...
async fn u1_entry_stale_checkpoint_beyond_budget_gives_named_outcome() {
    ...
    // между описью и ловушкой, а не в начале тела (находка ТЕСТЕРА круга задачи 12).
