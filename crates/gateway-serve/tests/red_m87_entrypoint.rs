...
impl Drop for LatchGuard {
    fn drop(&mut self) {
        ...
        //
        // `C-244` (NOTE): платформенная привязка выражена ЯВНО, а не подразумевается
        // комментарием. На чужой платформе сборка упадёт с внятным сообщением вместо того,
        // чтобы взять неверное числовое значение и тихо сломать освобождение канала.
        #[cfg(target_os = "linux")]
        const O_NONBLOCK: i32 = 0o4000;
        #[cfg(not(target_os = "linux"))]
        compile_error!(
            "защёлка C4 требует POSIX-семантики O_NONBLOCK для именованного канала; \
             значение задано для Linux (A-038 §5 У-1). Платформа деплоя — Linux."
        );
...
/// **СТОРОЖ ЗАЩЁЛКИ — зависание превращается в ИМЕНОВАННЫЙ провал (находка 2026-09-22).**
///
/// `A-038` закрыл ОТПУСКАНИЕ: открытие на запись ушло в отдельный поток с пределом, а
/// [`LatchGuard`] освобождает канал на любом выходе из теста. Оба механизма опираются на
/// допущение, которое НИГДЕ НЕ БЫЛО НАЗВАНО: блокирующее чтение защёлки происходит **не на
/// потоке рантайма** — то есть внутри `spawn_blocking`. Допущение верно ровно до тех пор,
/// пока реализация не начнёт читать журнал ПРЯМО на асинхронном пути.
///
/// Как только это случается, не работает НИ ОДИН из перечисленных механизмов:
/// поток рантайма встаёт внутри `open(2)` на канале, таймеры не крутятся (значит
/// `tokio::time::timeout` НЕ СРАБАТЫВАЕТ), тест не доходит до `release_latch`, `Drop`
/// стража не вызывается. Тест не падает — он МОЛЧИТ.
///
/// **Замер, из-за которого сторож появился.** 2026-09-22 прогон
/// `c4_slot_lifecycle_hold_refuse_release` простоял **1 ч 45 мин** в `wait_for_partner` на
/// потоке, названном по имени теста, и всё это время блокировал работавшего агента; диск
/// не удавалось рекультивировать, потому что сборщик видел «активную сборку». Зависший
/// оракул дороже ложно-зелёного: ложно-зелёный хотя бы отдаёт управление.
///
/// **Конструкция.** Сторож — ОТДЕЛЬНЫЙ ПОТОК ОС, а не задача рантайма: замороженный рантайм
/// его не касается. По истечении предела он
/// 1. открывает канал `O_RDWR` — единственная форма, которая НИКОГДА не блокируется и при
///    этом даёт заблокированному читателю писателя (`O_WRONLY` здесь не годится: он сам
///    ждёт читателя, а `O_WRONLY|O_NONBLOCK` вернёт `ENXIO`, если читатель ещё не дошёл);
/// 2. снимает запись каталога ДО закрытия — порядок `A-038` D-2(б) сохранён;
/// 3. **взводит флаг** и закрывает дескриптор: читатель получает конец файла, рантайм
///    оживает, тест продолжается.
///
/// Тест ОБЯЗАН спросить флаг ПЕРЕД тем, как толковать собственные наблюдения: после
/// вмешательства сторожа они описывают не сценарий, а последствия спасения. Без этой
/// проверки спасённый тест упал бы с сообщением «работа не удержана защёлкой» — диагнозом,
/// прямо противоположным действительному.
///
/// **Что именно доказывает срабатывание сторожа.** Защёлка блокирует ЧТЕНИЕ ЖУРНАЛА, и по
/// спеке M-87 §4.0bis это чтение живёт в `spawn_blocking`. Замороженный поток рантайма
/// означает ровно одно: чтение выполняется на асинхронном пути. Это и есть дефект, ради
/// которого существует M-87, — один запрос кладёт сервис. Поэтому срабатывание сторожа не
/// «инфраструктурный сбой теста», а НАХОДКА, и сообщение обязано называть её так.
struct LatchWatchdog {
...
impl LatchWatchdog {
    fn arm(path: &std::path::Path, limit: std::time::Duration) -> Self {
        ...
        let handle = std::thread::spawn(move || {
            match rx.recv_timeout(limit) {
                // Тест сам дошёл до отпускания (или завершился) — спасать нечего.
                Ok(()) | Err(std::sync::mpsc::RecvTimeoutError::Disconnected) => {}
                Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                    let opened = std::fs::OpenOptions::new().read(true).write(true).open(&p);
                    let _ = std::fs::remove_file(&p);
    ...
    /// Спросить сторожа ПЕРЕД толкованием любого наблюдения, снятого при живой защёлке.
    fn assert_runtime_alive(&self, step: &str) {
        assert!(
            !self.fired.load(std::sync::atomic::Ordering::SeqCst),
            "НАХОДКА (шаг {step}): поток рантайма ЗАМОРОЖЕН — защёлку пришлось отпускать \
             сторожу. Защёлка блокирует ЧТЕНИЕ ЖУРНАЛА, а по спеке M-87 §4.0bis это чтение \
             обязано жить в `spawn_blocking`. Замороженный рантайм означает, что чтение \
             выполняется ПРЯМО на асинхронном пути: таймеры не крутятся, соседние соединения \
             не обслуживаются, один запрос кладёт сервис. Это ровно тот дефект, ради которого \
             существует M-87, а не сбой инфраструктуры теста"
        );
    }

    /// Взведён ли сторож. Для проб ниже: они судят САМ механизм и обязаны читать флаг, а
    /// не ловить панику `assert_runtime_alive`.
    fn fired(&self) -> bool {
        self.fired.load(std::sync::atomic::Ordering::SeqCst)
    }

    /// Снять сторожа, когда защёлки больше нет: дальше спасать нечего, а ложное
    /// срабатывание на медленной машине дало бы провал с неверным диагнозом.
    fn disarm(&mut self) {
...
impl Drop for LatchWatchdog {
    fn drop(&mut self) {
        self.disarm();
    }
}

// ═════════════ ПРОБА САМОГО СТОРОЖА — обе стороны, без участия реализации ═════════════
//
// Сторож — механизм гейта, и он сам обязан быть под пробой (`gates.md` §9: «барьер сам под
// пробой»). Обе пробы ниже судят ТОЛЬКО поведение канала и потока, не касаясь ни сервера,
// ни журнала: они верны независимо от того, в каком состоянии находится реализация M-87,
// и потому годятся как постоянный контроль.
//
// Доказывать надо ДВЕ стороны, и вторая важнее первой. Сторож, который срабатывает ВСЕГДА,
// так же бесполезен, как тот, что не срабатывает никогда: он превратил бы каждый штатный
// прогон в «находку» и обесценил диагноз.

/// СТОРОНА «СРАБАТЫВАЕТ»: читатель стоит на канале, отпускать его некому — сторож обязан
/// освободить его сам, взвести флаг и снять запись каталога.
#[test]
fn c4_watchdog_fires_when_nobody_releases_the_latch() {
    let dir = tempfile::tempdir().expect("tempdir");
    let latch = dir.path().join("segment-00000000.jrnl");
    let st = std::process::Command::new("mkfifo")
        .arg(&latch)
        .status()
        .expect("запуск mkfifo");
    assert!(
        st.success(),
        "setup-страж: проба без канала судила бы не то"
    );

    // Читатель встаёт на канал — ровно то, что делает заблокированное чтение журнала.
    let rp = latch.clone();
    let (rtx, rrx) = std::sync::mpsc::channel::<()>();
    let reader = std::thread::spawn(move || {
        let f = std::fs::File::open(&rp);
        let _ = rtx.send(());
        drop(f);
    });
    // Setup-страж: читатель НЕ должен дойти сам — иначе проба судит пустоту.
    assert!(
        rrx.recv_timeout(std::time::Duration::from_millis(500))
            .is_err(),
        "setup-страж: читатель открыл канал БЕЗ писателя — семантика FIFO не та, \
         которую проба предполагает, и всё ниже бессмысленно"
    );

    let mut wd = LatchWatchdog::arm(&latch, std::time::Duration::from_secs(2));
    // Ждём срабатывания с запасом, но КОНЕЧНО: проба сторожа не имеет права виснуть сама.
    assert!(
        rrx.recv_timeout(std::time::Duration::from_secs(20)).is_ok(),
        "сторож НЕ освободил читателя за 20 с при пределе 2 с — зависание осталось бы \
         зависанием, а весь смысл сторожа в том, чтобы его не было"
    );
    reader.join().expect("поток читателя");
    assert!(
        wd.fired(),
        "читатель освобождён, но флаг НЕ взведён — тест истолковал бы спасение как \
         штатный ход сценария и выдал бы диагноз, противоположный действительному"
    );
    assert!(
        !latch.exists(),
        "сторож не снял запись каталога — опоздавший читатель снова встал бы на канал"
    );
    wd.disarm();
}

/// СТОРОНА «МОЛЧИТ»: защёлку отпустили штатно и сторожа сняли — флаг обязан остаться
/// невзведённым. Без этой стороны сторож, срабатывающий всегда, выглядел бы исправным.
#[test]
fn c4_watchdog_is_silent_when_latch_is_released_in_time() {
    let dir = tempfile::tempdir().expect("tempdir");
    let latch = dir.path().join("segment-00000000.jrnl");
    let st = std::process::Command::new("mkfifo")
        .arg(&latch)
    ...
    let rp = latch.clone();
    let reader = std::thread::spawn(move || {
        let _ = std::fs::File::open(&rp);
    });

    let mut wd = LatchWatchdog::arm(&latch, std::time::Duration::from_secs(60));
    // Штатный путь сценария: отпускание тестом, затем снятие сторожа.
    release_latch(&latch);
    wd.disarm();
    reader.join().expect("поток читателя");
    assert!(
        !wd.fired(),
        "сторож сработал на ШТАТНОМ пути — каждый нормальный прогон объявлялся бы \
         находкой, и диагноз перестал бы что-либо значить"
    );
}
...
async fn c4_slot_lifecycle_hold_refuse_release() {
    ...
    // СТОРОЖ — сразу за RAII-стражем. Предел 60 с выбран так, чтобы заведомо пережить
    // медленную сборку и холодный старт, но не превратить провал в молчание: замеренный
    // штатный путь сценария укладывается в единицы секунд.
    let mut watchdog = LatchWatchdog::arm(&latch, std::time::Duration::from_secs(60));
    let addr = serve(dir.path(), Some(ckpt.path().to_path_buf())).await;
    ...
    // Спросить сторожа ПЕРВЫМ: если рантайм был заморожен, `held` описывает не удержание,
    // а последствия спасения, и любое толкование ниже дало бы противоположный диагноз.
    watchdog.assert_runtime_alive("1 — удержание");
    assert!(
        held.is_err(),
    ...
    watchdog.assert_runtime_alive("2 — отказ второму");
    ...
    release_latch(&latch);
    // Защёлки больше нет — спасать нечего. Снимаем сторожа явно, чтобы медленная машина на
    // шагах (3)-(4) не дала ложного срабатывания с неверным диагнозом.
    watchdog.disarm();
