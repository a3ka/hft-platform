...
/// **ЦЕНА-СЕНТИНЕЛЬ: одноразовый признак СВЕЖЕГО, пост-switch события** (`A-029` §3).
///
/// Прежняя редакция кормила ОБЕ дописки одним `ETH_PRICE` и объявляла кадр свежим по классу
/// «ниже BTC». Такой предикат отличает эфир от биткоина, но НЕ отличает старое событие эфира
/// от нового: реализация, доставившая до-switch кадр и не прокачавшая пост-switch дописку,
/// его удовлетворяет. `C-190` B-2 нашёл это, `C-192` B-1 нашёл ПОВТОРНО — и повтор стоил
/// арбитража `A-029`, который назвал класс: «признак назначается, но его различающая сила
/// не проверяется против того, что он обязан отличать» (правило `Р-4`).
///
/// Диапазон `[7777.77 … +n*0.01]` не пересекается ни с одним другим диапазоном фикстуры и
/// лежит НИЖЕ `BTC_PRICE`. Непересечение не предполагается, а ПРЕДЪЯВЛЯЕТСЯ исполняемым
/// стражем `assert_sentinel_is_unique`.
#[cfg(feature = "testing")]
const ETH_SENTINEL_PRICE: f64 = 7_777.77;
...
#[cfg(feature = "testing")]
/// Возвращает ФАКТИЧЕСКИ записанный диапазон цен в fixed-point — чтобы страж уникальности
/// сравнивал записанное, а не намерение.
fn append_priced(
    dir: &Path,
    symbol: &str,
    n: usize,
    ...
    (
        to_fixed(base_price),
        to_fixed(base_price + (n.saturating_sub(1)) as f64 * 0.01),
    )
}

/// **СТРАЖ УНИКАЛЬНОСТИ ПРИЗНАКА** (`A-029` §3 п.1, правило `Р-4`а). Утверждает, что
/// диапазон сентинеля не пересекается ни с одним другим диапазоном фикстуры. Фикстура
/// целиком под контролем теста, поэтому проверка полна: сравниваются границы, возвращённые
/// самими вызовами `append_priced`, а не соглашение об именах констант.
///
/// Зачем исполняемый страж, а не комментарий: он краснеет, если будущая правка переиспользует
/// значение или сдвинет базу так, что диапазоны сойдутся. Именно этого не хватило прошлой
/// редакции — там непересечение было ЛОЖНЫМ, и никто не заметил, потому что никто не проверял.
#[cfg(feature = "testing")]
fn assert_sentinel_is_unique(sentinel: (i64, i64), others: &[((i64, i64), &str)]) {
    for (range, what) in others {
        assert!(
            sentinel.1 < range.0 || sentinel.0 > range.1,
            "SETUP НЕ СОСТОЯЛСЯ: диапазон сентинеля [{}..{}] ПЕРЕСЕКАЕТСЯ с дописком «{what}» \
             [{}..{}]. Признак свежести перестал быть одноразовым, и предикат объявит свежим \
             кадр из СТАРЫХ событий — ровно дефект C-192 B-1",
            sentinel.0,
            sentinel.1,
            range.0,
            range.1
    ...
    assert!(
        sentinel.1 < to_fixed(BTC_PRICE),
        "SETUP НЕ СОСТОЯЛСЯ: сентинель [{}..{}] не ниже BTC ({}) — страж «кадр старого \
         инструмента» отвергнет свежий кадр по неверной причине",
        sentinel.0,
        sentinel.1,
        to_fixed(BTC_PRICE)
    );
...
async fn td177_stale_pump_does_not_kill_new_sub() {
    ...
    gateway::set_effective_max_response_bytes(TEST_CAP);
    let dir = journal_of_trades(200);
    let pre_eth = append_priced(dir.path(), "ETHUSDT", 20, T0 + 1_000_000, ETH_PRICE);
    ...
    let fresh_eth = append_priced(dir.path(), "ETHUSDT", 8, T0 + 2_000_000, ETH_SENTINEL_PRICE);

    // Р-4(а): недоступность признака миру без события ПРЕДЪЯВЛЯЕТСЯ, а не предполагается.
    assert_sentinel_is_unique(
        fresh_eth,
        &[
            (pre_eth, "ETH до переключения"),
            (
                (to_fixed(MID), to_fixed(MID + 200.0)),
                "BTC-серия journal_of_trades + append_dense",
            ),
        ],
    );
        ...
        match recv(&mut ws).await {
            ...
            Some((_, v)) if type_of(&v) == Some("frame") && sub_of(&v) == Some(SHORT_SUB) => {
                match max_price_in(&v) {
                    ...
                    // `A-029` §3 п.2: предикат утверждает ТОЖДЕСТВО сентинелю, а не класс.
                    // Диапазон `fresh_eth` не пересекается ни с чем — это утверждает
                    // исполняемый страж выше, а не соглашение об именах. Классовая проверка
                    // «ниже BTC» осталась ВЕТВЬЮ ВЫШЕ как страж против кадра чужого
                    // инструмента, но доказательством свежести больше НЕ является: она
                    // одинаково истинна для до-switch и пост-switch событий эфира, и ровно
                    // на этом `C-190` B-2 и `C-192` B-1 поймали набор ДВАЖДЫ.
                    Some(p) if p >= fresh_eth.0 && p <= fresh_eth.1 => {
                        priced_seen += 1;
                        fresh_eth_frame = true;
                        break;
                    }
                    // Кадр эфира с ЛЮБОЙ иной ценой — до-switch событие: оно не доказывает
                    // ничего и дренаж не прерывает. Прежняя редакция здесь ломала цикл и
                    // объявляла успех — на СТАРОМ кадре.
                    Some(_) => continue,
                    None => continue,
                }
            }
