...
/// Политика для ОДНОГО сценария — `u2_readiness_stale_beyond_budget` (`C-247` R1).
///
/// Щедрый порог `pol()` для него ЯДОВИТ, и это моя ошибка круга 2: я раздал щедрые пороги
/// всему файлу разом, «чтобы посторонняя величина не решала исход», — но у ЭТОГО сценария
/// посторонней величины нет, свежесть и есть его предмет. При `max_tail_events = 1_000_000`
/// и фактическом отставании в 500 событий ВЕРНАЯ реализация обязана ответить `Ready`,
/// значит тест не мог позеленеть НИКОГДА: он требовал от кода противоречить контракту.
///
/// Порог здесь назван ОТНОСИТЕЛЬНО фикстуры (500 дописанных событий), а не абсолютным
/// числом с потолка: связь «лаг больше порога» обязана быть видна в самом тесте.
fn pol_stale() -> AdmissionPolicy {
    AdmissionPolicy {
        max_tail_events: 100,
        expected_warmup_events: 50,
        ..pol()
    }
}
...
fn u2_readiness_stale_beyond_budget() {
    ...
    assert_eq!(
        readiness(ckpt.path(), dir.path(), &s, &pol_stale()),
        ServingOutcome::NotReady,
