...
/// ST-I-5 (NO-LOOKAHEAD): интенты, произведённые на ПРЕФИКСЕ потока, обязаны быть в точности
/// префиксом интентов полного потока. Любая подсмотренная вперёд информация ломает это.
#[test]
fn st_i_5_prefix_stability_proves_no_lookahead() {
