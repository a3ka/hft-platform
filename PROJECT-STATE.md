...
## `П-014` круг 2 — `depth_band_provenance` по стороне и наблюдённому охвату (✅ КОД В MAIN, merge `686327f` 2026-08-23, `R-113` APPROVED; **подпись `П-014` НЕ ИСПОЛНЕНА**)

**Закрыты предусловия (а) и (б)** `П-017` §«Следствие для `П-014`»: метка
`depth_band_provenance` различает СТОРОНУ (bid `liveness=confirmed` / ask `unconfirmed` —
основание M-58, `A-002` З-2 не снят) и НАБЛЮДЁННЫЙ ОХВАТ (полоса за охватом →
`not-observed band=… reach=…`, честность при ресинке).

**`GW-I-4`/`VB-I-2` восстановлен на ДВУХ путях, и это главное содержание круга 2.** Круг 1
был отклонён (`R-110`, блокер Б-1): метка стала функцией времени, а merge-путь
`Snapshot::apply` держал правило «первый непустой побеждает» — метка ЗАЛИПАЛА у WS-клиента,
который получает `Snapshot` однажды и дальше живёт на `Frame`'ах. Круг 2 закрыл это дважды:

1. `481403a` — merge-семантика метки стала `close` (последнее наблюдение побеждает), как у
   значений полосы строкой выше;
2. `6447b2a` — охват СНИМАЕТСЯ в ветке `L2Snapshot`, там же, где считаются сами числа полосы.
   Без этого равенство путей недостижимо структурно: delta-only кадр строк `depth_series` не
   несёт вовсе, и склейка о сдвиге живой книги узнать не может ни при какой семантике слияния.

**Нормативное решение architect'а (принято reviewer'ом):** метка описывает охват НАБЛЮДЕНИЯ,
из которого взяты числа полосы, а не охват живой книги. Ошибка выбранной семантики направлена
в БЕЗОПАСНУЮ сторону: ресинк приходит `L2Snapshot`-ом и двигает числа с охватом вместе
(немедленное `not-observed`), а восстановление дельтами метку не поднимает до следующего снимка.

**Оракулы:** `crates/gateway/tests/red_depth_provenance_by_reach.rs` — 9 тестов
(6 снимок-only + 3 на дельта-хвост). Припиннено мутациями reviewer'а: «вернуть чтение живой
книги в `finish_ref`» → 6 passed / 3 failed; откат merge-правки → 7 passed / 2 failed;
нейтрализация съёма охвата → 5 passed / 4 failed. `CKPT_SCHEMA_VERSION` 1→2 (в состоянии
редьюсера два новых `f64`; postcard не self-describing) ⇒ `ckpt-*.bin` на проде пересобраны
ОДИН раз тихим rebuild'ом (`GW-I-9б`). `GATEWAY_SCHEMA_VERSION` остаётся 8 — форма выдачи не
менялась, изменилось содержимое строки.

**Подпись `П-014` НЕ ИСПОЛНЕНА — открытыми остаются два её пункта:**

> **Состояние на 2026-09-20:** п.2 закрыт `M-68` (каденция объявляется в выдаче), п.4
> ИСПОЛНЕН включением канонического набора на проде — раздел «9-C» в конце файла. Текст ниже
> датирован 2026-08-23 и оставлен как летопись.

- **п.2 (каденция) — ОТКРЫТ** (`TD-158`). Депт-серия по-прежнему пересчитывается только в
  ветке `L2Snapshot` (`crates/gateway/src/lib.rs:986`): 1 Гц против дельт 100 мс. Выдача
  обязана называть это отставание (`PL-I-7`) и не называет. Этой работой закрыт быть не мог:
  перенесена точка съёма охвата, пересчёт чисел не тронут.
- **п.4 (состав выдачи) — НЕ НАЧАТ.** `GATEWAY_BANDS` не тронут ни в репозитории
  (`docker-compose.yml:134,197` — дефолт `0.001`), ни на проде (замер на живом контейнере
  2026-08-23: `GATEWAY_BANDS=0.001`). Полосы глубже 1.3 % НЕ эмитятся, то есть ни одно
  утверждение о живости сегодня до потребителя не доходит. Зона `deploy/**`, отдельная работа.
  **Предусловие включения:** `TD-159` (метка — один скаляр на ряд точек разного качества)
  обязан быть закрыт ДО смены `GATEWAY_BANDS`, иначе дыра станет достижимой в тот же момент.
...
## M-69 «`GW-I-14`: fail-closed разбор `GATEWAY_WINDOW_MS`» — ✅ КОД В MAIN (merge `56a0dfc` 2026-08-24, `R-128` APPROVE) + **В ПРОДЕ, §8 PASSED 2026-08-24T15:58Z**; milestone **НЕ ЗАКРЫТ — close-out за architect'ом**

**Что приземлилось.** Закрыто **единственное оставшееся отступление read-path от
fail-closed** — риск **R7**, классифицированный **CRIT** в
`docs/08-arch-improvement-roadmap.md:35`. До M-69 невалидное значение операторской ручки
`GATEWAY_WINDOW_MS` МОЛЧА давало `None`, а `None` по **`VB-I-10`** означает offline-режим =
полная свёртка журнала = **unbounded** — тот самый режим, который уже разваливал прод
(`gateway-serve` дорастал до RSS 7.3 GB и убивался host-OOM, `TD-039`).

**Гвард стоит в ДВУХ точках, и вторая — не перестраховка:**

| точка | файл | что делает |
|---|---|---|
| старт прод-бинаря | `crates/gateway-serve/src/lib.rs` (`serve_config_from_env`) | parse-error, переполнение `i64`, отрицательное значение → `Err` на старте, сообщение НАЗЫВАЕТ переменную |
| библиотека | `crates/gateway/src/lib.rs` (`validate_selector`) | `Some(w<0)` → `Err`; закрывает байпас для чекпоинтера M-38b, будущего shared-tailer M-39 и research-cli, собирающих `Selector` НАПРЯМУЮ |

Гвард только в транспорте оставил бы ровно ту байпас-поверхность, которой посвящены
`TD-019`/`TD-020` («механизм есть, никто не зовёт»).

**Канонизация offline — три формы, одно представление.** `unset`, пустая строка/пробелы и
`"0"` дают РОВНО `window_ms == None`. Недостаточно совпадения наблюдаемого поведения:
`Some(0)` даёт тот же `None` на выходе `window_lo_time_s`, но ДРУГОЙ
`selector_fingerprint` (M-38b) ⇒ два ключа чекпоинта на один режим. Это найдено критиком
(`C-099` B-2) и исполнено.

**Что осознанно НЕ сделано:** `"0"` не отвергается — паритет с уже принятым argv-путём
`crates/gateway/src/bin/gateway-checkpoint.rs:162` («`0` ⇒ `None` (offline unbounded)»).
Развилка названа спекой и охраняется оракулом в ОБЕ стороны (см. мутацию M3 в `R-128` §11).

**Прод не задет:** `docker-compose.yml:139` подаёт `${GATEWAY_WINDOW_MS:-60000}` — валидное
положительное значение, запиннено отдельным кейсом `prod_window_value_still_starts`.
Дефект был ЛАТЕНТНЫМ, но достижимым с прода: форма `:-` закрывает отсутствие и пустоту, но
НЕ мусор — опечатка вида `60s`/`60_000`/`60000ms` в host-`.env` проезжала насквозь.

**Гейты.** Три круга критика REJECT (`C-099`, `C-100`, `C-104`) → арбитраж `A-014`
(`gates.md` §0 п.2: два REJECT по одной причине) → ограниченный круг 4 на сильной модели
`C-132` **NOTE** (он же засчитан за перепроверку `gates.md` §9 — диф трогает `DESIGN.md`) →
engine-dev задачи #1–#4 → флип фактуры architect'ом → tester PASS → reviewer `R-128`
**APPROVE**. RISK-BLOCK неприменим: read-path без order-egress.

**Пруфы (сырые — в `research/reviews/R-128-M-69-window-guard.md` §12).**
`verify_M-69.sh` на ветке: `VERDICT: PASS`, 13 PASS / 0 FAIL, exit=0 — все ЧЕТЫРЕ
объявленных `A-014` B-8 plan-time красных стёрты, красного сверх списка нет. На ДЕРЕВЕ
СЛИЯНИЯ (обязательно при `strict:false`): fmt=0, clippy=0, `cargo test --all` 877/0
(208 блоков); `gate-meta`, `artifact-ids`, `docs-freeze`, `protected-artifacts`,
`design-claims` — все exit=0. PR #32: `All checks passed`.

**Мутационный контроль — оба вопроса `testing.md`.** M1 (гвард старта снят) и M2 (гвард
`validate_selector` снят) роняют свои оракулы, exit=101. M3 — обратная мутация: искусственное
ужесточение `"0"` → `Err` роняет `offline_forms_still_start`. То есть оракул пиннит
ГРАНИЦУ, а не одну её сторону, и осознанное послабление не может быть тихо снято.

**Что НЕ закрыто и осталось за architect'ом** (`R-128` §9, три NOTE): `GW-I-13` остался
дырой в семействе — спека сочла его занятым, но замер даёт лишь прозаическое «следующий
свободный» в `docs/plans/gateway-ws-contract.md:556`; шапка `milestones/M-69-window-guard.md`
всё ещё `📝 PROPOSED` при `✅ DONE` в §Tasks; `docs/SESSION-HANDOFF.md` описывал M-69 как
«круг 2 у критика». Все три — документальные, вне кода.

**§8 деплой-гейт — PASSED, и гвард предъявлен ИСПОЛНЕНИЕМ на прод-образе.**

```
CI main   32743602862  success   (exit=0)
Deploy    32743602658  success   (exit=0; «Gate on CI (fail-closed)» → success, «Catch-up decision» → skipped)
ssh … git rev-parse HEAD → 56a0dfc9c9dfb27773bdab7c82225576346b9ba7   ← точный SHA, не дата образа
docker ps → hft-gateway-serve Up (healthy) · hft-recorder Up (healthy)
RestartCount=0 ExitCode=0    (gateway-serve поднялся с новым гвардом, не упал на нём)
heartbeat ts_wall_ms=1787587102311 при now=1787587107 → свежесть 5 s
next_seq 344911401 → 345575559 — журнал растёт
RssAnon (НЕ docker stats, TD-021): gateway-serve 368 kB · recorder 11 560 kB
env прода: GATEWAY_WINDOW_MS=60000 — валиден, гвард его пропускает
```

**Liveness НЕДОСТАТОЧЕН — контейнер жив и со старым бинарём.** Поэтому механизм предъявлен
исполнением на ТОМ ЖЕ прод-образе (`hft-platform-recorder:local`), парой в обе стороны:

| проба на прод-образе | результат |
|---|---|
| `GATEWAY_WINDOW_MS=60000ms` (мусор с суффиксом) | **exit=2**, `config error: GATEWAY_WINDOW_MS="60000ms" не парсится как i64 … а не сигнал к unbounded-режиму` |
| `GATEWAY_WINDOW_MS=-60000` (отрицательное) | **exit=2**, `… отрицателен … selector_fingerprint расходится с offline ⇒ чекпоинт снимается под незаказанным режимом` |
| `GATEWAY_WINDOW_MS=0` (легитимный offline) | **стартует**, `Running=true`, `Restarts=0` — гвард его НЕ отвергает |
| `GATEWAY_WINDOW_MS=60000` (прод) | работающий контейнер, `healthy`, `RestartCount=0` |

Это и есть разница между «деплой прошёл» и «механизм на пути»: первые две строки
доказывают, что `GW-I-14` доехал до прод-бинаря, вторые две — что он не сломал ни один
режим, который работал до него.

**Собственной FA у `gateway`/`gateway-serve` по-прежнему НЕТ** — названный долг
(`docs/workflow/reading-map.md` §2). Семейство `GW-I` живёт только в оракулах: `DESIGN.md`
§22 после M-69 — «заявлено=0, в оракулах=13». Обратный дрейф не создан этим милестоуном, но
и не уменьшен им.
...
вместо чтения-и-выбрасывания клиентских сообщений; состояние подписок — НА СОЕДИНЕНИЕ, у
каждого своё пространство `sub id`; смена инструмента даёт новый `snapshot` и НИ ОДНОГО
`frame` прежнего селектора после него; коды ошибок машиночитаемы, соседние подписки живы.
Диапазон `a9f0bf5..87d9c87`: 29 файлов, +8809/−24, из `crates/**` затронут ТОЛЬКО
`gateway-serve`. Оракулы — `tests/red_ws_session.rs` (`o0`..`o12`), `red_max_subs_config.rs`,
`red_grace_from_env.rs`, `red_wire_null_sub.rs`; гейт — `scripts/verify_M-65.sh` (16 шагов,
включая батарею мутантов 13/13 и шаг `O12`, наблюдающий ОТСУТСТВИЕ оракула).

**Главное содержание последнего круга — исполнение арбитража `A-015`, а не новая функция.**
Спор шёл о поведении прод-бинаря при ОТСУТСТВИИ `GATEWAY_MAX_SUBSCRIPTIONS`. Позиция Б
(«отсутствие ⇒ отказ старта», `d5a1cf5`) была реализована и слита в ветку; арбитр
(`research/arbitration/A-015-m65-n2-absent-config.md`) развернул её в пользу позиции А:

- **отсутствие И пустая строка ⇒ подписанная норма `16` + `warn`** (переменная, значение,
  «продуктовая норма»);
- **невалидное ЗНАЧЕНИЕ** (`0`, отрицательное, нечисловое) ⇒ **отказ старта** — без изменений.

Основание — не авторитет, а замер: обоснование нормы («parse-error → unbounded запрещено»,
урок R7) к случаю ОТСУТСТВИЯ не относится, потому что отсутствие даёт величину ОГРАНИЧЕННУЮ
и равную подписанной (`A-015` §2.2). Маршрут §5 исполнен целиком: узкий contract-RFC на
`CT-RFC-09` §2.6 (`f4f0a49`) → критик на СИЛЬНОЙ модели, свежий контекст (`C-125` REJECT →
**`C-126` PASS**) → sacred-оракул (`135bc8d`) → impl (`d784e15`) → §Tasks к факту (`87d9c87`)
→ tester PASS → reviewer `R-118` APPROVED.

**`d5a1cf5` остаётся в истории намеренно.** `d784e15` — откат СУТИ новым коммитом, а не
revert-механикой: переписанная история читается как «так и было всегда» — ровно тот класс
расхождения носителей, ради которого арбитраж и созывался. Причина живёт в трёх местах и
нигде не пересказывается: §Tasks 13 milestone'а («`d5a1cf5` вытеснен и доказательством N-2
больше не является»), тело `d784e15`, тело merge-коммита `63c3866`.

**Что этот milestone впервые ЗАКРЫЛ гейтом, а не рассуждением.** До `135bc8d` шаг `L`
`verify_M-65.sh` гонял ТОЛЬКО невалидные значения (`A-015` F14) — поведение, о котором шёл
весь спор, не было покрыто гейтом НИ В ОДНУ СТОРОНУ. Теперь шаг пересекает границу процесса:
запускает прод-бинарь той же формой argv/env, какой его зовёт `docker-compose.yml`, и решает
по КОДУ ВОЗВРАТА. Шесть проверок: `absent`/`empty` ⇒ старт состоялся И уход на норму НАЗВАН
в логе; `0`/`-1`/`abc` ⇒ отказ старта `exit=2`.

**Мутационное предъявление (reviewer, независимо от dev'а).** Снятие `tracing::warn!` ⇒
падает только `warn_on_default` (4 passed / 1 failed); ветка дефолта ⇒ `Err` ⇒ падают
`absent_var_yields_signed_default_16` + `empty_var_is_same_as_absent` + `warn_on_default`
(2 passed / 3 failed) — ровно та раскладка, которую engine-dev заявил как исходный RED.

**Нормативное решение architect'а (оспорено reviewer'ом и ПРИНЯТО по замеру).** `MAX_SUBS` в
`red_ws_session.rs` оставлен ЛИТЕРАЛОМ `16` и НЕ привязан к введённой
`server::DEFAULT_MAX_SUBSCRIPTIONS`. Замер решает спор: при подписи, уехавшей на 32, оракул с
литералом краснеет (`o4` FAILED), а с привязкой к константе — зелен (`o4` ok), то есть
подписанная норма ушла бы МОЛЧА. «Один источник истины» — правило для РЕАЛИЗАЦИИ; у
sacred-оракула источник истины другой и внешний: подпись founder'а (`CT-RFC-09` §6 п.2).

**Формы контракта не менялось:** `crates/contracts/**` в дифе отсутствует, `SCHEMA_VERSION` и
`GATEWAY_SCHEMA_VERSION` не тронуты; правка `CT-RFC-09` §2.6 — норматив ПОВЕДЕНИЯ при
...
статус спеки, дальнейшие задачи) — зона architect'а. Открытыми остаются четыре NOTE
вердикта: `N-1` (`EXPECT_TESTS=11` отстаёт от набора `o0..o11`=12 — потеря одного оракула
шаг `F` не покраснит), `N-2` (литерал `16` в тексте `warn!` вместо константы — лог может
врать, `warn_on_default` этого не поймает), `N-3` (`FA-WAIVER` в `d784e15` несёт ложную
причину и несуществующий `GS-I-3`), `N-4` (шаг `L` «absent» без `env -u` — setup-guard'а нет).

---

### M-65 · круг гейта 5 — PR #89: остаток `R-118` закрыт, набор находок СОШЁЛСЯ ЦЕЛИКОМ

**Статус: ✅ КОД В MAIN** (merge `bba42cd` 2026-08-24, `R-129` APPROVED) **+ В ПРОДЕ,
`gates.md` §8 PASSED 2026-08-24T22:21Z.** Milestone по-прежнему НЕ ЗАКРЫТ — close-out за
architect'ом.

**Предмет.** Один коммит `ff1092f`, ровно `crates/gateway-serve/src/lib.rs`, 23/11:
литерал `GATEWAY_MAX_SUBSCRIPTIONS=16` в тексте `tracing::warn!` заменён подстановкой
`{}` с аргументом `server::DEFAULT_MAX_SUBSCRIPTIONS`; doc-комментарий константы приведён
к факту (три потребителя названы поимённо, `red_max_subs_config::SIGNED_DEFAULT = 16`
объявлен намеренным ВНЕШНИМ бенчмарком). История `d784e15` НЕ переписана намеренно —
на неё смотрят `audited_head` цепочки `R-093`/`R-111`/`R-118`.

**Четыре NOTE `R-118` закрыты — двумя разными руками:**

| находка | чем закрыта |
|---|---|
| `N-1` `EXPECT_TESTS=11` при наборе из 12 | ✅ `main` (architect): `EXPECT_TESTS=12`, число снято замером |
| `N-2` литерал `16` в тексте `warn!` | ✅ **PR #89** (engine-dev), этот круг |
| `N-3` ложный `FA-WAIVER` + несуществующий `GS-I-3` | ✅ **PR #89** — новое тело называет `GS-I-4`, и он существует |
| `N-4` шаг `L` «absent» без `env -u` | ✅ `main` (architect): `env -u GATEWAY_MAX_SUBSCRIPTIONS` |

**Мутационный контроль перепрогнан reviewer'ом независимо** (не принят по телу коммита):
M1 (константа→32, подстановка сохранена) → `warn_on_default` **FAILED**, 2 passed/3 failed;
M2 (константа→32, литерал `=16` возвращён — воспроизведение бага) → `warn_on_default` **ok**,
3 passed/2 failed. То есть подстановка ДЕЙСТВИТЕЛЬНО привязала оракул к смене подписи, а не
только «выглядит правильнее». Восстановление: 58 passed / 0 failed.

**Дерево слияния проверено целиком, а не ветка.** Ветка отстала: `base.sha` PR'а — `25470c5`,
вершина `main` на момент гейта — `c00b726`, и дельта `main`'а трогала ТОТ ЖЕ файл (M-69
window-guard) И сам `scripts/verify_M-65.sh`. На реальном дереве слияния: `cargo test --all`
**passed=877 failed=0**, `verify_M-65.sh` **VERDICT: PASS** (16 шагов, батарея 13/13),
`verify_design_claims --merge-preview` **PASS (0 нарушений)**, fmt/clippy чисты.

**Обе прод-формы барьеров сверены — урок `R-118` §10 применён, а не пересказан.**
Форма `pull_request` (CI) и форма `push` (`PUSH_BEFORE` = вершина `main`) дали ОДИН ответ:
`protected-artifacts` / `docs-freeze` / `artifact-ids` / `gate-meta` / `review-fa` — все
exit=0. Расхождения, стоившего прошлому кругу красного `main` и несостоявшегося деплоя,
здесь НЕТ, и это проверено ДО merge'а, а не после.

**`gates.md` §8 — пруф.**

```
$ gh run list --branch main --limit 3
32784032766  Deploy to VPS  completed  success    bba42cd5
32783192893  CI             completed  success    bba42cd5
```

ssh 2026-08-24T22:21:01Z и 22:21:12Z (ДВА замера — рост, а не снимок):

```
hft-gateway-serve Up 30 seconds (healthy)   hft-recorder Up 35 seconds (healthy)
heartbeat 22:21:01Z: {"events":4208,"next_seq":349255354,"segment_index":405,"writable":true}
heartbeat 22:21:12Z: {"events":6126,"next_seq":349257273,"segment_index":405,"writable":true}
журнал: 74 241 670 698 B → 74 242 147 470 B (+476 772 B за 11 с)
RssAnon: recorder 11 512 kB · gateway-serve 372 kB      (НЕ docker stats — TD-021)
load average: 1.59, 1.00, 0.49
```

`Up 30 seconds` — прямое доказательство, что образ ПЕРЕЗАПУЩЕН, т.е. деплой состоялся
(в отличие от прошлого круга, где прод был здоров, но работы не получил).

**Доставка проверена в САМ БИНАРЬ, а не по «Deploy success».** Лог `warn!` на проде не
сработает никогда — compose всегда подаёт переменную (`A-015` F8), поэтому наблюдать фикс
в логе НЕЛЬЗЯ, и это названо, а не обойдено молчанием. Проверено содержимое бинаря из
работающего контейнера:

```
$ grep -aoc "GATEWAY_MAX_SUBSCRIPTIONS" /tmp/gs-prod      → 4     ← позитивный контроль
$ grep -aoc "GATEWAY_JWT_SECRET"        /tmp/gs-prod      → 1     ← позитивный контроль
$ grep -aoc "GATEWAY_MAX_SUBSCRIPTIONS=16" /tmp/gs-prod   → 0     ← баговой слитной формы НЕТ
$ grep -ao  "отсутствует или пуста); GATEWAY_MAX_SUBSCRIPTIONS=" /tmp/gs-prod
отсутствует или пуста); GATEWAY_MAX_SUBSCRIPTIONS=                ← сообщение обрывается на «=»
$ grep -ao  " — подписанная норма (founder 2026-08-11" /tmp/gs-prod
 — подписанная норма (founder 2026-08-11                          ← хвост — ОТДЕЛЬНЫЙ фрагмент
```

Разрыв формата на две части — сигнатура `{}`-подстановки. Позитивный контроль обязателен:
первая попытка мерила `strings`, которого нет ни в контейнере, ни на хосте, и её «0» был
ВАКУУМНЫМ (`testing.md` §«Целостность гейта», свойство 3).

**Две новые находки — обе NOTE, обе зона architect, merge не блокируют:** `TD-166`
(`warn_on_default` сверяет значение подстрокой по строке лога с wall-clock таймстампом —
красный исход вероятностный) и `TD-165` (норма о месте `FA-WAIVER` расходится со своим
барьером). Разбор и замеры — `research/reviews/R-129-M-65-r118-n2-n3-fix.md` §4-§5.

## PR #66 «два барьера `П-017` A2, шаг 1» — ✅ В MAIN (merge `4b974a4` 2026-08-24, `R-123` APPROVED). **Итог: барьер A НЕ СТРОИТСЯ; класс «вакуумный ассерт» остаётся `COGNITIVE-ONLY`**

**Что это было.** Спека `docs/plans/two-barriers-spec-2026-08-23.md` предписывала построить два
барьера и требовала: «Первый шаг постройки — НЕ код, а разбор шести». Разбор выполнен, исход
**ОТРИЦАТЕЛЬНЫЙ**: узкая форма барьера A не строится. **Это РЕЗУЛЬТАТ шага, а не его пропуск** —
и он предъявлен замером, а не рассуждением.

**Состав приземлённого (3 файла, `+342 −0`):**
- `docs/plans/two-barriers-step1-2026-08-23.md` — разбор и решение;
- `crates/gateway/tests/red_segment_meta_bound.rs` **+12** — свидетель непустоты каталога;
- `research/arbitration/A-016-pr66-routing.md` — решение арбитра о маршруте.

**Почему барьер A отвергнут (числа — из вердикта, перепроверены reviewer'ом поимённо).**
Детектор красит 9 функций «фильтр + негативный ассерт», из них 6 — «без свидетеля». При ОТКРЫТИИ
файлов **пять из шести** оказались со свидетелем, невидимым шаблону детектора: `assert!(скаляр > 0)`,
`assert_eq!(поле, N>0)`, `assert_eq!(код возврата, 0)`, setup-guard прозой, встречный сторож
`claims()`. **83 % ложных красных.** Шаблон, накрывающий все формы, перестал бы отличать свидетеля
от обычного ассерта. Это ТРЕТЬЕ сужение одного прокси ⇒ прокси убирается, а не сужается.
Reviewer проверил все пять свидетелей грепом — **все пять существуют по названным путям**
(`R-123` §3.1).

**Единственный ПОДТВЕРЖДЁННЫЙ вакуум закрыт здесь же.** `assert_catalog_truthful`
(`red_segment_meta_bound.rs`): три негативных ассерта над результатом ФИЛЬТРА, все три истинны на
ПУСТОМ каталоге — реализация, отдающая пустой кеш, проходила «сверку правдивости» на всех шести
тактах. Свидетель снимается с **НЕЗАВИСИМОГО** обхода (`truth_indices` → `journal::list_segments`
с нуля), а не с проверяемого кеша: иначе пустой кеш подтверждал бы сам себя.
Защищаемые инварианты — **`VB-I-2`** (`live == replay`: лгущий каталог расходит live-хвост с
replay) и **`VB-I-10`** (бюджет тика не зависит от числа сегментов, M-62/`TD-120`).

**Мутационный контроль — УЗКАЯ мутация, воспроизведена reviewer'ом независимо:**
```
обнулён ТОЛЬКО вход свидетеля (`let observed`) →
test result: FAILED. 8 passed; 2 failed
  sm8_per_segment_compaction_keeps_catalog_truthful … panicked at :772
  sm9_compaction_midstate_does_not_duplicate_index  … panicked at :772
```
Широкая мутация («независимый обход выродился в пустой» целиком) даёт `7 passed; 3 failed` и ДО
правки тоже, но красит ДРУГИЕ ассерты — доказательством правки она не является. Различие узкой и
широкой мутации зафиксировано как норма приёмки.

**Честный остаток, зафиксированный решением.** Класс «вакуумный ассерт» (`R-086` §2, `R-106` Б-1)
остаётся **БЕЗ механического барьера** и помечен `COGNITIVE-ONLY` — изображать иное запрещено.
Против него работает ПРОЦЕДУРА, а не текст: изолирующая мутация как шаг приёмки («предъяви, ЧТО
именно краснеет»). За одну сессию она нашла пять вакуумов, ни одного из которых барьер A не
увидел бы. **Барьер B** (замер держится: 6 из 340 = 1.8 %; меряет САМ признак, а не прокси)
строится, но не этим кругом; порядок — `#65` → `#60` → барьер B.

**Маршрут задан арбитражем.** `A-016` постановил: PR не дробится, идёт к reviewer'у ЦЕЛИКОМ
(решение против позиции architect'а, обязательно к исполнению). Дробление потребовало бы
пересборки истории под чужим стеком веток и открыло бы окно, в котором `main` несёт документ,
утверждающий о состоянии оракула то, чего в `main` ещё нет.

**Гейт `R-123` — APPROVED с тремя находками уровня NOTE** (дефекты чисел/ссылки в тексте, не в
выводе и не в коде; основание непропорциональности блокировки — `R-123` §6bis):
- **F-1** заголовок §2 «ЧЕТЫРЕ» против собственной таблицы и `:35` «5 из 6 = 83 %»;
- **F-2** «67 %» в §3 — то же протухшее число (4/6) вторым экземпляром;
- **F-3** мутация падает на `:772` (`assert!`), а не на `:771` (`let observed`) — третий подряд
  номер строки в этой цепочке, снятый не с той редакции дерева (`R-114` N-1 поймал тот же класс
  в §2 ТОГО ЖЕ документа).
Условие APPROVED: architect приземляет doc-only правку F-1/F-2/F-3 своим PR (`docs/plans/**` вне
триггеров `gates.md` §9), нового круга гейта не требуется.

**`O-1` — остаточный класс `TD-163` воспроизведён reviewer'ом СЛУЧАЙНО, заведён как `TD-164`.**
Merge-превью, собранное командой `git merge origin/main` **изнутри ветки** (`parent1=ветка`), дало
`check_protected_artifacts.sh` **exit=1, FAIL=24** в обеих прод-формах; форма GitHub
(`refs/pull/66/merge`, `parent1=main`) — **exit=0, FAIL=0, NOTE=24**. **Деревья побитово идентичны**
(`cbfc2cc`). Подтверждает `A-017` Р-2 третьей рукой и без умысла: намеренным должен быть ДРОП, но
невыгодная топология возникает сама собой. Побочно: `gates.md` §8 велит проверять «на дереве
слияния» и не уточняет порядок родителей — кандидат на правку проводки (зона architect'а).

**Гейты на дереве слияния (`R-123` §8):** `cargo fmt` exit=0 · `cargo clippy --all-targets
--all-features -D warnings` exit=0 · `cargo test --all` **855 passed / 0 failed** (206 блоков) ·
`verify_design_claims.sh --merge-preview origin/main` **VERDICT: PASS** · барьеры на
`refs/pull/66/merge` в ОБЕИХ прод-формах exit=0, FAIL=0, NOTE=24 · `gh pr checks 66` **exit=0**
(`All checks passed`).

**RISK-BLOCK НЕ ПРИМЕНИМ** — `crates/{risk,killswitch,oms,venue-*}/**` и `crates/contracts/**` в
дифе отсутствуют; `crates/*/src/**` не тронут ни одной строкой (диф — только `tests/`).

## M-71 «предел объёма ответа выдачи» (`PL-I-5`, величина подписана `П-020`) — ✅ КОД В MAIN (merge `eb1c20a` 2026-08-28, `R-146` APPROVED, круг 5); **milestone НЕ ЗАКРЫТ — close-out за architect'ом; четыре находки уехали объявленным долгом**

**Что изменилось по существу.** Ответ выдачи, не влезающий в подписанный предел
(`GATEWAY_MAX_RESPONSE_BYTES = 2 000 000` Б, `П-020`), теперь ОТВЕРГАЕТСЯ явно — одинаково на
библиотечном (`gateway::snapshot`/`frames_since`) и на живом (`LiveReducer::resume → pump →
snapshot_checked`) путях. Молчаливого усечения (`volume_bubbles.clear()` + захват чужого флага
`history_truncated`) больше нет: `VB-I-2` («live == replay») и `VB-I-11` (провенанс истории)
перестали расходиться между путями. Разобранное значение ручки доходит до крейта, который его
ПРИМЕНЯЕТ (`crates/gateway`), и читается точками применения — мёртвая пара сеттера/геттера в
`gateway-serve` снесена тем же кругом. Невалидное значение → отказ СТАРТА; отсутствие и
пустое/пробельное → подписанная норма (единая политика по `A-015`).

**Отказ по пределу ТЕРМИНАЛЕН для подписки, а не бесконечная повторная попытка.** Сверхлимитный
кадр не станет меньше, пока предел стоит: подписка завершается явно, клиент извещается и
пересобирается снапшотом (штатная ступень `DESIGN` §16). Признак ставится в точке отказа
гварда и ЧИТАЕТСЯ всеми тремя прод-вызывателями (`crates/gateway-serve/src/lib.rs:1367`,
`:1781`, `:1871`) — не built-not-wired.

**Цена тика не выросла — и это ПРЕДЪЯВЛЕНО МЕРОЙ, а не обещанием.** Круг 4 ввёл транзакционный
commit, перечитывавший батч через `journal::stream_from` на КАЖДОМ тике: замер reviewer'а
`rchar` из `/proc/self/io` дал 1 703 434 Б на тик из ТРЁХ событий при сегменте 32 000 против
65 880 Б у родителя (×25.9, рост линеен по длине сегмента) — возврат P0-дефекта M-57/M-62.
Круг 5 заменил это двумя позициями (`cursor` доставки + `full_applied_seq` состояния), и
свойство закрыто оракулом `F-036` (`crates/gateway/tests/red_tick_read_cost.rs`), который судит
ОТНОШЕНИЕ двух замеров на границе ПРОЦЕССА, а не счётчик участника (`TD-148`: `ReadStats`
первичного стрима ко второму пути слеп по построению).

**Гейты (прогон reviewer'а на чистом worktree, не пересказ):** `verify_M-71.sh` → **47 PASS /
0 FAIL, `VERDICT: PASS`, exit=0**; `cargo test --workspace` → **919 passed / 0 failed** (216
блоков); `fmt`/`clippy --all-targets --all-features -D warnings` → exit=0;
`verify_design_claims.sh --merge-preview origin/main` → **PASS** (ветка отставала на 51
коммит, `strict: false`).

**Стоимость гейта названа честно:** пять кругов PR-ревью (`R-133`, `R-139`, `R-140`, `R-143`,
`R-146`), восемь кругов критика, три арбитража (`A-021`, `A-022`, `A-026`) и решение founder'а
`П-022` («готовое вливать, спорное — долгом»). Четыре круга подряд фикс покупался новым
дефектом; пятый закрыл оба блокера предъявлением исполнением.

**Что НЕ закрыто и уехало долгом (видно в списке, а не умолчано — условие `П-022`):**
`TD-177` (снятие подписки по терминальному отказу не сверяет generation ⇒ отставший `pump`
сносит только что переключённую подписку), `TD-178` (у терминальности нет оракула точки входа
на push-пути — харнесс существует, оракул не написан), `TD-179` (побатчевый `cursor` теряет
кадры при НЕ-cap отказе середины `pump`), `TD-180` (`snapshot()` в окне «состояние впереди
закладки» объявляет отстающий курсор). Предусловие первых трёх на сегодняшнем проде не
наступает (ответ ≈62 КБ против предела 2 МБ, полосы `M-70` выключены границей C) и станет
достижимым вместе с `M-70` — до того момента карточки обязаны быть закрыты. Закрыт этим
кругом: `TD-174`.

## M-68 «депт-серия пересчитывается от книги по собственной каденции» (`MD-I-8`, `TD-158`) — ✅ КОД В MAIN (merge PR #124, `R-156` APPROVED, круг 7); **milestone НЕ ЗАКРЫТ — открыт `TD-185`**

**Что реализовано.** Депт-серия перестала быть snapshot-only: полосы пересчитываются от книги
на КАЖДОМ L2-событии, включая ветку `L2Delta`, из тех же материализованных уровней, что уже
берёт heatmap (ноль дополнительных аллокаций на такте). Каденция серии стала СВОИМ интервалом,
ведомым временем события, и вынесена в конфиг: `GATEWAY_DEPTH_CADENCE_MS` объявлена в
`docker-compose.yml` у ОБЕИХ служб — писателя чекпоинта и `gateway-serve`, — иначе отпечаток
селектора у них расходится и тёплый старт вырождается в полный реплей. Непредставимая каденция
отвергается на старте с сообщением, называющим переменную (класс `GW-I-14`), а не схлопывается
в дефолт. Смена семантики закрыта бампом `GATEWAY_SCHEMA_VERSION` 8→9 — старый чекпоинт
отвергается явной инвалидацией, а не подгонкой отпечатка.

**Цена пересчёта замерена, а не оценена:** книга 10 уровней → 548 посещений, книга 400 →
20 048 (×36.6 при пороге ×5); одна полоса → 20 048, семь полос → 20 048 (×1.000 при бюджете
×1.5). То есть семь полос стоят ровно одного обхода. Оба оракула проверены мутацией и каждый
ловит СВОЁ: «счётчик = константа» роняет первый и не трогает второй, «отдельный проход на
полосу» — наоборот.

**Гейт круга 7 (reviewer, дерево слияния ≡ вершина ветки, merge fast-forward):**
базовая тройка CI — exit 0; `scripts/verify_M-68.sh` — `VERDICT: PASS`; `check_gate_meta.sh`
в прод-форме — `VERDICT: PASS`, 20 вердиктов, 0 до-нормативных приземлений;
`verify_design_claims.sh --merge-preview origin/main` — `PASS (0 нарушений)`;
`gh pr checks 124 --watch` — 17/17 `pass`, агрегат `All checks passed` — `pass`.

**Долг, уехавший ОБЪЯВЛЕННЫМ (`gates.md` §4 это допускает, молчание — нет):**
`TD-185` (окно ложности FA до приземления `docs/fa-viz-M-68-close`), `TD-186` (пробел
`scope-guard` про комментарии в чужой зоне), `TD-187` (`continue-on-error` глушит агрегат CI —
текст продиктован `A-027` §6), `TD-188` (порядок «гвард ДО сеттера» не пиннится),
`TD-189` (две реализации одной публикуемой величины), `TD-190` (расхождение `fa/book.md` §7
microprice↔mid), уточнение `TD-168` замером `MUT-CLOSE`.

**`TD-158` ЗАКРЫТ этим милестоуном** — обе его половины: отставание 1 Гц против дельт 100 мс
устранено (серия считается на дельтах), а каденция теперь НАЗВАНА в выдаче полем
`SeriesBundle::cadence_ms`, чего и требовал `П-014` п.2 («выдача обязана это называть, а не
умалчивать»).

**Почему milestone НЕ закрыт.** `docs/fa/viz-backend.md` после этого merge'а содержит два
ложных утверждения (предусловие §4 про snapshot-only и оборванная на 7→8 цепочка бампов §5).
Правка готова и запушена (`3a88f90`, ветка `docs/fa-viz-M-68-close`), но обязана лечь ПОСЛЕ
этого merge'а — раньше она утверждала бы о коде то, чего в нём нет. Приземление — своим PR
через круг critic'а по `gates.md` §9. До того — `TD-185` OPEN.

## M-73 «офсайт-копия журнала по расписанию + сторож кэша сборок» (`П-023`) — ✅ КОД В MAIN (merge `7581f60` 2026-08-30, `R-158` APPROVED, круг 4) + **В ПРОДЕ, §8 ПРОЙДЕН**; milestone **НЕ ЗАКРЫТ — close-out за architect'ом**

**Что теперь есть у продукта, чего не было.** Боевой журнал перестал существовать в одном
экземпляре: `deploy/bin/journal-offsite-cron.sh` копирует его на Hetzner Storage Box **раз в
час** (`deploy/cron.d/journal-offsite`, `22 * * * *`), инкрементально, только файлы старше
15 минут (активный сегмент пишется — его копия была бы обрывком под видом целого).
Отдельно `deploy/bin/builder-prune-cron.sh` (`30 4 * * *`) не даёт BuildKit-кэшу расти молча —
именно он в августе увёл диск прода на 90 %.

**Конструктивные запреты, а не пожелания** (каждый пиннится шагом `verify_M-73.sh`):
`rsync` БЕЗ `--delete` — команда, создающая бэкап, физически не может его уничтожить;
`--partial-dir=.rsync-partial` — оборванная передача лежит в подкаталоге, а не под финальным
именем; `docker image prune -a` и `volume prune` запрещены (первый ломает rollback, второй
сносит журнал); `RETENTION_MODE` остаётся `dry-run` — включение реального удаления есть
отдельное решение founder'а ПОСЛЕ restore-drill.

**Главная находка цикла — `R-157` `Б-5`, и она про наблюдаемость, а не про копирование.**
Сторож сбоя источника был МЁРТВ: `PIPESTATUS` сбрасывается после КАЖДОЙ команды, включая
присваивание, поэтому `find_rc=${PIPESTATUS[0]}` во второй строке всегда читал ноль. При
падении `find` обёртка писала в лог `OK`, ставила свежую отметку успеха, ГАСИЛА alert и
выходила с нулём — **не скопировав ничего**. Ровно то состояние, которое милестоун объявил
недопустимым: «копия устарела, а никто не знает» (`OPS-I-2`), и класс `OPS-I-8` («жив, но не
работает»). Дефект достижим не гипотетически: `find` выходит с `1`, когда файл исчезает между
`readdir` и `stat`, а компакция (03:50) переписывает `.jrnl` → `.jrnl.zst` при копии,
ходящей каждый час, в том числе 04:22.

**Закрыто ДВУМЯ звеньями, и второе важнее первого.** Фикс дева (`dda3a80`) — снимок
`PIPESTATUS` одним массивом до присваиваний. Оракул архитектора (`712b96d`,
`scripts/tests/red_offsite_pipeline_rc.sh`) — то, чего у механизма наблюдаемости не было:
СВОЕГО наблюдателя. Проба ИСПОЛНЯЕТ обёртку прод-формой вызова со стаб-`PATH` (`nice`,
`ionice`, `flock`, `date` — настоящие) и судит тройку `(exit, наличие alert, наличие
last-success)` в трёх сценариях. Прежний гейт из 17 шагов не запускал конвейер ни в одном
шаге — судил тексты и argv, поэтому мёртвый сторож прошёл ДВА круга ревью зелёным.

**Мутационный контроль прогнан reviewer'ом, а не принят из отчётов** (`R-158`):

| мутация | исход |
|---|---|
| прямая — вернуть дефект `Б-5` | `FAIL 1/3`, красный РОВНО сценарий `C` |
| обратная — снять развязку `SIGPIPE 141` | `FAIL 1/3`, красный РОВНО сценарий `A` (фикс «любой ненулевой `find` — сбой» сломал бы штатный путь) |
| несостоявшийся setup — стаб `find` не исполняем | `SETUP НЕ СОСТОЯЛСЯ`, `exit=1` — проба не зеленеет по чужой причине |

**Гейты (прогон reviewer'а на чистом worktree).** `verify_M-73.sh` → **18 PASS / 0 FAIL**,
`exit=0`, `stderr` пуст (включая паритет с CI: `fmt` + `clippy --all-targets --all-features
-D warnings` + `cargo test --all`). `red_offsite_pipeline_rc.sh` → **3/3**, `exit=0`.
`check_review_fa` (`VB-I-1`/`VB-I-3`), `check_gate_meta`, `check_artifact_ids`,
`check_protected_artifacts` — все `exit=0`. Дерево слияния проверено отдельно (ветка отставала
от `main` на **134 коммита**, а защита стоит с `strict: false`): `verify_design_claims.sh
--merge-preview origin/main` → `PASS`; локальная тройка CI на merge-дереве → `passed=948
failed=0` (221 блок). PR #129 — 17/17 `pass`, агрегат `All checks passed`.

**§8 деплой-гейт — ПРОЙДЕН, подключение проверено ГЛАЗАМИ (`gates.md` §4 DoD).**
CI `33330478744` success, Deploy `33330478723` success. На проде:

```
$ ssh … 'git -C /root/hft-platform rev-parse HEAD; ls -la /etc/cron.d/; ls -l deploy/bin/*.sh'
7581f603bddff2f18a4356f90e74e7f2f4eb70b0                  ← ревизия = вершина main
-rw-r--r-- 1 root root 3119 Aug 30 19:28 hft-builder-prune
-rw-r--r-- 1 root root 5009 Aug 30 19:28 hft-journal-offsite     ← поставлены деплоем
...
**Eyes-on ПЕРВОГО АВТО-прогона (не ручного) — ПРОЙДЕН 2026-08-30T20:22Z.** Именно этого
требует `deploy/README.md` §2 («установлено» ≠ «работает») и урок `TD-020`:

```
$ ssh … 'grep journal-offsite /var/log/syslog | tail -1'
20:22:01 CRON[561596]: (root) CMD (/root/hft-platform/deploy/bin/journal-offsite-cron.sh)
                                          ← задание запустил CRON, а не человек
$ ssh … 'cat /var/lib/hft/journal-offsite.last-success'
2026-08-30T20:22:28Z                      ← свежая отметка успеха
$ ssh … 'tail -6 /var/log/hft/journal-offsite.log'
Number of created files: 1 … Total transferred file size: 1.07G bytes
2026-08-30T20:22:28Z OK duration=26s      ← закрытый сегмент доехал за 26 с
$ ssh … 'ls /var/lib/hft/journal-offsite.alert' → (alert-файла нет)
$ ssh … 'docker ps; df -h /'
hft-gateway-serve Up 54 minutes (healthy) · hft-recorder Up 54 minutes (healthy) · / 65 %
```

Числа сходятся с формой, объявленной в шапке cron-файла (≈1100 МБ на сегмент, ≈27 с);
recorder не задет. Механизм работает САМ.

Это же снимает предел, названный `R-157` `Б-4`: до merge'а исполняемая ревизия обёртки была
недоказуема (дев копировал скрипт руками и убирал за собой), теперь она приходит из
`git reset --hard <SHA>` и равна вершине `main` по построению.

**Долг, уехавший ОБЪЯВЛЕННЫМ** (`gates.md` §4 это допускает, молчание — нет):
`TD-191` (`deploy/README.md:126` объявляет установку cron ручным шагом с подписью founder'а,
тогда как `deploy.yml:288` ставит её сам с M-48 — этот самый merge включил на проде два
расписания автоматически), `TD-192` (гейт не зовёт `crontab -n`, хотя деплой на нём
обрывается fail-closed), `TD-193` (`OPS-I-3` restore-drill не проводился ни разу, и после
M-73 это опаснее: «копия есть» без доказанной читаемости).

**`TD-183` и `TD-184` ЗАКРЫТЫ этим милестоуном.** `TD-183` — runbook больше не предписывает
CIFS-монтирование, которого на коробке нет (§1 переписан на SSH-субаккаунт, форма
`user@host:path` совпадает символ в символ с печатаемым `DST_URL`). `TD-184` — у кэша сборок
появился наблюдатель, подключённый к проду.

**Почему milestone НЕ закрыт.** Три вещи в зоне architect'а: `§Tasks` спеки стоит `⏳ OPEN` на
всех пяти задачах при фактически закрытых, а задачи круга 4 (`Б-5`, `Н-10`) в таблицу не
внесены вовсе; `docs/fa/ops.md:399` всё ещё утверждает «журнал существует… **сегодня — в
одном** [месте]» — строка протухла 29.08; и `OPS-I-3` (restore-drill) не выполнен, то есть
цель `П-023` достигнута наполовину — `TD-020` (acceptance-ворота Ф0) остаётся OPEN, retention
остаётся `dry-run`. Порядок «копия → восстановление → удаление» необратим, и второй шаг ещё
не сделан.

## M-45 «раскатка L2Delta на ETHUSDT» (подпись `П-026`, эпоха `E-002`) — ✅ КОД В MAIN (merge PR #140 `a72ca04` 2026-09-01, `R-168` APPROVED, круг 2) + **В ПРОДЕ, §8 ПРОЙДЕН 2026-09-01T18:50Z**; milestone **НЕ ЗАКРЫТ — close-out за architect'ом**

**Что приземлилось.** Состав записи `L2Delta` расширен с `BTCUSDT` на `BTCUSDT,ETHUSDT` —
на СПОТЕ и на ФЬЮЧЕРСАХ, как подписано `П-026` (founder, 2026-08-31, дословно «спот и
фьючерсы»). Раскатка — две строки литералом в `docker-compose.yml` на сервисе `recorder`:
`L2DELTA_CAPTURE_SYMBOLS: BTCUSDT,ETHUSDT` + `EPOCH_ID: own-2026-09-m45-ethusdt`. Смена
состава = новая эпоха (`CT-RFC-06` §3), поэтому обе переменные внесены ОДНИМ коммитом
(`f3b84d4`, шаг `T10b` это пиннит).

**`crates/**` не тронут ВООБЩЕ.** Дифф — compose (+2), докс, артефакты гейтов, харнесс.
Следствия, проверенные reviewer'ом замером, а не рассуждением: Block-C пройден
(`crates/contracts/**` не тронут, `SCHEMA_VERSION` не бампался); **risk-critic не требовался**
— order-egress отсутствует как класс, MD-only carve-out `gates.md` §5; `crates/risk`,
`crates/killswitch` не тронуты. Живой FA-инвариант предмета — `VN-I-3` (`docs/fa/venues.md:176`):
ветвления по `venue_id` в core не появилось, состав задаётся конфигурацией на границе.

**Цепочка гейтов (5 кругов критика + 2 арбитража + 2 круга reviewer'а).**
`C-195` → `C-197` → `C-199` → `C-202` → `A-030` (форма факта: состав пишется ЛИТЕРАЛОМ, не
подстановкой) → `C-203` → `R-167` **REJECT** (эпохальная половина раскатки не держалась
ничем) → `C-204` → `A-031` (класс стражей присутствия: «отсутствие оракула ≠ успех») →
`C-205` CLEARED → tester PASS → **`R-168` APPROVED**.

**Механизм, которым это удерживается.** `scripts/verify_M-45.sh` (24 шага) + компаратор
`scripts/lib/rollout_symbols_check.py`. Хватка ИЗМЕРЕНА семью мутациями (пять tester'а, две
reviewer'а), не заявлена: снос фьючерсного оракула захвата роняет `T6`; расширение состава
третьим символом при валидной эпохе роняет `T10` с текстом «неподписанное расширение
границы C»; устаревшая эпоха под новым составом роняет `T10`; проза с тем же литералом вне
ячейки факта роняет `T9`.

**§8 деплой-гейт — ПРОЙДЕН, eyes-on по каждой площадке отдельно** (подпись покрывает обе, а
один литерал `L2DELTA_CAPTURE_SYMBOLS` их не различает, поэтому проверялись раздельно):
CI `33544787774` success, Deploy `33544787866` success + `33545828263` success;
`hft-recorder (healthy)`, env применён (`EPOCH_ID=own-2026-09-m45-ethusdt`,
`L2DELTA_CAPTURE_SYMBOLS=BTCUSDT,ETHUSDT`); журнал растёт (events 10 900 → 23 223,
`next_seq` 432 821 288 → 432 833 615); heartbeat свежий (10 с); **RssAnon 14 096 → 14 228 kB**
— стабилен, не `docker stats` (`TD-021`). Содержательная санити (обязательна, деплой менял
СОСТАВ данных): `ETHUSDT` живёт на обеих площадках — `venue_binance` «book levels
symbol=ETHUSDT bids=5004 asks=5001» и `venue_binance_futures` symbol=ETHUSDT.
**Всплеск WARN разобран, а не проигнорирован:** 55 предупреждений «depth continuity gap /
snapshot stale» пришлись РОВНО на минуту старта `T18:49` и прекратились полностью (`T18:50`
и далее — ноль). Это bootstrap книги после рестарта, а не деградация; отличено замером
частоты по минутам, а не на глаз.

**Долг, уехавший с merge'ем (заведён reviewer'ом, `R-168`):**
- `TD-194` MAJOR, В ПЛАНЕ — после merge'а литерал состава не удерживается ничем машинным:
  `verify_M-45.sh` в CI не зовётся (`grep -c 'verify_M-45' .github/workflows/ci.yml` → `0`).
  Класс «built-not-wired» `gates.md` §4; предмет охраны — граница C фазы Ф1.
- `TD-195` MINOR, ЗАМОРОЖЕНО — `verify_M-45.sh` зеленее CI (нет `cargo test --all`,
  clippy без `--all-features`); перекрыт агрегатом `All checks passed` на PR-маршруте.
- `R-168` Н-2 (перенос `R-167` Н-2, **решение за founder'ом**): компаратор
  `scripts/lib/rollout_symbols_check.py` решает, исполнена ли подпись founder'а, но не
  сторожится НИ ОДНИМ барьером — его нет ни в `scope-guard.md`, ни в зоне
  `check_docs_freeze.sh`, ни в перечне харнесса `gates.md` §9. Спека называет это пределом
  явно; правка НОРМЫ заперта (`gates.md` §11).
- `R-168` Н-4 — дифф вышел за `§5 Allowed paths` четырьмя путями (артефакты гейтов и докс),
  семейство `TD-080`, MINOR, не блокер.

**Что осталось architect'у для закрытия:** `§Tasks` строка 7 → `✅ DONE` уже стоит; закрытие
milestone'а как такового (Status → DONE) и решение по развилке Н-2 — за architect'ом/founder'ом.


## M-75 «расцепление окна heatmap/COB от `Selector.bands`» — ✅ КОД В MAIN (merge `571403d`, PR #147, `R-169` APPROVED); **milestone НЕ ЗАКРЫТ — close-out за architect'ом**

**Что изменилось по существу.** Полуширина окна heatmap и COB перестала выводиться из
`max(Selector.bands)` и стала СЕРВЕРНОЙ настройкой. Полосы вернулись своему предмету —
депт-серии.

Расцепление сделано СУЖЕНИЕМ ВХОДА, а не проверкой: `build_heatmap_and_cob` принимает
`window_frac: f64` вместо `&Selector` (`crates/gateway/src/lib.rs:1577-1581`). Пока функция
принимала весь селектор, ничто не мешало следующей правке снова достать из него `bands`;
теперь связка невыразима на уровне типов.

**Два следствия, ради которых милестоун существовал:**

1. **подписанные полосы (`П-014` п.4) больше не выносят ответ за подписанный предел `П-020`.**
   До расцепления канонический набор давал `observed=7 882 335 Б` при `limit=2 000 000` —
   усиление ×3.9 на фикстуре, ×22.6 на прод-форме;
2. **закрыто усиление ресурса, управляемое клиентом (`PL-I-5`)** — ширина карты перестала быть
   клиентским входом ПО ПОСТРОЕНИЮ, а не по проверке. Предел `M-71` остался бэкстопом и не
   тронут.

**Env-контракт (fail-closed).** `GATEWAY_HEATMAP_WINDOW`, дефолт `0.001`. Отсутствие →
подписанный дефолт; заданное значение обязано разбираться и лежать строго внутри `(0, 1)`,
иначе ОТКАЗ СТАРТА. Пустая строка считается заданным невалидным значением (прод зовёт
`std::env::var(k).ok()`, то есть получает `Some("")`), `NaN`/`inf` отсекаются `is_finite()`.
Подстановки дефолта на мусоре нет — прямое требование `PL-I-5` и урок `R7`.

**Данные не изменились ни на байт, и это проверено на проде, а не по compose-дефолту.**
Замер reviewer'а 2026-09-02 с живого контейнера: `GATEWAY_BANDS=0.001`, то есть
`max(bands) = 0.001 = DEFAULT_HEATMAP_WINDOW_FRAC`. Расцепление снимает ЗАВИСИМОСТЬ, а не
сдвигает величину.

**Чего милестоун НЕ сделал (и это не упущение):** `GATEWAY_SCHEMA_VERSION` НЕ бампнут,
`Selector` и `selector_fingerprint` не тронуты, `crates/contracts/**` и `CT-RFC-09` не
изменены, `GATEWAY_BANDS` не тронут. Бамп зарезервирован за пирамидой разрешений — два бампа
с разных веток дали бы коллизию версий (класс `C-190`).

**Чекпоинт НЕ инвалидируется — установлено кодом, а не аналогией.** `read_checkpoint`
(`lib.rs:3362-3367`) возвращает `Reducer`, то есть СОСТОЯНИЕ; бакет несёт ПОЛНЫЙ снимок книги
(`entry.refresh(bids, asks)`, `lib.rs:1204`) без отсечения по окну; окно применяется на
ПОСТРОЕНИИ (`lib.rs:1506`). Тёплый старт равен полному реплею (`MD-I-8` обязательство 9),
`VB-I-2` цел: `finish` делегирует в `finish_ref`, у построителя карты РОВНО ОДИН вызыватель,
оба пути читают один процессный атомик. Отдельно проверено reviewer'ом, что чекпоинтер —
другой процесс — выдачу не строит вовсе и `GATEWAY_HEATMAP_WINDOW` не читает, поэтому
расхождения окна между процессами не существует по построению.

**Гейты (артефакты, не пересказ).** plan-time — пять кругов критика: `C-194` REJECT →
`C-196` REJECT → `C-198` REJECT → `C-201` REJECT → **`C-207` NOTE**; арбитраж `A-031` §1 п.1
исполнен (выписана группа стражей присутствия, четыре сужены до предмета требования).
tester — PASS на чистом чекауте. PR-time — **`R-169` APPROVED**.
RISK-BLOCK не применялся: диапазон не трогает `risk`/`killswitch`/`oms`/`venue-*`.

**Прогон reviewer'а на своём worktree (не пересказ):** `cargo fmt` exit=0 · `cargo clippy
--all-targets --all-features -D warnings` exit=0 · `cargo test --all` **960 passed / 0 failed**
(225 блоков) · `scripts/verify_M-75.sh` **`VERDICT: PASS`, exit=0, 42 PASS / 0 FAIL**. Совпало
с замером тестера по всем четырём числам.

**Мутационный контроль reviewer'а — оракулы пиннят предмет, а не шумят.** Точка мутации
`lib.rs:1581`. Мир «жёсткая константа, конфиг игнорируется» (`w = 0.001`) убит `H-6`
(`hw_i_6` FAILED, парный `hw_i_6b` остался зелёным) — важно, что пять оракулов `crates/gateway`
этот мир ПРОПУСКАЮТ, его ловит только цепь `env → serve_config_from_env → выдача`. Мир
«расцепить обнулением» (`w = 0.0`) убит всеми пятью (`H-1`/`H-3`/`H-4` и `H-5`/`H-5b`).
Восстановление предъявлено: 12 passed / 0 failed, дерево чисто.

**Merge-preview (`strict: false`).** `main` уехал на 84 коммита от базы милестоуна, но дрейф
не содержит ни одного `.rs` и ни одного `Cargo.toml` — тройка CI на дереве слияния тождественна
прогону на ветке структурно, а не по вере. Сухое слияние без конфликтов (`docker-compose.yml`
тронут обеими сторонами, но в разных секциях). Новый джоб `rollout-composition`, приехавший в
`main` и входящий в обязательный агрегат, прогнан reviewer'ом на дереве слияния — `PASS`.

**Долг, уехавший объявленным:** `TD-197` — окно не объявлено в кадре выдачи, серию нельзя
воспроизвести по одному ответу. Назван architect'ом в спеке §4bis(2) ЗАРАНЕЕ; носитель
развязки — спека пирамиды, где форма кадра меняется вместе с бампом схемы.

**Почему milestone НЕ закрыт:** close-out (статус спеки, архивирование гейта) — зона
architect'а; `PROJECT-STATE.md`/`TECH-DEBT.md` — зона reviewer'а, они обновлены этим коммитом.

## M-77 «кадр строится БЕЗ книги: `VB-I-2` восстановлен на прод-пути `pump`» (`TD-199`) — ✅ КОД В MAIN (merge PR #167 `b008f5e` 2026-09-07, `R-176` APPROVED) + **В ПРОДЕ, §8 ПРОЙДЕН 2026-09-07T14:03Z**; milestone **НЕ ЗАКРЫТ — close-out за architect'ом**

**Что было сломано.** Клиент, собравший состояние из `snapshot(C) + Σ frames`, получал НЕ ТЕ
книго-зависимые серии, что даёт полный реплей того же окна: кадр строился свежим
`Reducer::new` с ПУСТОЙ книгой, а `depth_series`/`heatmap`/`cob` считаются ИЗ книги. Нарушен
`VB-I-2` (`docs/fa/viz-backend.md:199`), карточка `TD-199` (MAJOR), предмет выделен из `M-70`
решением `П-022`.

**Что сделано (развязка Б, выбрана `C-211`).** Кадр берёт книго-зависимые серии срезом ЖИВОГО
редьюсера, адресованным ДИАПАЗОНОМ кадра `(from, upto]`, а не текущей позицией состояния:
`Reducer::book_series_in` (`crates/gateway/src/lib.rs:1611`) поверх per-event наблюдений
`Reducer::book_series` (`:843`); `SeriesBundle::set_book_series` замещает, а не сливает.
Закрыты **ОБЕ** точки создания кадра — `:4502` (граница батча) и `:4572` (хвост); правка одной
из них оставляла дефект живым на всяком тике длиннее батча, и четыре теста круга 1 против неё
оставались зелёными.

**Почему это не купили ценой `VB-I-10`.** Захват наблюдений включён ТОЛЬКО на `self.full`
живого редьюсера (`LiveReducer::resume`); `snapshot`/checkpoint/replay идут с выключенным
флагом. Эвикция — по закладке ДОСТАВКИ, после успешного commit'а кадра (до него наблюдения
обязаны жить: К-1 требует, чтобы повторная попытка того же диапазона дала побайтно тот же
кадр). Удержание ограничено одним батчем: `pump` при отказе по пределу возвращает `Err` ДО
применения следующих событий.

**Гейт прогнан РЕВЬЮЕРОМ заново, а не принят пересказом:** `scripts/verify_M-77.sh` →
`VERDICT: PASS`, exit=0, 13/13 подтестов `T0`–`T8` (включая паритет с базовым CI). Отдельно —
мутационный контроль ревьюера: нейтрализация ОБЕИХ точек применения даёт
`red_m77_frame_book_continuity` 5/6 красных и `red_m77_delivery_window` 2/3 красных, зелёными
остаются ровно контроли. То есть зелёное не вакуумно.

**Цена названа числом, а не словом.** Ось «число батчей» при постоянной книге: было `0.994`,
стало `1.160` при потолке сторожа `1.350`; абсолютный прирост тика +7.3 % на одном батче и
+25 % на 64. Условие (iii) `C-211` исполнено.

**§8 деплой-гейт (сырые строки).**
```
CI            success  10m11s  run 34129336776   (push b008f5e)
Deploy to VPS success   2m34s  run 34130273105
ssh 167.233.192.131:
  hft-gateway-serve Up 3 minutes (healthy)
  hft-recorder      Up 4 minutes (healthy)
  heartbeat {"events":28725,"next_seq":493702701,"segment_index":580,"writable":true,
             "ts_wall_ms":1788789797357}   ← свежий (сервер: 14:03:19Z)
  segment-00000580.jrnl 312 234 820 Б, mtime 14:03  ← журнал растёт
  RssAnon recorder 17 260 kB
```
**Предел этого пруфа назван, а не умолчан:** проверена ЖИВОСТЬ, а изменённый путь на проде
сегодня НЕ нагружен — живых подписчиков `gateway-serve` нет (в логе только 30-секундный
healthcheck-хендшейк). Формат записи деплой не менял, поэтому sanity свежих событий здесь
не применим; поведенческое покрытие изменённого пути дают оракулы, а не прод.

**Долг, уехавший ОБЪЯВЛЕННЫМ (карточки заведены этим close-out'ом):**
- `TD-200` (MAJOR, ПРЕДСУЩЕСТВУЕТ в `main`) — per-event стоимость `pump` растёт с размером
  книги: отношение 4.39 при равном выходе. `M-77` не вносит и не чинит; гейт намеренно не
  судит эту ось (`A-031`).
- `TD-201` (MINOR) — новое хранилище наблюдений не сторожится названным сторожем: `T8` меряет
  отношение аллокаций по оси «число батчей», `red_gateway_bounded` идёт с выключенным флагом
  захвата. Плюс: цифра стоимости в док-комментарии поля не замерена.

**`TD-199` ЗАКРЫТ** этим merge'ем (карточка помечена в `TECH-DEBT.md`, тело оставлено —
оно носитель замера).

**Почему milestone НЕ закрыт:** close-out (статус спеки, архивирование гейта) — зона
architect'а; `PROJECT-STATE.md`/`TECH-DEBT.md` — зона reviewer'а, они обновлены этим коммитом.

## M-74 «restore-drill: холодная копия обязана ЧИТАТЬСЯ» — ⚠️ **ЦЕЛЬ ДОСТИГНУТА РУЧНЫМ ПРОГОНОМ 2026-09-08 и ПРИНЯТА founder'ом (`П-028`); милестоун НЕ ВЫПОЛНЕН как набор задач**

**Две формулировки, смешение которых и есть способ закрыть милестоун поверх недоделанного:**
**цель достигнута** ≠ **набор задач выполнен**. Обе верны одновременно, и ниже они разведены.

**Что ДОСТИГНУТО и чем предъявлено.** 2026-09-08 restore-drill исполнен ВРУЧНУЮ на настоящей
холодной копии (Storage Box) и принят founder'ом подписью `docs/PENDING-SIGNATURE.md` `П-028` —
это объявленные ворота фазы Ф0 (`docs/DESIGN.md` §10, колонка founder: «★ приём restore-drill'а»).
Предъявлены ТРИ свойства, а не одно; сырой вывод команд — в теле `П-028` (оно самодостаточно:
`R-177` `Б-1` заставил перенести доказательство в `main`, а не адресовать за ним на ветку):

| свойство | предъявлено |
|---|---|
| копия ЧИТАЕТСЯ | отбор по правилу спеки из 593 индексов — первый / `floor(N/2)` / последний = `00000001` (15 июля, сжатый) · `00000297` (17 августа, сжатый) · `00000593` (8 сентября, сырой). `DRILL_READER rc=0`, `segments_read=3`, **`events_read=2170780`**, `seq` 16 049 334 → 509 116 924, `intra_segment_continuous=true` |
| чтение ДЕТЕРМИНИРОВАНО | повторное чтение той же копии дало ТОТ ЖЕ отпечаток `96a231ce9731…` |
| зелёное ЧТО-ТО ЗНАЧИТ | 32 байта инвертированы в середине сжатого сегмента ⇒ `rc=4`, `events_read=0`, отпечатка нет, `reason=corrupt:frame crc mismatch` |

Третья строка — не формальность: позитивный контроль без негативного доказывал бы только то,
что процедура не падает.

**Инструментом стало наследство шести кругов гейта, а не обход их.** Ручной прогон исполнен
ЭТАЛОННЫМ ЧИТАТЕЛЕМ из набора (`fixture_restore_drill_cold.rs::reference_reader_for_shell_probe`);
формула отпечатка и классы исхода `0`/`4`/`5` — те, что вылизаны кругами `C-217`/`C-218`.

**Что НЕ СДЕЛАНО — перечислено, а не подразумевается:**

- **Задачи 2 / 2b / 3 / 4 / 5 НЕ ВЫПОЛНЕНЫ** (состав — по шапке `scripts/verify_M-74.sh`):
  прод-обёртка, прод-читатель, продюсер метрики, расписание.
- **`scripts/verify_M-74.sh` КРАСЕН — и красен ЧЕСТНО:** гейт написан RED-first ДО работы, шаги
  на незакрытых задачах краснеют по построению. Последний известный прогон — `FAIL (2 из 18)`.
- **Гейта нет в `main`.** `git show origin/main:scripts/verify_M-74.sh` → `exit=128`. И спека
  `milestones/M-74-restore-drill.md`, и гейт, и оракулы живут на ветке
  `docs/M-73-closeout-architect`, которая в `main` НЕ вливается: она несёт намеренно красный
  RED-набор (`cargo test --all` красен по построению). Это объявленное состояние, а не поломка —
  но означает, что предъявить набор из `main` сегодня НЕЛЬЗЯ.
- **ПОВТОРЯЕМОСТЬ не исполнена.** `OPS-I-3` (`docs/fa/ops.md:405`, `:471`) требует буквально
  «периодически (раз в месяц)». Проверена ОДНА копия — сегодняшняя; следующая не проверена ничем.
- **НАБЛЮДАЕМОСТЬ не исполнена.** `backup_restore_drill_ok` (`docs/fa/ops.md:102`) объявлена и
  не эмитится; правило алерта `OPS-BKP` ждёт величину, которой никто не производит — класс
  `OPS-I-10` (`docs/fa/ops.md:478`): «ОБЪЯВЛЕНА ⟹ ЭМИТИТСЯ», объявления недостаточно.

**Кругов гейта по ЭТОМУ набору больше не назначается — их было шесть.** Решение founder'а
2026-09-08: вопрос «читается ли копия» снят замером, автоматизация вынесена ОТДЕЛЬНЫМ предметом
ДРУГОМУ исполнителю. Вместе с остатком переезжают ДВЕ открытые находки круга 6 (`C-219`,
ветка `docs/M-73-closeout-architect`, в `main` их нет):

- `C-219` `B-1` — `INV-DELIVERY` racy: отсутствие источника не удерживается, пока о нём судят
  (обёртка успевает перепривязать `cold` после того, как проба переименовала источник);
- `C-219` `B-2` — обязательные `*.json` sidecar'ы можно молча опустить или подменить, а `H`
  остаётся зелёным; нейтрализация цикла копирования sidecar'ов оракул не уронила.

**Долг, уехавший ОБЪЯВЛЕННЫМ (реестр обновлён этим close-out'ом):**
- `TD-193` — переведена из «не выполнялось НИ РАЗУ» в **ЧАСТИЧНО** (не закрыта): выполнено —
  однократный принятый drill; остаётся — повторяемость и наблюдаемость.
- `TD-202` (MAJOR, **built-not-wired**) — том `gateway-ckpt` не смонтирован в `journal-retention`,
  уборщик fail-closed на каждом кандидате. Заведена этим же close-out'ом, к `M-74` не относится,
  но стоит ровно на том же пути Ф0 (включение удаления).

**Что подпись `П-028` РАЗБЛОКИРУЕТ и чего НЕ разблокирует.** Снято то условие `П-023`, которое
требовало доказанной читаемости копии перед удалением. Включение `RETENTION_MODE=apply` этим НЕ
разблокировано: это отдельное решение founder'а (граница C), и техническая готовность ещё не
достигнута — см. `TD-202` и `docs/ROADMAP.md` п.2.

**Разовое ручное удаление 2026-09-08 (санкция founder'а, граница C) — исполнено ВНЕ пути
ретеншена.** 392 сегмента старше 14 суток; пофайловая сверка `sha256` с хранилищем — 392 из 392
без расхождений; покрытие чекпоинтом выше удаляемого диапазона; активный сегмент не тронут;
осталось 203 при `keep-min=4`. Освобождено 57 ГиБ (свободно 27 → 84 ГиБ). После удаления
предъявлено ИСПОЛНЕНИЕМ, что журнал читается: индексы 393..595 без дыр, чекпоинтер прошёл
насквозь `exit=0` и сдвинул покрытие. Аудит-след проверен reviewer'ом на сервере 2026-09-09:
`/var/lib/hft/prune-audit/pruned-2026-09-08.{list,sha256}` — оба файла на месте.
**Удаление было РАЗОВЫМ:** без развязки `TD-202` вопрос места вернётся.

**Почему milestone НЕ закрыт:** набор задач не выполнен (см. выше), а close-out спеки
(статус, архивирование) — зона architect'а. `PROJECT-STATE.md`/`TECH-DEBT.md` — зона reviewer'а,
обновлены этим коммитом. Вердикт close-out'а — `research/reviews/R-178-closeout-2026-09-09.md`.

## M-86 «профиль объёма отдаётся на ФИКСИРОВАННОЙ ценовой сетке» (`TD-206`) — ✅ **ЗАКРЫТ 2026-09-20** (код: merge PR #198 `46cd25c`, `R-188` APPROVED; close-out architect'а: PR #200 `63854d4`; финальный гейт и решение по долгу: `R-189` APPROVED) + **В ПРОДЕ, §8 ПРОЙДЕН ПОЛНОСТЬЮ**

**Что сделано.** Профиль объёма отдаётся корзинами фиксированной ширины `W` вместо одной
корзины на торгованную цену. Ключ = `price_e8.div_euclid(W) * W`, якорь — ноль, дефолт
`DEFAULT_VP_BIN_WIDTH_E8 = 25_000_000` e8 (0.25 USD, решение founder'а 2026-09-18, вариант A).
Огрубление — в ЕДИНСТВЕННОЙ точке выдачи `VolumeProfileAcc::vp_rows()`, откуда его получают и
снапшот, и дельта-кадры, и реплей. `POC/VAH/VAL/VA%` считаются ПОСЛЕ огрубления.

**Чего НЕ тронуто — и это конструкция, а не совпадение:** состояние остаётся тиковым, поэтому
чекпоинт валиден; `selector_fingerprint` не тронут (sha256 тела сверен гейтом), поэтому слепки
не инвалидированы и ≈23 минут пересборки мы не платим; `GATEWAY_SCHEMA_VERSION` остался `10`;
`crates/contracts` не тронут — T1 и contract-RFC ни при чём.

**PR-гейт `R-188` (`research/reviews/R-188-M-86-vp-bin-width.md`).** Reviewer прогнал гейт сам:
`verify_M-86.sh` — 38 PASS / 0 FAIL, exit=0; `cargo test` — `passed=1040 failed=0`.
Мутационный контроль после ремонта по `A-035` печатает три свидетеля (W0 baseline GREEN,
W1 мутант собрался, W2 упал СВОИМ диагнозом) — класс «PASS по неспецифичному свидетелю»
(`C-231` B3) закрыт helper'ом `scripts/lib/mutation_gate.sh`, а не внимательностью автора.

**Барьеры проверены на ДЕРЕВЕ СЛИЯНИЯ, и там нашлось то, чего гейт ветки не видел:**
`check_gate_meta` дал `exit=1` — (а) subject-lock: `verify_M-86.sh` тронут ПОСЛЕ проходного
вердикта `A-035`; (б) merge называет `M-86`, а вердикта в дереве нет. Оба закрыты вердиктом
`R-188` и явным `ALLOW-SUBJECT-CHANGE` (токен — аудит-след, не доказательство; доказательство —
разбор ремонта в §5 вердикта).

**Деплой-гейт `gates.md` §8 — четыре пункта из пяти, пятый назван, а не замолчан:**

| # | проверка | результат |
|---|---|---|
| 1 | CI + Deploy на `46cd25c` | оба **success** |
| 2 | контейнеры / heartbeat / рост журнала | `hft-gateway-serve` и `hft-recorder` `(healthy)`; heartbeat свежий; `next_seq` 629 811 122 → 629 816 136 за 90 с |
| 3 | ресурсы (RssAnon, не `docker stats`) | gateway-serve **10 760 kB**, recorder **15 748 kB**; CPU 0.00 % / 2.65 %, load 0.33 |
| 4 | ручка доехала | `docker inspect` → `GATEWAY_VP_BIN_WIDTH_E8=25000000` **без единой строки оператора** (в host `.env` из `GATEWAY_*` только `GATEWAY_JWT_SECRET`) |
| 5 | **E2E: кадр доходит до клиента** | зонд внутри прод-контейнера получил КАДР — `snapshot.json` **95 597 Б** при пределе 2 000 000; отказов `exceeds limit` в логе после рестарта — **0**. Токен выписан вручную по сырым байтам секрета (обход `TD-207`) |

**Сетка РАБОТАЕТ на проде, и это предъявлено числом:** все **167 из 167** ключей корзин кратны
`25 000 000` e8, `poc/vah/val` — тоже ключи сетки. Для сравнения, цены в `cob` того же кадра
тиковые (`81208.83`, `81212.10`) — то есть огрубление применяется именно к профилю.

**Первый замер НЕ доказывал главного, и это было записано, а не сглажено.** UTC-сутки на
момент зонда шли 7 минут (`session_id=20716`, ход 50.75 USD, 167 корзин) — такой кадр был бы
мал и БЕЗ `M-86`. Поэтому `R-188` пометил `TD-206` **ЧАСТИЧНО** закрытым и назвал условие
полного: повторный зонд на зрелой сессии.

**ВТОРОЙ ЗАМЕР — ЗРЕЛАЯ СЕССИЯ, 2026-09-20T16:26Z (16.5 ч), прод-форма вызова.** Снят
architect'ом, **независимо перемерен reviewer'ом** (слепок скопирован с прода и разобран
своим кодом, а не прочитан по таблице) — совпал по каждому числу:

| | до фикса (20.6 ч сессии, 18.09) | после (16.5 ч сессии, 20.09) |
|---|---:|---:|
| кадр | **5 533 287 Б** — отвергался fail-closed | **227 579 Б** = 0.114× предела |
| корзин профиля | 211 381 | **4 810** |
| корзин НЕ на сетке | — | **0 из 4 810** |
| `poc`/`vah`/`val` кратны шагу | — | все три (живой `VP-I-4`) |
| сессий в кадре | 1 | 1 (эвикция прошлой работает) |
| отказов `exceeds limit` | непрерывно | **0 за 24 ч** |

Состав кадра: профиль 120 008 Б · heatmap 67 465 · bubbles 24 402 · ohlcv 7 566 · depth 3 621.
Прод на момент замера: `46cd25c`, оба контейнера `(healthy)` 16 ч, RssAnon 24 896 kB /
24 208 kB, heartbeat свежий.

**Что разбор reviewer'а ДОБАВИЛ к замеру, и почему `TD-206` закрыт полностью (`R-189`):**
плотность заполнения сетки — **0.999** (4 810 корзин на 4 817 слотов в пройденном диапазоне
1 204 USD). Профиль НАСЫЩЕН ⇒ число корзин ограничено ХОДОМ ЦЕНЫ, а не временем:
`корзин ≤ ход / 0.25 + 1`. Это граница, а не экстраполяция по плотности наблюдений. До
предела нужно ≈74 000 корзин ⇒ ≈18 500 USD пройденного диапазона; за 16.5 ч пройдено 1 204 USD.
Зонд ближе к 23:59 UTC признан НЕ нужным: предмет долга — «кадр не доходит» — опровергнут
прямо, а остаточный класс («волатильный день переполнит снова и так же тихо») уже имеет
собственную живую карточку `TD-210`; держать `TD-206` ради него значило бы завести дубль.

**Находка `R-189` `F-1` (MINOR, ничего не держала):** объявленный запас `TD-210` — «шаг
0.25 USD держит до суточного хода ≈34 730 USD, 6.8×» — оптимистичен в 1.9×. Таблица
`docs/plans/vp-frame-size-measurement-2026-09-18.md` §7 построена на заполнении 41 %, снятом
на ТИКОВОЙ сетке 0.01; огрубление в 25 раз эту долю съедает (измеренное заполнение на сетке
0.25 — 99.9 %). Пересчёт по измеренным 24.95 Б/корзину: ≈18 500 USD, запас **≈3.6×**.
Карточка `TD-210` исправлена тем же close-out'ом.

**Close-out architect'а (PR #200, `63854d4`)** закрыл находки `R-188`: `N-1` (`scripts/lib/**`
внесён в `Allowed paths` с основанием), `N-3` (шапка и пять статусов задач приведены к факту),
`N-2`/`N-4`/`N-5` приняты записью. `PR-гейт R-189` — **APPROVED**.

**Что осталось — ОДИН шаг, и он в зоне architect'а:** переезд `milestones/M-86-vp-bin-width.md`
и `scripts/verify_M-86.sh` в `docs/archive/` (`04-workflow.md` §Close-out). Разрешение выдано
`R-189` §5: `M-86` назван ЗАКРЫТЫМ, универсум номеров проверен (в архиве 29 гейтов), ВЫЗОВОВ
`verify_M-86.sh` нет нигде, включая CI. Коммит переезда обязан нести ОБА токена
(`ALLOW-SUBJECT-CHANGE` + `ALLOW-ARTIFACT-DELETE`) и тронуть `docs/ROADMAP.md` тем же
диапазоном (`roadmap-sync`).

## M-85 «кадр `frames_since` строится ОТ КНИГИ» (`TD-204`, вторая половина `TD-199`) — ✅ КОД В MAIN (merge PR #186 `3f399ec` 2026-09-17, `R-183` APPROVE) + **В ПРОДЕ, §8 ПРОЙДЕН 2026-09-17T10:25Z**; milestone **НЕ ЗАКРЫТ — close-out за architect'ом**

**Что было сломано.** `M-77` восстановил `VB-I-2` (live == replay) ТОЛЬКО на прод-пути `pump`.
Второй путь сборки кадра — публичный `gateway::frames_since` — продолжал отдавать окно СВЕЖЕМУ
`Reducer::new(selector)` с ПУСТОЙ книгой: события `seq <= after` шли только в `seed_vwap`,
который книгу не трогает. На дельта-хвосте без якорного снимка в окне книго-зависимая
депт-серия теряла точки И отдавала **фантомный ноль** как настоящее число — не «нет данных», а
ложный факт (`PL-I-7`, `docs/fa/viz-backend.md` §4). Карточка `TD-204` (MAJOR).

**Что сделано.** `Reducer::seed_book` (`crates/gateway/src/lib.rs:1117`) — зеркало `seed_vwap`
для книги: применяет к `self.book` ТОЛЬКО `MdPayload::L2Snapshot`/`L2Delta`, отобранные
`selector.matches`, НЕ сворачивая событие в серии. Вызов стоит в ОДНОЙ точке — seed-ветке
`reduce_event_stream` (`:2300`), рядом с `seed_vwap`. Весь диф импла — **46 строк** в
`crates/gateway/src/lib.rs` плюс 4 строки док-комментария в `crates/gateway-serve/src/lib.rs`
(отдельным коммитом, задача 5: шапка модуля называла `serve::frames_msgs` живым
«инкрементальным push'ем» при НУЛЕ вызовов — класс `TD-138`).

**Почему это не купили ценой соседних инвариантов** (проверено reviewer'ом открытием кода, а не
по Done Block'у):
- **VP/CVD/VWAP не бутстрапятся.** Полный bootstrap дал бы cumulative state в `frame.delta` и
  double-counting при apply — запрет `M-85` §2.1. Сторож `m85_6` пиннит именно это; на мутации
  «`apply` вместо `seed_vwap` на затравке» он краснеет.
- **Буфер наблюдений НЕ включается.** `set_capture_book_observations(true)` стоит ровно в двух
  местах (`:4281`, `:4353`), оба — живой `LiveReducer`. `VB-I-10` (память ограничена ОКНОМ, не
  длиной истории) цел; сторож `m85_4` меряет аллокации по оси «длина истории» вместе со стражем
  оси `m85_5`.
- **Фильтр затравки тот же, что у `apply`** (`selector.matches`), а `self.book` не участвует в
  оконной эвикции ни на одном пути (`evict_window_state`, `:1003`) — асимметрии «затравка внесла
  то, чего реплей бы не внёс» нет.
- **Публичная форма не тронута:** шаг гейта `СИГНАТУРА` сравнивает обе функции с `95d2422`.
  Seek-вариант `frames_since_with_stats` объявлен вне предмета (событий `seq <= after` в его
  потоке нет) и прикрыт стражем ОТСУТСТВИЯ его вызовов в `crates/*/src/**`.

**Гейт прогнан РЕВЬЮЕРОМ заново, а не принят пересказом:**
```
bash scripts/verify_M-85.sh   → 26 PASS / 0 FAIL, VERDICT: PASS, exit=0
cargo test --workspace        → passed=976 failed=0 (блоков: 230), exit=0
```

**RED предъявлен ПРЯМЫМ прогоном, а не мутацией в рабочем дереве.** Вместо того чтобы писать в
чужую зону, reviewer завёл дерево на `3b6a16c` — последнем коммите architect'а, где оракулы уже
есть, а `seed_book` ещё нет:
```
$ grep -c 'seed_book' crates/gateway/src/lib.rs        # в дереве 3b6a16c
0
$ cargo test -p gateway --test red_m85_frames_book_continuity
test result: FAILED. 3 passed; 1 failed                exit=101
  реплей: [(…010,5e8), (…011,5e8), (…012,7e8), (…013,7e8)]   ← 4 точки
  клиент: [(…010,5e8),             (…012,7e8), (…013,  0)]   ← 3 + фантомный 0
```
Расхождение побуквенно совпало с заявленным спекой, dev'ом и tester'ом; три сторожа на той же
ревизии зелены ⇒ красное принадлежит предмету, а не фикстуре.

**Второй замер — на ДЕРЕВЕ СЛИЯНИЯ с невлитым `M-70`, потому что именно там дефект был виден.**
`origin/main` (`3f399ec`) + `origin/docs/M-70-rev2` = `e7eb410`, слияние чистое:
```
$ cargo test -p gateway --test red_depth_provenance_by_reach
test gw_i_4_holds_when_the_tail_frame_is_delta_only ... ok
test result: ok. 9 passed; 0 failed
```
16.09 ТЕМ ЖЕ способом замера этот оракул был КРАСЕН (`R-182`, `TD-204`). То есть `M-85` снял
красное, которое держало `M-70`, — и это предъявлено прогоном, а не выведено рассуждением.

**§8 деплой-гейт (сырые строки).**
```
CI            success  10m1s  run 35209282404   (push 3f399ec)
Deploy to VPS success  2m10s  run 35210168745
ssh 167.233.192.131 (10:25:07Z):
  hft-gateway-serve Up 28 seconds (healthy)
  hft-recorder      Up 34 seconds (healthy)
  heartbeat {"events":2664,"free_bytes":71153139712,"next_seq":602283810,
             "segment_index":709,"ts_wall_ms":1789640703051,"writable":true}   ← свежий
  segment-00000709.jrnl 152 802 308 Б, mtime 10:25   ← журнал растёт
  RssAnon recorder 11 564 kB
```
**Предел пруфа назван:** проверена ЖИВОСТЬ. Sanity СОДЕРЖАНИЯ свежих событий здесь не
применим — деплой не менял ни формат записи, ни парсеры: правка целиком в read-path
(`gateway`), а рекордер и venue-адаптеры не тронуты (замер дифа). Поведенческое покрытие
изменённого пути дают оракулы, а не прод: живых подписчиков `gateway-serve` нет.

**Долг:**
- **`TD-204` ЗАКРЫТ** этим merge'ем (карточка помечена в `TECH-DEBT.md`, тело оставлено — оно
  носитель замера).
- **`TD-205` ЗАВЕДЁН** (MINOR): работа `frames_since` линейна по ДЛИНЕ ИСТОРИИ (память — нет,
  `m85_4`). Класс предсуществует `M-85`; спека §6 прямо требовала записать остаток долгом, а не
  выдать за решённое.

**Находки вердикта, уехавшие NOTE (merge не держали):** статусы задач 1/2/2b/4/5 в §Tasks
остались `⏳ OPEN` при исполненной работе (правка — carve-out dev'а/architect'а, не моя);
мутационный контроль tester'а сделан временной записью в `crates/gateway/src/lib.rs` с полным
откатом (эффект на репозиторий ноль — но пред-фиксная ревизия штатнее и ею гейт и закрыт);
`GS-I-5` не объявлен в `docs/fa/viz-backend.md` §5 (ранее названный пробел самого FA); висячая
ссылка на `A-028` унаследована от `C-225`.

**Почему milestone НЕ закрыт:** close-out (статус спеки, архивирование гейта, порядок в
роадмапе) — зона architect'а (`docs/04-workflow.md` §2, таблица разведения ролей).

## M-70 «полосы глубины: предел, метка на точку, единый словарь» — ✅ КОД В MAIN (merge PR #193 `a54c7d5` 2026-09-18, `R-187` APPROVED, круг 6) + **ДЕПЛОЙ ЗЕЛЁН, но §8 ПРОЙДЕН ЧАСТИЧНО**; milestone **НЕ ЗАКРЫТ — ЦЕЛЬ НЕ ДОСТИГНУТА: полосы НЕ ВКЛЮЧЕНЫ** (включены 2026-09-20 отдельным шагом — раздел «9-C» ниже; сам milestone закрывает architect)

**Что сделано (задачи 0-9, все `PASS`, гейт 42/0, exit=0 на дереве слияния `58ad3b6`).**

| закрыто | чем |
|---|---|
| `TD-198` — клиентское число полос ничем не ограничено ДО построения ответа | `MAX_BANDS = 32` в `gateway::validate_selector` (не в транспорте — иначе байпас для `research-cli`, чекпоинтера, replay), `DB-I-3` |
| `TD-159` — метка достоверности ОДНА на ряд точек разного качества | метка на ТОЧКУ: `series_provenance` параллельным массивом, снимается ТЕМ ЖЕ наблюдением книги; сторожа `DB-I-4c` (равенство длин) и `DB-I-4d` (связка против полного реплея) |
| `TD-161` — словарь метки рассогласован heatmap ↔ депт-серия | один `depth_provenance_label(band, side, reach_бакета)` на всю выдачу; приведение В СТОРОНУ депт-серии |
| форма выдачи объявлена | `GATEWAY_SCHEMA_VERSION` 9 → 10 (`VB-I-4`, аддитивность), sacred-пины согласованы задачей 6b |

**ЦЕЛЬ МИЛЕСТОУНА НЕ ДОСТИГНУТА, и это записано, а не умолчано.** Исходная цель — включить
канонический набор из семи полос (`П-014` п.4). **Задача 7 ПЕРЕОПРЕДЕЛЕНА rev12: прод-дефолт
остаётся УЗКИМ (`0.001`)**, включение вынесено founder'у (граница C — `gates.md` §7). Задача
7 сегодня — СТОРОЖ отложенного включения, а не включение. Проверено ревьюером рендером, а не
текстом: `docker compose config` даёт `GATEWAY_BANDS: "0.001"` — идентично `main`; прод несёт
`GATEWAY_BANDS=0.001`, переопределения в `.env` нет. **Подпись `П-014` п.4 НЕ ИСПОЛНЕНА.**
**Обновление 2026-09-20 (reviewer):** подпись ИСПОЛНЕНА — канонический набор включён
операторской строкой в host `.env`, прод отдаёт 14 рядов `depth_series`. Запись выше верна на
свою дату (2026-09-18) и оставлена летописью; действующее состояние — раздел «9-C» ниже.

**Деплой-гейт `gates.md` §8 в усиленной форме §5 спеки (пять пунктов) — исполнен ЧЕТЫРЬМЯ из
пяти; пятый недостижим по причине, которую сам гейт и нашёл.**

CI `35366837067` success · Deploy `35366836916` success (`Gate on CI` success, `Deploy (build
on VPS)` success) · прод на `a54c7d5`, оба контейнера `(healthy)`, журнал растёт
(`next_seq` 616 862 624 → 617 042 489).

| # | требование §5 | исход |
|---|---|---|
| 1 | `RssAnon` обоих контейнеров до и после (`TD-021`: НЕ `docker stats`) | ✅ `hft-gateway-serve` 396 → 404 КБ; `hft-recorder` 27 200 → 10 612 КБ. Построитель на пике холодной пересборки — 39 752 КБ, то есть `VB-I-10` (окно памяти) не нарушен |
| 2 | реальный размер кадра на проводе | ⛔ **5 052 685 Б против предела 2 000 000 Б — кадр отвергается fail-closed, клиент не получает НИЧЕГО.** Заведено `TD-206` (MAJOR). Мержем НЕ вызвано (диф не трогает прод-код VP и константу); доминирует volume profile — 194 641 бакет, `depth_rows=2` |
| 3 | sanity метки на свежих событиях | ⛔ **НЕ ИСПОЛНЕН — снимок не доставлен из-за п. 2.** Не «пропущен», а физически недостижим, пока `TD-206` открыт |
| 4 | **ВРЕМЯ до свежего `ckpt-*.bin` и сдвига `covered_through_seq`** | ✅ **≈22 мин 46 с** — числа не было ни у кого, включая бамп `M-68` |
| 5 | холодный пересбор назван ОЖИДАЕМЫМ, а не аварией | ✅ назван спекой ЗАРАНЕЕ (rev12.2), наблюдён ровно как предсказано |

**Пункт 4 — число, ради которого §5 переписывали.** Бамп 9 → 10 отверг живой слепок
(`read_and_validate` шаг (3), `crates/gateway/src/lib.rs:4198`), и построитель по расписанию
`*/15` пошёл ПОЛНЫМ реплеем:

```
16:30:19  старт (cron */15, flock)      журнал 67 ГБ / 321 сегмент, ~617 млн событий
16:53:05  свежий ckpt-b0f1ed89ec2ec142.bin, gw_v 9 → 10
16:53:06  covered_through_seq 616 918 363 → 617 422 142
          ≈ 22 мин 46 с        (инкрементальные прогоны до бампа — 1-2 с)
```

Цена подтверждена и со стороны ПОЛЬЗОВАТЕЛЯ: в окне пересборки зонд не получил ни одного
сообщения за 600 с и отвалился с текстом, который сам же этот сценарий и называет — «нет
сообщения от сервера за 600s — холодный чекпоинт?». После появления свежего слепка
подключение проходит за **1.9 с**. То есть цена бампа схемы = **≈23 минуты недоступности
кокпита**, и теперь она записана.

**Что гейт §8 нашёл сверх предмета — и это его главный результат.** Пункт 2 впервые в проекте
измерил размер кадра на проводе и обнаружил, что прод НЕ ОТДАЁТ данные вообще (`TD-206`,
MAJOR): предел `PL-I-5` переехан volume-profile'ом, растущим с историей. Три liveness-проверки
при этом зелены — контейнер здоров, heartbeat свеж, журнал растёт. Ровно класс `TD-011`/
`TD-031`, ради которого §8 требует содержательных пунктов сверх liveness.

**Открыто по предмету:** `TD-206` (MAJOR, кокпит не отдаёт кадр), `TD-207` (MAJOR, штатный
зонд §8 не аутентифицируется против прод-формы секрета), `TD-208` (MINOR, ролевая git-личность),
`TD-209` (замер, worktree с незапушенными коммитами исчезают). Подпись `П-014` п.4 (включение
семи полос) — за founder'ом; `TD-205` предметом `M-70` не является.

**Close-out милестоуна — за architect'ом** (`docs/04-workflow.md` §2): цель не достигнута,
решение о включении — граница C.

## 9-C «канонический набор из семи полос глубины ВКЛЮЧЁН на проде» (`П-014` п.4 — ✅ **ИСПОЛНЕНА 2026-09-20**) — включение операторской строкой, кода не менялось; **прод проверен reviewer'ом самостоятельно, не по отчёту**

**Что произошло.** `GATEWAY_BANDS` на проде переведён с `0.001` на канонический набор
`0.015,0.03,0.05,0.08,0.15,0.3,0.6` строкой в host `.env` (`/root/hft-platform/.env`,
режим 600, mtime 2026-09-20 19:43). Репозиторий не тронут: дефолт
`docker-compose.yml:147` остаётся `${GATEWAY_BANDS:-0.001}`, то есть сторож отложенного
решения (`verify_M-70.sh` task #7) продолжает судить ДЕФОЛТ и остаётся осмысленным.
Решение — граница C (`gates.md` §0.1, `П-014` п.4); разблокировано замером 9-B
(`docs/plans/depth-liveness-window-measurement-2026-09-20.md`, `b1ddbba`).

**Это первое исполнение подписи от 2026-08-17.** Раздел `M-70` ниже фиксировал обратное
(«`GATEWAY_BANDS` … переопределения в `.env` нет. Подпись `П-014` п.4 НЕ ИСПОЛНЕНА») — та
запись верна НА СВОЮ ДАТУ (2026-09-18) и оставлена как летопись; действующее состояние —
здесь.

### Замер reviewer'а — прод, а не пересказ

Все числа ниже сняты мной на живом проде 2026-09-20T20:19Z и повторно ≈20:35Z. Числа
founder'а в мандате (7 полос, 14 рядов, кадр ≈290 КБ, тёплый старт) **воспроизведены**, а не
приняты на слово; там, где мой замер разошёлся, написано моё число.

```text
$ ssh … 'cd /root/hft-platform && git rev-parse --short HEAD; docker ps --format "{{.Names}} {{.Status}}"'
46cd25c
hft-gateway-serve Up 34 minutes (healthy)
hft-recorder Up 20 hours (healthy)

$ ssh … 'docker inspect hft-gateway-serve … | grep -E "BANDS|MAX_RESPONSE|CHECKPOINT_DIR"'
GATEWAY_CHECKPOINT_DIR=/ckpt
GATEWAY_BANDS=0.015,0.03,0.05,0.08,0.15,0.3,0.6
GATEWAY_MAX_RESPONSE_BYTES=2000000

$ grep -n CHECKPOINT_BANDS /etc/cron.d/hft-journal-retention
37:CHECKPOINT_BANDS=0.015,0.03,0.05,0.08,0.15,0.3,0.6
$ grep -n BANDS /root/hft-platform/deploy/bin/gateway-checkpoint-cron.sh
47:CHECKPOINT_BANDS="${CHECKPOINT_BANDS:-0.001}"
92:  --bands "${CHECKPOINT_BANDS}"          # расписание */15, flock -n
```

**`46cd25c` — не отставание прода.** Разрыв до `main` (`4ddd392`) — тринадцать файлов, ВСЕ
документные (`git log --name-only 46cd25c..origin/main`); `deploy.yml` `paths:` содержит
только `crates/**`, `Cargo.*`, `Dockerfile`, `docker-compose.yml`, `.github/workflows/deploy.yml`,
поэтому docs-коммиты деплой не триггерят по конструкции. Прод несёт последнюю КОДОВУЮ
ревизию. CI `35532286938` success · Deploy `35532872608` success.

**Тёплый старт подтверждён дважды — чужим подключением и моим собственным:**

```text
19:44:40  snapshot-при-подключении построен events_decoded=8678  segments_opened=2 ckpt_dir_present=true   (sub=arch-9c)
20:19:36  snapshot-при-подключении построен events_decoded=25282 segments_opened=2 ckpt_dir_present=true   (sub=reviewer-9c)
$ ls -la /var/lib/docker/volumes/…-ckpt/_data
-rw-r--r-- 2825008 Sep 20 20:15 ckpt-8f69809dd707e8c9.bin     ← новая конфигурация
-rw-r--r-- 2793213 Sep 20 19:43 ckpt-b0f1ed89ec2ec142.bin     ← прежняя
-rw-r--r--       9 Sep 20 20:15 covered_through_seq
-rw-r--r--       9 Sep 20 19:43 covered_through_seq.warm7      ← прогрев ДО переключения
```

Простоя действительно не было: слепок под новый набор полос прогрет заранее (`*.warm7`,
19:43), и первое подключение после рестарта догоняло хвост в 8 678 событий, а не шло полным
реплеем. Цена холодного пути здесь НЕ уплачена — для сравнения, бамп схемы `M-70` стоил
≈23 минут недоступности (раздел `M-70` ниже, пункт 4).

### Выдача на проводе — прод-форма, селектор строит СЕРВЕР

Зонд `wsprobe` внутри прод-контейнера (клиент селектор не шлёт ⇒ измерена прод-конфигурация):

```text
"selector":{"venue":"Binance","symbol":"BTCUSDT","timeframe_ms":1000,
            "bands":[0.015,0.03,0.05,0.08,0.15,0.3,0.6],"window_ms":60000,"depth_cadence_ms":1000}
schema_version = 10 · depth_series = 14 рядов (7 полос × 2 стороны) · latency_first_snapshot_ms = 1781

кадр:   20:19Z — 289 896 Б = 0.145× предела 2 000 000
        20:35Z — 277 149 Б = 0.139× предела
состав (20:35Z): профиль 137 137 · heatmap 67 795 · depth_series 58 047 · bubbles 5 284 ·
                 ohlcv 5 115 · cob 1 007 · vwap 1 135 · cvd 1 093
отказов `exceeds limit` за 24 ч — 0 · RssAnon: gateway 13 324 kB, recorder 24 828 kB
heartbeat: ts_wall_ms 1789935606046 при `date -u +%s` 1789935613 ⇒ 7 с · next_seq растёт
           637 921 588 → 637 949 635 · сегмент 750 · свободно 56.8 ГБ (диск 64 %)
```

Заявленные founder'ом 290 212 Б моим замером **не воспроизведены буквально** и не могли быть:
кадр плавает с составом окна (289 896 / 277 149 на двух снятиях за 15 минут). Порядок и вывод
те же — это 0.14× предела, а не 0.145 «ровно».

**`VB-I-5` (`docs/fa/viz-backend.md:203`) — живой инвариант тронутого модуля — предъявлен
ПРОДОМ, а не тестом:** у всех 14 рядов `series_provenance` имеет длину 60 при 60 точках
`series`, то есть метка принадлежит ТОЧКЕ и сторож равенства длин (`DB-I-4c`) держится на
прод-данных. Словарь меток совпадает с таблицей `crates/gateway/src/lib.rs:2453-2485`
(`bid → confirmed`, `ask → unconfirmed`, `band > reach → not-observed`).

### Что включение показало сверх ожидаемого — три находки, все заведены долгом

| # | находка | замер | куда |
|---|---|---|---|
| F-1 | депт-серия весит **58 047 Б**, из них **35 534 Б (61 %)** — повторяющийся литерал метки (60 одинаковых строк на ряд). Оценка `П-027` «депт-серия ≈25 КБ» занижена в **2.3×**, а на неё опирается ёмкостный расчёт `DESIGN` §16 | `json.dumps` по `depth_series` прод-кадра: `series` 21 554 Б, `series_provenance` 35 534 Б | `TD-215` (MINOR) |
| F-2 | полоса **60 %** отдаёт число, которое НИКОГДА не наблюдено: `not-observed band=0.600000 reach=0.593…/0.597…` — **120 точек из 120** на двух снятиях с интервалом 15 мин, обе стороны. Седьмая полоса канонического набора при текущем охвате книги неверифицируема в принципе | `series_provenance` обоих рядов `0.6` | `TD-216` (MINOR) |
| F-3 | `TD-197` («окно heatmap не объявлено в кадре») стоял с оговоркой «пока `GATEWAY_HEATMAP_WINDOW=0.001` равен `GATEWAY_BANDS=0.001`, долг ненаблюдаем». **Предпосылка отпала сегодня:** окно осталось `0.001`, полосы ушли до `0.6` — потребитель видит карту и полосы в РАЗНЫХ охватах и не может это различить | `GATEWAY_HEATMAP_WINDOW=0.001` в том же `docker inspect` | `TD-197` уточнён |

Плюс принятая цена, названная замером 9-B и включением: словарь метки не различает
«наблюдено полно внутри REST-охвата» и «накоплено диффами с момента ресинка» — до **1 мин
45 с** недостоверной метки на промежуточных полосах после ресинка, вне ресинка **0 точек из
1153**. Починка требует смены формы выдачи (bump схемы ⇒ ≈23 мин простоя), поэтому заведена
долгом `TD-214` и делается вместе со следующим бампом, а не отдельно.

**Что НЕ утверждается.** Два снятия по 60 точек — не суточное наблюдение: устойчивость
`not-observed` на `0.6` показана на 15-минутном интервале, суточная динамика охвата не
мерена. Ресинк в окне моих замеров не происходил, значит окно `TD-214` я наблюдал не
собственным прогоном, а по замеру 9-B.

## M-88 «снятая ликвидность исчезает и у клиента: контракт обновления книго-зависимых серий» (`P0-CORR`) — ✅ **МИЛЕСТОУН ЗАКРЫТ 2026-09-23** (`R-198`, close-out завершён); код в `main` (`R-197` APPROVED, круг 2), гейт и спека вынесены в `docs/archive/` по норме `Р-2`; **предмет `P0-CORR` этим НЕ закрыт — открыт `TD-217` MAJOR, и его закрытие лежит ЗА границей этого репозитория**

**ЗАКРЫТОСТЬ — СУЖДЕНИЕ REVIEWER'А, И ВОТ ЕГО ОСНОВАНИЕ** (`04-workflow.md` Р-2, строка 1 таблицы: «назвать, какие милестоуны ЗАКРЫТЫ» — зона reviewer'а; машинно закрытость не выводится, `R-098` §3.1 — три ревизии критерия дали три разных ответа). Закрытым называется МИЛЕСТОУН, а не дефект: у `M-88` исполнены все задачи §Tasks, гейт прогнан приёмкой (17 PASS / 0 FAIL / exit=0, `R-197` §4), прод проверен глазами, обе половины close-out'а состоялись — архивная (architect, PR #218) и настоящая запись (reviewer, `R-198`). **Милестоун и предмет роадмапа здесь НЕ СОВПАДАЮТ, и это не дефект учёта, а факт границы:** `M-88` чинит серверную половину и заканчивается ею, а строка `P0-CORR` описывает то, что видит ПОЛЬЗОВАТЕЛЬ. Пользователя обслуживает потребитель, которого в этом репозитории нет (founder, 2026-09-23: фронт — не предмет этого репозитория, появится позже и в другом месте). Объявить закрытым милестоун — можно и нужно; объявить вылеченным дефект, предъявить который не на чем, — нельзя.

**Предмет.** `merge_heatmap` объединял ячейки карты по ключу `(time_s, side, price_e8)`, а путь
полного пересчёта отбрасывал `size <= 0` — снятый уровень оставался у клиента НАВСЕГДА, и живое
расходилось с пересчитанным (регресс `VB-I-2`). Шесть кругов гейта по глубине этого не поймали:
пересечение множеств {тесты со склейкой кадров} × {тесты со снятием уровня на входе} × {тесты с
ассертом по карте} было ПУСТО.

**Что сделано (восемь участков, а не два).** К слиянию и источнику добавились сборка кадра из
наблюдений (`book_series_in`) и отсутствие сверки курсора в `Snapshot::apply`:

- карта — `Replace` по ВРЕМЕННОЙ КОЛОНКЕ: `SeriesBundle.heatmap_observed_time_s` объявляет,
  какие колонки наблюдены; объявленная колонка заменяется ЦЕЛИКОМ, включая пустую;
- стакан — `Replace` по явному признаку `SeriesBundle.cob_observed`: «наблюдали пустоту»
  перестало быть неотличимым от «не наблюдали»;
- курсор — `Snapshot::apply` возвращает `#[must_use] ApplyOutcome` с тремя исходами;
  `frame.from != cursor` ⇒ `OutOfOrder` БЕЗ изменения состояния (ранние `return` стоят ДО
  любых мутаций). Повторный кадр больше не удваивает суммируемые ряды, запоздалый не
  откатывает курсор;
- полосы глубины — правило не менялось (§4);
- форма провода: два аддитивных поля, `GATEWAY_SCHEMA_VERSION` **10→11**; contract-RFC не
  требуется (T-designate, `05` §2).

**Круг 2 (`R-195` → `R-197`).** PR-time гейт круга 1 вернул REJECT не по предмету, а по ЦЕНЕ:
новое состояние редьюсера `heatmap_buckets_observed` росло с ИСТОРИЕЙ журнала, а не с окном —
на прод-глубине снимок не отдавался ВООБЩЕ (`PL-I-5`: `limit=2000000 observed=2423152`, все
содержательные счётчики нулевые). Прямой регресс `VB-I-10`. Закрыто: тот же предикат
`retain(|&t| t >= lo_time_s)` в двух местах, где уже эвиктится карта — `Reducer::evict_window_state`
(`crates/gateway/src/lib.rs:1278`) и `evict_series_bundle_under_window` (`:3176`). Симметрия
сделана СВОЙСТВОМ КОНСТРУКЦИИ: односторонняя починка красит сторож `W2`.

**Оракулы.** `red_m88_update_contract` (14), `red_m88_contract_form` (9),
`red_m88_golden_vectors` (2, одиннадцать эталонных фикстур), `red_m88_observed_window`
(`W1`–`W4`), `red_m88_observed_response_limit` (ресурсный). Канарейка `W4` разбирает
`SeriesBundle` ИСЧЕРПЫВАЮЩЕ, без `..`: следующее поле бандла не скомпилируется, пока автор
не отнесёт его к оконным или неоконным явно (приём `CT-I-*` для `EventKind`).

**Приёмка (прогон reviewer'а, `R-197` §4):** `bash docs/archive/verify_M-88.sh` (на момент прогона — `scripts/verify_M-88.sh`; вынесен в архив 2026-09-23 нормой `Р-2`) → **17 PASS / 0 FAIL /
`VERDICT: PASS` / exit=0**. Мутационный контроль исполнен reviewer'ом трижды, включая обратный
вопрос «что ослаблено рядом».

**Ревизия чужих оракулов (задача 9).** `#[must_use]` сделал незаконными 18 вызовов `.apply(` в
12 оракулах. Ни одно ожидание НЕ УДАЛЕНО — каждый вызов усилен до
`assert_eq!(.., ApplyOutcome::Applied)`; перечень `red_gateway_bounded` дополнен полем
`heatmap_observed_time_s`. Решения — `docs/archive/M-88-liquidity-removal-contract.md` §14.1.

**ЧТО НЕ СДЕЛАНО И ПОЧЕМУ ЭТО ЗАПИСАНО, А НЕ УМОЛЧАНО.** Контракт применяет КЛИЕНТ, а фронт
(`code2alpha`) живёт в другом репозитории — прогон эталонных векторов против него НЕ
выполнялся. Отсрочка санкционирована арбитром (`A-036` §4.3) на четырёх условиях, все
исполнены или назначены. Следствия:

- **`TD-217` «built-not-wired», severity MAJOR** — заведён этим close-out'ом;
- **строка `P0-CORR` в `docs/ROADMAP.md` ОСТАВЛЕНА ОТКРЫТОЙ** (`A-036` §4.3 п.3 — замок на
  сквозной приёмке; решение reviewer'а на close-out — `R-198` §6): merge закрывает серверную
  половину, а не предмет. **Условие закрытия ЗАМЕНЕНО**, потому что прежнее («прогнать
  эталонные векторы против фронта») в этом репозитории неисполнимо в принципе — у него здесь
  нет ни исполнителя, ни предмета, а невыполнимое требование не исполняется, оно заменяется
  догадкой (`CLAUDE.md`). Действующее условие — появление ПЕРВОГО потребителя, гейтящего на
  `schema_version == 11`, и его сходимость на 7+4 эталонных векторах; владелец — founder.
  До этого момента строка не порождает задач НИ ОДНОЙ роли здесь, и это записано в самой
  строке, чтобы она не читалась как невыполненная работа;
- потребитель ОБЯЗАН гейтить на `schema_version == 11` — иначе `cob_observed` прочитается
  `false` и стакан замрёт МОЛЧА.

**Названные пределы поставки (спека §11):** «уровень снят» и «уровень вышел из окна» для
клиента неразличимы (свойство оконной конструкции); внутри одной секунды история не
восстановима; масштаб накопления призраков за час НЕ МЕРЕН; `Invalidated` не вводится
(до закрытия `TD-159`); стоимость прогрева слепка на форме v11 не измерена.

**Открытые замечания `R-197` (не блокеры, зона architect):** колонка Status задач 10-11
осталась `⏳ OPEN`; батарея мутантов `R4` не покрывает новую эвикцию (кандидат `МУТАНТ-В`);
док-комментарий скаляра `cob_observed` обещает окно, которого у него нет (флаг никогда не
сбрасывается — безопасно по существу, неточно по тексту).
