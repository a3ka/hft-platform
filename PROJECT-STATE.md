...
| **509** | **2742 / 3555 / 1834** | 20.03 MiB |

**Решающая точка — третья:** backlog практически НОЛЬ (509 событий, снято через 18 s после
чекпоинта), а подключение всё равно стоит **1.8–3.6 s**. Разброс внутри одной точки
перекрывает разницу между точками ⇒ зависимости от backlog'а в данных нет вовсе.
