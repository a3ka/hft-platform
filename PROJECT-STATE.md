...
## Аудит воркфлоу einhard vs наш (`docs/workflow-audit` — ⛔ REJECT 2026-08-05, `R-034` §A; в `main` НЕ вошёл)
- Ветка `docs/workflow-audit` (`08ef175`, +442) **не смержена.** Блокер `F-034-1`: строка §1.1:50
  «Лаунчеры/обёртки: нет» ложна на базе самого аудита — `.claude/wrappers/` содержит 8 файлов
  (`pi-dev.sh` 10 KB исполняемый, «адаптация einhard W1-W3», `dispatch-mandate.md`, 5 ролевых
  симлинков), коммиты `19f1c4c`/`39591f4`. Строка несущая: из неё выведены §5 Г2, §7-5 и
  ранжирование §6 (оно сортирует по ЦЕНЕ ВНЕДРЕНИЯ, которая зависит от наличия носителя).
- **Ошибка успела распространиться:** `milestones/M-60-mechanisms.md:70` унаследовал «у нас нет
  носителя» и на этом основании вычёркивает лаунчер из пространства решений M-60.
- Корпус остальных замеров аудита проверен reviewer'ом и **устоял точно** (949 строк правил,
  92/7.8 KB `CLAUDE.md`, 440/43 KB профилей, 41 verify-скрипт, 4 пробы, размеры ledger'ов, Г3) —
  реджект по одной строке, не по качеству работы.
- Следствие: `F-064-1` (M-60 стоит на замерах вне проверяемой цепочки) **остаётся открытым** —
  лечится тем же кругом после исправления документа.
