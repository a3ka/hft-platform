...
## M-88 «снятая ликвидность исчезает и у клиента: контракт обновления книго-зависимых серий» (`P0-CORR`) — ✅ КОД В MAIN (`R-197` APPROVED, круг 2); **`P0-CORR` НЕ ЗАКРЫТ — открыт `TD-217` MAJOR**
...
**Приёмка (прогон reviewer'а, `R-197` §4):** `bash scripts/verify_M-88.sh` → **17 PASS / 0 FAIL /
...
`heatmap_observed_time_s`. Решения — `milestones/M-88-*.md` §14.1.
...
- **строка `P0-CORR` в `docs/ROADMAP.md` НЕ ЗАКРЫТА** (`A-036` §4.3 п.3): merge закрывает
  серверную половину, а не предмет. Закрывает его прогон векторов против фронта;
