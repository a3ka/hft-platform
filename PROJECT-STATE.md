...
## M-49 — честность `next_seq` при нечитаемом сжатом хвосте (TD-049) — ⚠ НЕ ЗАКРЫТ, PR-гейт REJECTED (`feat/M-49` @ `b4e5995`, в `main` НЕ мержен)

Цель — закрыть TD-049 (M-40 PR-гейт, `.zst`-ветка `tail_last_seq_of` фейлит `open_with` на битом
хвосте там, где сырая ветка терпит и ресинкается). architect выдал RED-набор + спеку
(`milestones/M-49-journal-tail-integrity.md`), engine-dev реализовал 8 задач (операторская
декларация `journal.force-next-seq.json`, runbook `docs/ops-journal-tail-unreadable.md`, честный
`seg_ts` в `print_plan` — попутно закрывал бы TD-051/TD-050).

**Reviewer REJECT.** Реализация ослабила `segments()` — строгий прод-путь чтения журнала
(`journal::stream`/`list_segments`) стал МОЛЧА пропускать повреждённый хвостовой сегмент вместо
отказа. Это «тихая ложь в данных»: потребитель прод-пути (research, бэктест) получил бы журнал с
дырой без единой ошибки — именно тот класс дефекта, от которого `stream`/`list_segments`
защищают по конструкции fail-closed.
...
толерантности от fail-closed прод-пути чтения, а не от отдельного диагностического пути.
Architect готовит rev2: `segments()` возвращается к строгому виду; «максимальный читаемый seq»
для операторской диагностики берётся ОТДЕЛЬНЫМ путём (`iter_segments_sorted`/`recover`-класс);
оракулы переписываются под это разделение.

**Переоценка TD-049: MINOR → CRITICAL по последствию** (подробности — `TECH-DEBT.md`, запись
TD-049, UPDATE 2026-07-30). Замер на фикстуре: наивная толерантность к битому хвосту даёт
перекрытие **342 события** между переиспользованным диапазоном `seq` и оригиналом — необратимый
seq-reuse при restore из холодного хранилища (тот же класс, что M-40 закрывал как R2b), не
«остановка сбора», отсюда переоценка вверх.

**TD-050 и TD-051 остаются OPEN**, прицепом к M-49 (попытка фикса жила в том же REJECTED дифе,
`0fe38b7`, но весь `feat/M-49` не мержен — фикс не приземлился). rev2 architect'а переиграет обе
задачи заново на исправленной спеке.
