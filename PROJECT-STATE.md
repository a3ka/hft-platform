...
## Байтовый курсор хвоста (M-57 «tail-follower» — ⚠️ MERGED `710b1ad`, reviewer APPROVED 2026-08-07 `R-040`, круг 3 — **MILESTONE НЕ ЗАКРЫТ: на проде механизма НЕТ**)

**Читать вместе с `TD-115`.** Merge состоялся, но это НЕ close-out. Ниже разделено то, что
доказано, и то, что НЕ проверено — смешивать их и есть способ, которым milestone «закрывают»
поверх неработающего прода.
...
- **Цена вопроса, замеренная на живом проде ДО merge** (`R-043` §B, мера — `/proc/<pid>/stat`,
  не `docker stats`; CPU 0.0 % перед каждой точкой):

  | клиентов | CPU `gateway-serve` | RssAnon |
  |---|---|---|
  | 0 | 0.0 % | 307 268 kB |
  | 1 | **≈52 %** | — |
  | 5 | ≈261 % (= 5 × 52, линейно) | 354 668 kB |
  | 20 | ≈397 % — **потолок 4 ядер** | 465 352 kB |

  **Один зритель стоит половину ядра. Точка насыщения прод-машины — ≈7.7 сессии.** Не
  «десятки тысяч» из требования founder'а, а восемь. Это и есть предмет M-57.
...
- **ЧЕГО НЕТ — и почему milestone остаётся OPEN.** Deploy отработал fail-closed и НЕ
  задеплоил: `Gate on CI` → `failure` (steps=0), `Deploy (build on VPS)` → **SKIPPED**.
  Прод остался на `dca889a`; `grep stream_from_at` на VPS → **0**. Замер «после» снять
  физически не с чего. **Milestone закрывается ТОЛЬКО замером против таблицы предсказания
  `R-043` §C** (зарегистрирована ДО merge отдельным коммитом `3e2a1ac`, что проверяется
  `git merge-base --is-ancestor`). Три круга гейтов уже были зелёными при неработающем на
  проде механизме — это ровно тот урок, который здесь и применяется.
- **Прод на момент close-out:** оба контейнера `healthy` (`Up 41 h`), heartbeat свежий (8 с),
  журнал растёт (`next_seq` 189 351 026, сегмент 207), CPU 0.0 %, load 0.22, свободно 68 GB.
  Прод не тронут и не пострадал.
- **Открыто после merge:** `TD-115` (M-57 в `main`, но не на проде + первый деплой без
  свидетеля — CRITICAL), `TD-109` (слепые меры работы), `TD-116` (`hint.pos` без валидации),
  `TD-117` (три stateless-вызова `stream_from` без оракула), `TD-118` (`wsprobe` течёт
  сессиями — бьёт по достоверности прошлых замеров), `TD-119` (гейт не отличает «код
  красный» от ENOSPC; 37 worktree / 370 GB), `TD-120` (метаданные-путь `O(#сегментов)` на
  тик, `A-004` §C.2), `TD-121` (комментарий guard'а `JR-I-11` недосчитывает пути).
- **Цепочка гейтов:** `R-035` (REJECT) → `R-039` (REJECT) → `R-040` (APPROVED) · критики
  `C-059`/`C-060`/`C-063` · арбитр `A-004` · close-out `R-043`. RISK-BLOCK не применялся:
