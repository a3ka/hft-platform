...
## M-86 «профиль объёма отдаётся на ФИКСИРОВАННОЙ ценовой сетке» (`TD-206`) — ✅ КОД В MAIN (merge PR #198 `46cd25c` 2026-09-20, `R-188` APPROVED) + **В ПРОДЕ, §8 ПРОЙДЕН ЧАСТИЧНО 2026-09-20T00:07Z**; milestone **НЕ ЗАКРЫТ**
...
**Чего замер НЕ доказал — названо прямо.** UTC-сутки на момент замера шли 7 минут
(`session_id=20716`, ход 50.75 USD, 167 корзин), поэтому кадр был бы мал и БЕЗ `M-86`. Полного
дневного кадра под пределом пока не видел никто; проекция (20 417 корзин на суточный ход
5 104 USD ⇒ профиль ≈ 487 КБ) — арифметика, а не измерение. Поэтому `TD-206` помечен
**ЧАСТИЧНО ЗАКРЫТ**, а не закрыт: условие полного закрытия — повторный зонд на зрелой сессии.

**Долг, объявленный этим merge'ем (спека §8, «пределы, названные честно»):** `TD-210`
(гарантии предела нет — шаг отодвинул границу, MAJOR), `TD-211` (ширина одна на процесс и под
BTCUSDT, MINOR), `TD-212` (Value Area плохо определена — предшествует `M-86`, MAJOR), `TD-213`
(состояние остаётся тиковым, MINOR).

**Что осталось за architect'ом до закрытия милестоуна** (находки `R-188` §7, merge не держали):
1. `scripts/lib/mutation_gate.sh` не назван в `Allowed paths` §5 спеки — тот же класс, что
   `C-229` R6 в этом же предмете; полномочие есть (`A-035` §1, `scope-guard.md`), отражения нет.
2. Коммит ремонта гейта (`2b0cfd3`) не нёс `ALLOW-SUBJECT-CHANGE` — цепочка узнала о локе в
   самом дорогом месте, на merge'е.
3. Шапка спеки всё ещё «🚧 PLANNED (plan-time гейт не пройден)», задачи 1, 2, 4, 5, 7 — `⏳ OPEN`
   при реализованном коде и зелёном гейте.
4. Задача #3 своего коммита не имеет (внутри `333942f` вместе с #2) — по существу неотделима,
   зафиксировано как NOTE.
