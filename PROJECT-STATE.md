...
**Чем предъявлено, а не чем устроено.** RED-набор `scripts/tests/red_artifact_ids.sh` — 47
сценариев по семи осям + батарея из 25 мутантов (`BATTERY: PASS (27/27)`), состав сверяется
...
Три NOTE вердикта `R-063` (непокрытая оракулом привязка к `.md`; неверное «побайтово» в шапке
`TD-PARSER-BLOCK`; мёртвая константа `TD_PAT`) — за architect'ом.
