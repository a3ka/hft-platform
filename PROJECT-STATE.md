...
а добор упавшего деплоя остаётся ручным. Открытый долг — `TD-142`; `П-008` п.3 (пуш-деплой vs
...
**Отпочковано, а не умолчано:** `TD-144` — инвариант «цена такта не зависит от `N`» на проде
НЕПРОВЕРЯЕМ (счётчик `segment_meta_ops` не экспортируется; замер был вынужден идти косвенно
через `/proc/<pid>/io`, а тот не видит чистые `stat`/`getdents`); `TD-145` — `RssAnon` не
