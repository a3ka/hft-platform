...
| L2D-I-8 | **Rollback-isolation по SCHEMA-ЭПОХЕ (C-018 + TD-031 rev2):** L2Delta изолирован в сегменте schema-3 — `decide_open_segment` reuse требует `header.schema_version == SCHEMA_VERSION`, поэтому schema-2 сегмент НЕ получает variant-6 ДАЖЕ при ИДЕНТИЧНОМ (прод-константа) provenance. ⚠ rev1 держал изоляцию на provenance (git-sha) — ВОИД в контейнере (git-константа `no-git-info`, TD-031). RED `red_l2delta_rollback_boundary` переписан прод-реалистично (падал на provenance-only reuse). Schema-forward ОДНОСТОРОННИЙ: silent-откат запрещён; post-деплой данные — терминальный архив, re-stitch ЗАПРЕЩЁН; fix-forward. Runbook `ops.md` §5.1 |
