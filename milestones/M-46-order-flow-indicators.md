...
## Описания (продуктовая спека — founder, BINDING)
- **Liquidity tracker** — отслеживает, как меняется стоящий объём на уровне: сколько добавлено и сколько
  снято за интервал. Отвечает на вопрос «ликвидность наращивают или убирают перед подходом цены». Обычно
  рисуется как add/pull-дельта поверх хитмапа или отдельной колонкой.
- **Time & Sales** — сырая лента сделок: время, цена, объём, сторона агрессора. Панель справа, покупки/
  продажи цветом, крупные принты подсвечены. Единственный из четырёх — не аналитика, а сырые данные.
- **Spoofing radar** — ищет заявки, которые поставили и сняли, не дав исполниться: фальшивые стены,
  которыми давят цену. Помечает уровни, где крупный объём появился и исчез за короткое окно, а цена туда
  так и не дошла.
- **Refill / hidden size** — айсберг с другой стороны: по уровню бьют, а отображаемый объём каждый раз
  восстанавливается → за видимой котировкой скрытый резерв. Показывает накопленный поглощённый объём
  против отображаемого.
