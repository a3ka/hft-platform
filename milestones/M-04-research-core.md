...
прогон OBI и отчёт `R-001` идут ТОЛЬКО на strategy-пайплайне M-07 (`sim::StrategyBacktest` +
`DirectionalStrategy`) — ad-hoc harness, которым мерили пилот, УДАЛЁН, и его результаты
описывают несуществующую логику. Обязательные условия валидности R-001:
...
5. `research/reports/` в репозитории ещё НЕ существует — R-001 создаётся вместе с каталогом
