...
## 10. Остаток после фикса — названо dev'ом честно, оценено architect'ом

`research/reports/M-53-engine-dev-report.md` §5. Ни один пункт не блокирует закрытие P0,
но все три обязаны быть записаны, а не раствориться в «работает же».

1. **Гонка снапшот↔live-редьюсер устранена АРХИТЕКТУРНО, но без оракула.** Оба шага берут
   курсор из одного `live.cursor()`, поэтому разъехаться не могут *по построению*. Дев прямо
   отмечает, что теста нет, и указывает точку врезки (`run_authorized_session`, шаг 6a) —
   правильное поведение: тест-файлы sacred, он их не пишет.
   **Оценка architect'а:** это ровно тот класс, который я весь milestone и критикую —
   свойство, обеспеченное конструкцией, а не проверкой. Конструкция может быть переписана
   завтра, и тогда никто не заметит. → **в `TECH-DEBT.md`** (зона reviewer'а), приоритет
   ниже P0: закрывается оракулом, который продвигает чекпоинт МЕЖДУ шагом резюме и шагом
   снапшота.
2. **Двойная стоимость на connect.** `resume()`+догон считает vwap-совместимое состояние
   ДВАЖДЫ на подключение (для `live` и внутри `snapshot_from_checkpoint`). Ограничено
   backlog'ом с последнего чекпоинта (НЕ `O(история)`) и происходит один раз на коннект,
   не на тик — то есть предмет TD-083 не затрагивает.
   **Следствие для прогона:** латентность ПЕРВОГО снапшота может вырасти против 8.7 s.
   Это ожидаемо и не является регрессом TD-083; но если вырастет КРАТНО — находка.
   Усугубляется `TD-085` (чекпоинт обновляется раз в сутки ⇒ backlog к вечеру миллионы).
3. **`serve::frames_msgs` больше не используется в проде**, но остаётся в коде — на неё
   завязан sacred-тест `red_serve_passthrough.rs`. Удалять нельзя, мёртвой не считается:
   это эталон passthrough-семантики.

## 11. Чек-лист ФИНАЛЬНОГО прогона против прода (закрытие milestone'а)

Гейт на фикстурах необходим, но не достаточен — дефект родом с прода.

| # | Что снять | Ожидание |
|---|---|---|
| 1 | `frames_received` за 15 s | **> 0** (было 0 — главный признак TD-083) |
| 2 | CPU `gateway-serve` через 5 мин после ухода клиента | норма (было 100.26%) |
| 3 | `CLOSE_WAIT` после ухода клиента | **0** (было 10 за 4 минуты) |
| 4 | ВТОРОЙ клиент после первого | подключается (было connect-timeout ×2) |
| 5 | латентность первого `Snapshot` | зафиксировать; рост против 8.7 s объясним §10.2, КРАТНЫЙ рост — находка |
| 6 | байтовый размер `Snapshot` и максимального `Frame` | зафиксировать (`WebSocketConfig` не задан — дефолты `tungstenite`) |
| 7 | осмысленность цифр | биды ниже асков, VWAP около рынка, CVD не абсурден |
