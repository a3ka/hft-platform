...
> ## ⛔ ВЕРДИКТ ЭТОГО MILESTONE'А SUSPENDED (2026-08-03)
>
> Вывод «30–60% ЖИВАЯ-РАЗРЕЖЕННАЯ» — это ОДНО сравнение `cancel_fraction` между полосами, а
> величина оказалась насыщающейся: она зависит от длины окна и плотности сетки, а не только от
> живости (дефект метрики `R-031` §A.1, `depth_lifetime.rs:155-196`). Поэтому и само число `bid=0.622`, и монотонный
> тренд `0.867→0.796→0.622`, и вывод «не замёрзшая у потолка» — **не подтверждены**.
> Пре-регистрированный founder-флаг (падение >50% ПО ОБЕ стороны) от этого тоже не защищает:
> он сравнивает те же насыщающиеся величины.
>
> Статус: `verification pending` до переснятия per-life метрикой (M-58). Действует замок
> `research/arbitration/A-002-depth-metric-tpp.md`. Работа milestone'а (расширение
> `BANDS_BPS` до `[3000,6000)`, прогон на segment 78) корректна и не отзывается — отозвана
> ИНТЕРПРЕТАЦИЯ полученных чисел.
