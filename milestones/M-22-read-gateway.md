...
STATUS: **PROPOSED** (2026-07-22, architect). Пивот P-COCKPIT, Трек B (MVP-1), критический путь.
...
- **Live-механизм:** `journal::stream` перечисляет сегменты ОДИН раз и завершается на текущем хвосте
  (`finished=true`). Follow строится в gateway: `frames_since(from_cursor)` заново открывает stream и
  отдаёт события с `seq > from_cursor`, свёрнутые тем же редьюсером, что snapshot/replay. Отсюда
  **live == replay архитектурно** (один код, разный источник хвоста), а не как проверка постфактум.
...
| **GW-I-2** (VB-I-2/NOTE-2) | **Bounded-memory прод-масштаб.** `snapshot`/`frames_since` над journal — память O(1) по размеру журнала: пик(64 MiB) − пик(16 MiB) < 1 MiB И абсолютный пик < бюджет на 64 MiB (ловит `read_all`). Анти-плацебо: контрольный `read_all` того же журнала ПРЕВЫШАЕТ бюджет (иначе оракул мёртв). Канарейка: нет `read_all`/`Vec<Event>`-материализации в `gateway/src`. | `red_gateway_bounded.rs` + verify grep |
...
| 3 | ⏳ | `snapshot(dir, EpochFilter, sel)` — полная свёртка серий через `journal::stream` (bounded) | engine-dev | GW-I-1/GW-I-2/GW-I-6 GREEN |
| 4 | ⏳ | `frames_since(from_cursor)` + `replay(window)` — инкрементальный/детерм. хвост тем же редьюсером | engine-dev | GW-I-3/GW-I-4 GREEN |
