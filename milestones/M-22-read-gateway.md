...
| **GW-I-7** (CT-RFC02-2) | **EpochFilter соблюдён на ВСЕХ путях — эпохи не смешиваются молча.** На СМЕШАННОМ журнале (OwnCapture+Vendor+Synthetic) фильтр соблюдён в `snapshot` И `frames_since` И `replay`: own-путь (только own) ≠ all-путь (по CVD и по кадрам). `Explicit([own,vendor])` отличается И от `OwnCaptureOnly`, И от `All`. Анти-плацебо: impl, игнорирующий фильтр в любом пути (хардкод `All`) → own==all → падение. Канарейка stream — по КОДУ (call-form, комментарии вырезаны), не по doc. | `red_gateway_epoch_filter.rs` (snapshot+frames_since+replay+explicit) + verify grep |
| **GW-I-8** | **Cursor/Frame-bounds + max_events-кап.** `snapshot(at).cursor` отражает окно (`LATEST`→`Some(last_seq)`, `START`→`START`+пустая серия); `frames_since(after, max)` возвращает НОВЫЙ курсор = последний свёрнутый `seq`; кадры контигуальны (первый `.from==after`, `f[i].to==f[i+1].from`, последний `.to==` курсор); **`max_events` — РЕАЛЬНЫЙ кап:** при N>max первый вызов покрывает ≤max событий и оставляет хвост. Анти-плацебо: неверный/игнорируемый курсор, дыры between-frame, ИЛИ весь-tail-одним-батчем → падение. | `red_gateway_live_eq_replay.rs::{cursor_and_frame_bounds, frames_since_respects_max_events_cap}` |
