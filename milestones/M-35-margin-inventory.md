...
| 2b | ⏳ | **Exhaustive-match ревизия (engine-dev):** новый вариант `MarginInventory` (дискр.7) ломает исчерпывающие `match &md.payload` БЕЗ `_=>` → `cargo build --workspace` E0004. Обновить: `crates/journal/src/segments.rs:~1523` (арм `\| MarginInventory { ts_exch_ms, .. } => *ts_exch_ms`, как Funding/OI/MarginRate/L2Delta), `crates/sim/src/exchange.rs:~223` (арм-`{}` как прочие md-not-relevant), **`crates/journal/examples/dump.rs`** (dev-дампер: арм `\| MarginInventory { .. } => {}` — `verify` гоняет `clippy --all-targets`, examples тоже компилируются). Оракул = `cargo build --workspace` GREEN (+ `clippy --all-targets`). | engine-dev | workspace + examples компилируются; арм семантически как соседние |
...
| 2e | ⏳ | **Recorder spawn wiring (engine-dev) — §8 выявил дыру:** `run_margin_inventory(tx)` ГОТОВ в venue-binance (poll-loop + rate-limit + read-key), но `crates/recorder/src/main.rs` его НЕ спавнит → коллектор дремлет (0 событий на проде, ключ есть, auth-ошибок 0). Добавить `tokio::spawn(venue_binance::run_margin_inventory(<writer_tx>.clone()))` рядом со спавном venue-эмиттеров (~390; сигнатура иная — только tx, без symbols). | engine-dev | на проде §8: MarginInventory USDT/USDC появляются в журнале (decode, НЕ grep — postcard по дискриминанту) |
| 3 | ⏳ | **§8 eyes-on** (после 2e): деплой → журнал несёт `MarginInventory` usdt/usdc (DECODE, не grep binary!), recorder healthy, hb свежий, 0 auth-ошибок; sanity `available_e8` ≈ live REST (~USDT 21.6M/USDC 20.9M ×1e8) | reviewer | decode свежего сегмента: MarginInventory для USDT+USDC ≥1 за интервал; прод healthy |
...
- **engine-dev (exhaustive-match ревизия, task 2b/2d):** `crates/{journal,sim,recorder}/src/**` + `crates/journal/examples/dump.rs` (dev-дампер — арм нового MdPayload-варианта; RN-18: exhaustive-match покрывает ВСЕ таргеты `clippy --all-targets`, не только `src/**`).
- **research-dev (task 2c):** `crates/research-cli/src/bin/latency_probe.rs` (арм-`continue`).
