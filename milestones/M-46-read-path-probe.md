...
**В close-out (чек-лист, дополнен по `C-055` §5):**
1. сырой дамп первого `Snapshot` (усечённо) и латентность до него;
2. длины всех десяти серий;
3. путь к HTML-артефакту;
4. **байтовый размер `Snapshot` и самого большого `Frame`** — сервер вызывает
   `accept_hdr_async` БЕЗ явного `WebSocketConfig` (ни `max_message_size`, ни конфига в
   `gateway-serve/src/lib.rs` — проверено грепом), значит действуют дефолты `tungstenite`
   (десятки MiB). На фикстурах это не проверяется в принципе; **sidecar-прогон — единственное
   место, где виден реальный масштаб** (прод: `heatmap len=1697`). Если размер приблизится к
   лимиту хотя бы на порядок — это находка, а не «просто цифра в отчёте».

**Вне периметра M-46 (решение `C-055` §5, зафиксировано осознанно, не забыто):**
slow-consumer (зависший клиент копит незакрытые таски — вопрос ресурсов/DoS, не корректности
серий), одновременные подключения (M-46 про ОДИН поток данных, не про ёмкость),
многополосный `Selector` (`bands` длиннее одного элемента). Первые два — кандидаты в
`TECH-DEBT.md` (зона reviewer'а), передать на PR-гейте.
