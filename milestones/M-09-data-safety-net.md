...
| 4D | 🚧 | **Метрик-контракт: NOTE-1/NOTE-2 из TD-027 (RED-first готов, architect):** NOTE-1 — `journal_bytes_written_total` считал КАДРЫ, не байты → **переименован `journal_frames_written_total`** (честное имя; TD-011-liveness сохранён). NOTE-2 — `journal_seq_gaps_total` НЕ имел продюсера (writer-seq монотонен → OPS-GAP-правило мёртвое = false-safety класса TD-027) → **reclassified read-side**: продюсер = restore-drill/replay (task 3); FA §3/§7.1 честно фиксируют. | architect RED (FA+тест+verify) → engine-dev (rename в `ops::metrics`/`alerts`/`run_writer`) → tester → reviewer | RED: emission-тест + паритет ждут `journal_frames_written_total` (metrics.rs rename → GREEN); §7.1 OPS-GAP → seq_gaps read-side; verify VERDICT PASS после rename |
