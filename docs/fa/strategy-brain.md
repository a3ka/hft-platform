...
| ST-I-5 | **No-lookahead:** интенты на префиксе потока = префикс интентов на полном потоке |
...
| ST-I-8 | Интеграция: `strategy` → `sim::BacktestExchange` (тот же код решений в бэктесте), прогон ×2 бит-идентичен; позиция стратегии == нетто филлов |
