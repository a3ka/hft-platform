...
- **✅ ЗАМОК `A-002` З-1 СНЯТ ПОДПИСЬЮ FOUNDER'А 2026-08-17 (`П-014`). З-2 ОСТАЁТСЯ.**
  ...
  **Что это значит по пунктам:**
  - **полосы глубже 1.3 % ВКЛЮЧАЮТСЯ** — `GATEWAY_BANDS` перестаёт быть ограничен `0.013`
    (сегодня дефолт `0.001`, `docker-compose.yml:134,197`; смена состава — зона `deploy/**`);
...
- **⛔ ДВА ПРЕДУСЛОВИЯ `П-014`, НИ ОДНО НЕ ЗАКРЫТО — включать до них нельзя.**
  - **(а) Провенанс не знает ни о стороне, ни о ресинке.** Сегодня метка ставится ТОЛЬКО по
    ширине полосы: `crates/gateway/src/lib.rs:1035` — `(row.band_pct_e8 > 1_300_000).then(...)`.
  ...
  - **(б) Каденция.** Депт-серия пересчитывается ТОЛЬКО в ветке `L2Snapshot`: ветка `L2Delta`
    книгу и heatmap обновляет, а `depth_series` — нет (`crates/gateway/src/lib.rs:938-941`,
    комментарий «депт-серия остаётся snapshot-only»). Это 1 Гц против дельт 100 мс. Пока не
    починено, включённые полосы отдают ОТСТАЮЩИЕ числа, и выдача обязана это называть, а не
    умалчивать (`PL-I-7`: деградация не выдаётся за норму).
  Смена формы выдачи ⇒ bump `GATEWAY_SCHEMA_VERSION` (прецеденты `VB-I-6`/`VB-I-11`).
