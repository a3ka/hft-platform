...
- **⛔ ВКЛЮЧЕНИЕ ОСТАЁТСЯ ЗАБЛОКИРОВАННЫМ — но СОСТАВ БЛОКЕРОВ СМЕНИЛСЯ (правка 2026-08-25).**
  ...
  - **(б) Каденция.** Депт-серия пересчитывается ТОЛЬКО в ветке `L2Snapshot`: ветка `L2Delta`
    книгу и heatmap обновляет, а `depth_series` — нет (`crates/gateway/src/lib.rs:984-986`,
    ...
    умалчивать (`PL-I-7`: деградация не выдаётся за норму). Заведено долгом **`TD-158`**
    (OPEN, MAJOR): `П-014` п.2 не исполнен.
