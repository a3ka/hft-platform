...
- **⚠️ ЛИТЕРАЛА ДВА, И ЭТО НЕ ОПЕЧАТКА (`C-148` B-2).** Сказанное выше верно для
  **депт-серии**. У **heatmap/COB** метка ставится ДРУГИМ путём и по-прежнему **только по
  ширине**: `crates/gateway/src/lib.rs:1211-1212` (`deep_thr = mid * 0.013`;
  `prov_str = "diff-reconstructed"`), вешается на `:1228` как `deep.then(...)` — без стороны
  и без охвата. Предусловие (а) `П-014` закрыто для депт-серии и **НЕ закрыто для heatmap**:
  два потребителя одного окна несут метки разной осведомлённости. Факт сегодняшнего кода,
  названный здесь, а не задача этого документа.
...
- **⛔ ВКЛЮЧЕНИЕ ОСТАЁТСЯ ЗАБЛОКИРОВАННЫМ — но СОСТАВ БЛОКЕРОВ СМЕНИЛСЯ (правка 2026-08-25).**
  ...
  - **(а) ЗАКРЫТО ДЛЯ ДЕПТ-СЕРИИ, НЕ ДЛЯ HEATMAP** (уточнено `C-148` B-2, см. §4 выше).
    Для депт-серии метка знает СТОРОНУ и ОХВАТ:
