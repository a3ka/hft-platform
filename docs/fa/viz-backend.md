...
| **Volume Profile** (SVP/CVP/FRVP/Anchored/Composite: POC/VAH/VAL/HVN/LVN, VA%) | Trade | ✅ есть (M-24 DONE; session-anchored, VB-I-6/VB-I-8) |
...
- **⛔ ВКЛЮЧЕНИЕ ОСТАЁТСЯ ЗАБЛОКИРОВАННЫМ — но СОСТАВ БЛОКЕРОВ СМЕНИЛСЯ (правка 2026-08-25).**
  ...
  - **(а) ЗАКРЫТО НА УРОВНЕ СТРОКИ.** Метка знает СТОРОНУ и ОХВАТ:
...
> не заведено. `GS-I-*` объявлены в §5 ЭТОГО документа, но их канарейка `GS-I-1`/`VB-I-9a`
> живёт в `scripts/verify_M-28.sh:22-27`, который **CI не зовёт** (`grep -c verify_M-28
> .github/workflows/ci.yml` → 0) — механизация частичная, и это здесь названо, а не подразумевается.
