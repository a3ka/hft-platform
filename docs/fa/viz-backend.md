...
- **⛔ ПРОВЕНАНС И ЖИВОСТЬ РАСЦЕПЛЕНЫ (2026-08-03, `A-002`).** `VB-I-5` мотивирован ДВУМЯ вещами,
  ...
  (дефект метрики, `R-031` §A.1). Инвариант остаётся обязательным; **но провенанс-метка НЕ является разрешением
  включать полосы глубже 1.3%** — включение запрещено замком З-1 до снятия (см. ниже).
...
- **⛔ SUSPENDED (2026-08-03): вердикт Q2 ниже отозван до M-58.** Статус полос 1.5–60% —
  `verification pending`, не `live-verified`. Замок `A-002` З-1/З-2: полосы глубже 1.3% не
  включаются в прод-выдачу (`GATEWAY_BANDS` > 0.013), числа `cancel_fraction` не служат
  доказательством живости. Снимается автоматически переснятием per-life метрикой (M-58).
