...
**ЗАМЕР 2026-07-22 (`depth_probe.rs`, `research/data-quality/depth-probe-staleness.md`) — корректирует ОБЕ позиции:**
- Фантом-сигнатура TD-016 (up→100%, dd→0) **НЕ подтвердилась**: полосы флуктуируют (up 40-60%, dd 100%) → НЕ чистое накопление. Моя гипотеза «чистый фантом» ослаблена.
- Но `dd=100%` конфаундится **resync-обнулением** (REST-ресинк ~1.3% → полосы в 0 → добор из diff); вывод «полосы достоверны» тоже не доказан.
- **Рефрейм:** глубже ~1.3% валидированного эталона нет НИ У КОГО (Binance капит снапшот; Tardis — тот же кап). Полосы 3-30% — diff-реконструкция для ВСЕХ (TPP тоже доверяет своей). ⇒ планка не «вендор для глубины», а **корректность поддержки книги** (TD-016 эвикция + resync-целостность). Tardis — для ИСТОРИИ, не для глубины. **Блокер «вендор y/n» снят (нет), заменён на «корректность книги».**
