...
- **Сигналы/альфы/стратегии — на паузу.** M-10 (OBI kill-screen) закрыт негативно (D-001, OBI мёртв);
  стек-фиксы C-020 A/B/C/D **заморожены** (RED-спека архивирована в `docs/archive/M-10-obi-killscreen-retired-2026-07/`, вернёмся при
...
| # | Решение | Статус |
|---|---|---|
| **D1 Транспорт** (уточнён 2026-07-23) | **Rust `gateway-serve` держит WS напрямую** (tokio-tungstenite), НЕ Fastify-middle-tier. Снапшот-при-подключении + инкрементальный push + replay-контролы; heatmap/depth — бинарные фреймы (postcard), остальное JSON. **Fastify отменён** как обязательный слой: горячий бинарь идёт Rust→браузер без Node-релея (Node-релей = тупой высокобандвидтный прокси + непротестированный слой в детерм-пути). Тейлер co-located с журналом на VPS ⇒ естественно Rust. См. **D6** (app-плоскость). **⟵ 2026-08-26: «Fastify отменён/лишний» БОЛЕЕ НЕ ДЕЙСТВУЕТ** — Fastify возвращён founder'ом и владеет ПУБЛИЧНЫМ API, см. `docs/PENDING-SIGNATURE.md` `П-021`; транспорт горячего пути и мост auth — ОТКРЫТЫЕ вопросы там же. Текст решения ниже не переписан намеренно: приведение к факту — круг разбора развёрнутой версии. | ✅ принято (Path 1) |
