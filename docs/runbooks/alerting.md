...
6. **`WD-CRON-FAILED`**: `cat /var/lib/hft/<job>.alert` — первая строка таймстамп сбоя, вторая —
...
сообщение несёт категорию сбоя (`timeout`/`connect`/...), но не сам URL/токен. HTTP-ошибки САМОГО
Telegram API (401/403 и т.п.) несут `status`+`body`, тоже без URL. Если в `/var/log/hft/watchdog.log`
всё же обнаружится токен — это регрессия, а не штатное поведение; см.
`crates/ops/tests/red_ops_transport_redaction.rs`.
