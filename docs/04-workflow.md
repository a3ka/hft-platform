...
**`COGNITIVE-ONLY`, и вот почему механизма нет.** Барьер на эту норму обязан был бы знать,
ЗАКРЫТ ли милестоун, — а это не выводится машинно: `R-098` §3.1 показал три ревизии
критерия по прозе `PROJECT-STATE.md` с тремя разными ответами. Закрытость есть СУЖДЕНИЕ
reviewer'а, и барьер, притворяющийся, что умеет его вычислить, был бы хуже отсутствующего.
Тег обязателен по BINDING-правилу «обязывающее правило живёт вместе с механизмом»
(founder 2026-08-17), которое называет `docs/04-workflow.md` поимённо.
...
2. **Гейт, на который ссылается ЧУЖАЯ зона, НЕ выносится, пока ссылку не снял её владелец.**
   Замер по `crates/**` и `deploy/**` (которого в первой редакции Р-2 не было): ссылки
   несут **восемь** из 30 кандидатов — `M-17 M-22 M-35 M-45 M-48 M-49 M-50 M-62`, — и лежат
   они в `crates/*/src/**` (architect не пишет impl-код) и `deploy/**` (зона engine-dev).
   Первая редакция требовала переписать их «в том же диапазоне» — **неисполнимо ни одной
   ролью в одиночку** (`R-104` Б-1, смежное). Поэтому: architect выносит только те гейты,
   чьи ссылки не выходят за его зону; остальные восемь ждут, пока владелец зоны ссылку
   снимет или переведёт на живой оракул. Первый проход — **22 гейта, не 30**.
...
**22 гейта из 30**: восемь отложены до снятия ссылок владельцами их зон.
