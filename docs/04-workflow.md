...
### Предмет гейта — ВЕТКА; вершину агент берёт КОМАНДОЙ (`COGNITIVE-ONLY`)

Единственное место этой нормы; профили ролей ссылаются сюда, а не пересказывают её.

1. **Мандат любой роли цепочки** (critic, risk-critic, арбитр, tester, reviewer, перепроверка
   `gates.md` §9) **называет ВЕТКУ** `origin/<ветка>` — это предмет — и, СПРАВКОЙ, вершину
   на момент передачи («на момент передачи был X»). Справка обязательна: без неё развилка
   п.3 «не предок ⇒ СТОП» обезоружена, и переписанную ветку не заметит никто.
2. **Агент берёт вершину сам** — `git fetch origin && git rev-parse origin/<ветка>` — и
   ПЕЧАТАЕТ её первой строкой своего Done Block'а. Вердикт-файл пишет ЕЁ в `audited_head`
   (`gates.md` §4, шапка `GATE-META`). **`fetch` обязателен:** `rev-parse origin/<ветка>` без
   него читает ЛОКАЛЬНЫЙ remote-tracking ref, и устаревший SHA будет напечатан как «вершина,
   взятая командой» — тот же класс, но уже с ложной строкой-доказательством. Ref не
   разрешился (ветка удалена, опечатка) ⇒ **СТОП**, «похожую» ветку не ищут.
3. **SHA, названный в мандате, — справка, а не предмет.** Совпал с вершиной — хорошо. Не
   совпал, но является её предком (`git merge-base --is-ancestor <sha> origin/<ветка>`) —
   ветка ушла вперёд: судится ВЕРШИНА, оба SHA печатаются рядом, чтобы расхождение видел
   следующий. НЕ является предком — ветку переписали или мандат ошибся: **СТОП**, вердикт
   не выносится, вопрос возвращается тому, кто запустил.
4. **Перед записью вердикта — повторный `git fetch origin && git rev-parse origin/<ветка>`.**
   Ветка ушла, пока шла работа, — вердикт НЕ пересуживает (судить движущуюся цель нельзя), а
   печатает строку «после взятия вершины ветка ушла: <судил> → <сейчас>». Так следующий видит
   расхождение без отдельного наблюдателя.

**Почему так, а не «проверь то, что назвали».** Мандат пишется в один момент, а ветка едет:
пока мандат идёт по цепочке, в неё приходят исправления. Агент, послушно судящий названный
SHA, судит прошлое и НЕ ОТКАЗЫВАЕТСЯ — у него нет признака, что что-то не так. Два
случая за 2026-09-20/21:

- критик `C-232` судил `684ea89` — первый из четырёх авторских коммитов ветки
  `docs/td-216-fa-depth-range`; ложное утверждение появилось во ВТОРОМ и гейтом не судилось
  вовсе, поймал внешний круг. Предъявлено артефактом: `R-194` §0 (вершина взята командой,
  расхождение показано `git log`);
- тестер прогнал `a65b859` вместо коммита-исправления. Носитель — только запись сессии
  (`docs/workflow/session-handover-2026-09-21.md` §4 (а)), артефакта гейта нет; назван, чтобы
  не выдавать два подтверждённых случая за один.

Форма, которая лечит, проверена исполнением: `R-194` взял вершину сам и судил верную ревизию
там, где `C-232` на том же предмете судил первую.

**Почему `COGNITIVE-ONLY`.** Барьер не отличит вершину, взятую командой, от SHA, списанного из
мандата: в обоих случаях `audited_head` существует и является предком `HEAD` — ровно то, что
`check_gate_meta.sh` проверяет. Правило держится на ПРИНИМАЮЩЕЙ стороне (п. 2–3 исполнимы
самим агентом), как и `П-019` у tester'а.

**Отвергнутый механизм — чтобы его не предлагали снова.** Расширить класс предметов замка
(`is_gate_class`, `scripts/check_gate_meta.sh`) на зону формы `gates.md` §9 — ОТВЕРГНУТО
замером `R-194` §5: на живых ветках покрасило бы пять вердиктов, из них четыре — `DECISION`
арбитра, по которым architect правит спеку ПО решению. Лок, красящий нормальный круг, вреднее
отсутствующего (спека `M-60b`, строка GM-11 таблицы запретов). И главное — он ловит правку ПОСЛЕ
вердикта, а дефект этого класса — вердикт НАД НЕ-ВЕРШИНОЙ, при котором правок после может не
быть вовсе.

**Кандидат-механизм назван и НЕ построен — решением, а не забвением.** Наблюдатель (не
барьер), печатающий расхождение `audited_head` последнего проходного вердикта с вершиной
ветки, — `R-194` §5, по образцу ВИСЯК/ДУБЛЬ в `scripts/check_branch_health.sh`. Founder
2026-09-26 разрешил его под условием «если уверен на 100 %, что надо»; architect уверенности
не имеет и не строит: подтверждённых случаев ДВА при пороге разморозки `П-017` A2 «≥3,
предъявленных замером», а п.4 выше делает дрейф видимым в самом вердикте без нового харнесса.
**Условие пересмотра — третий случай** вердикта над не-вершиной, предъявленный артефактом.
...
| **колонка «Состояние» в `docs/ROADMAP.md`** | **reviewer** | тот же довод, что в строке 1: закрытость — его суждение |
| **перечень и ПОРЯДОК в `docs/ROADMAP.md`** | **architect** | порядок работ есть проектное суждение, а не факт после merge'а |
...
**`docs/ROADMAP.md` обновляется ТЕМ ЖЕ close-out'ом — и это механизировано.** Роадмап
заведён 2026-09-07 по требованию founder'а как ЕДИНСТВЕННЫЙ источник ПОРЯДКА работ. До него
на вопрос «что дальше» отвечали шесть документов, ни один не главный, а преемственность
фактически жила в 23 записках сессий: замер и цена (план из 11 пунктов с ПЯТЬЮ ошибками) —
в шапке самого файла. Барьер `scripts/check_roadmap_sync.sh` + джоб `roadmap-sync` в
агрегате: коммит, уносящий спеку в `docs/archive/`, обязан в том же диапазоне тронуть
роадмап. Триггером выбран ПЕРЕЕЗД, а не «милестоун закрыт», по той же причине, по которой
close-out разведён по ролям выше: закрытость машинно не выводится (`R-098` §3.1 — три
ревизии критерия дали три разных ответа). **Предел назван:** барьер ловит отсутствие
правки, а не её осмысленность; неверно обновлённая строка пройдёт, против неё — круг гейта.

**Коммит переезда несёт ДВА токена, не один.** Пара «гейт + спека» пересекает два барьера:

| путь | барьер | токен |
|---|---|---|
| `scripts/verify_M-NN.sh` | `check_gate_meta.sh` — числит `scripts/verify_*.sh` гейт-классом, запертым проходным вердиктом (subject-lock, `:380-399`) | `ALLOW-SUBJECT-CHANGE: <причина>` |
...
**Поправка 2026-08-24: в первой строке стоял НЕ ТОТ токен.** Здесь было
`ARCHIVED-VERDICT: <путь> — <причина>`; механизм назван верно (subject-lock), но открывает
его другая строка — `check_gate_meta.sh:393` ищет ровно `ALLOW-SUBJECT-CHANGE:`, а
`ARCHIVED-VERDICT` (шапка того же файла, `:15-26`) обслуживает совсем иной случай:
приземление ДО-НОРМАТИВНОГО вердикта со статусом `A` в `research/{critiques,reviews,arbitration}`.

Ошибка была ЛАТЕНТНОЙ, и это объясняет, почему её не поймали ни автор нормы, ни её
перепроверка, ни первый проход переезда. Subject-lock взводится ТОЛЬКО когда в том же
диапазоне тронут ПРОХОДНОЙ вердикт. У прецедента `d788abf` (26 гейтов) и у переезда M-69
(`636a3ec`) вердиктов в диапазоне не было — барьер напечатал «вердиктов проверено: 0», лок
не взвёлся, и оба коммита прошли, неся токен, которого барьер не читал. Первый же переезд,
собранный ОДНИМ диапазоном с вердиктом, получил бы красное и был бы отправлен нормой за не
тем токеном. Класс — тот же `TD-138`: документ обосновывает инвариант механизмом, который
на этом пути не работает.
