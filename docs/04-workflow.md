...
### Close-out: гейт закрытого милестоуна уезжает в архив вместе со спекой

**Решение founder'а 2026-08-22 (Р-2).** На close-out reviewer переносит
`scripts/verify_M-NN.sh` и `milestones/M-NN-*.md` в `docs/archive/`. Замер, из-за которого
...
2. **Ссылки на вынесенный гейт переписываются В ТОМ ЖЕ диапазоне.** Замер по `crates/**`
   и `deploy/**` (которого в первой редакции Р-2 не было): ссылки несут **восемь** из 30
   кандидатов, худшая — `deploy/bin/gateway-checkpoint-cron.sh:39,78`, где `verify_M-48`
   объявлен машинной сверкой КОМПОЗИЦИИ путей прод-cron'а. Оставить строку как есть —
   произвести «built-not-wired» наоборот: контракт заявлен, проверяющего нет; тот же
   класс, что `TD-138` (`R-098` §4.3).
3. **Гейт, на который ссылается ЖИВОЙ барьер, не выносится, пока ссылка жива.** Сегодня
   это `M-60b` (`check_context_budgets.sh:19`).

**Норма не ретроспективна сама по себе:** она останавливает РОСТ. Разовый вынос 30 гейтов —
отдельная работа по списку `R-098` §3, и он идёт под теми же тремя требованиями.
