...
1. **Мандат любой роли цепочки** (critic, risk-critic, арбитр, tester, reviewer, перепроверка
   `gates.md` §9) **называет ВЕТКУ** `origin/<ветка>`, а не ревизию.
...
отсутствующего (`milestones/M-60b-gate-mechanisms.md` GM-11). И главное — он ловит правку ПОСЛЕ
вердикта, а дефект этого класса — вердикт НАД НЕ-ВЕРШИНОЙ, при котором правок после может не
быть вовсе.

**Кандидат-механизм назван и НЕ построен:** наблюдатель (не барьер), печатающий расхождение
`audited_head` последнего проходного вердикта с вершиной ветки, — `R-194` §5, по образцу
ВИСЯК/ДУБЛЬ в `scripts/check_branch_health.sh`. Новый харнесс под заморозкой `П-017` A2;
строится только после точечной разморозки founder'ом.
