...
2. **`SCHEMA_VERSION` 2 → 3 (rev2, ИСПРАВЛЕНО после TD-031).** ⚠ rev1 держал версию на 2
   («аддитивно, magic валидируется на чтении, bump не нужен») — это было НЕПОЛНО: оно закрывало
   wire-совместимость, но НЕ **изоляцию сегмента**. Изоляция (L2D-I-8) должна была держаться
   `provenance` (git-sha в заголовке), но §8 показал, что recorder считает git-sha В РАНТАЙМЕ, а
   контейнер БЕЗ git → provenance = КОНСТАНТА `"recorder v0.0.0 (git:no-git-info)"` на всех
   деплоях → `decide_open_segment` reuse'ил pre-M18 сегмент → L2Delta смешался в schema-2 сегмент
   (TD-031). Провенанс-изоляция ВОИД. Правильный маркер — `SCHEMA_VERSION` как **эпоха
   эмитируемых вариантов**: bump 2→3, и **`decide_open_segment` reuse требует
   `header.schema_version == SCHEMA_VERSION`** → schema-2 сегмент не reuse'ится schema-3 бинарём
   (git-независимо, детерминированно). Правило: **новый эмитируемый вариант ⇒ bump SCHEMA_VERSION**.
   Магия (framing) при этом НЕ меняется — `SCHEMA_VERSION` отделён от `SEGMENT_MAGIC` (magic =
   эпоха фрейминга, `HFTJRN02`; schema = эпоха вариантов). `schema_version` на чтении по-прежнему
   не валидируется (старые сегменты читаются, CT-I-3) — он гейтит только reuse на ЗАПИСИ.
...
**⚠ segments {55, 56} — одноразовый СМЕШАННЫЙ набор (артефакт TD-031, уточнено reviewer'ом на §8).**
До fix'а первое живое L2Delta ушло в pre-M18 `segment-00000055` (schema-2 заголовок, variant-6
внутри); pre-fix бинарь затем РОТИРОВАЛ 55→56, продолжая капчить L2Delta, поэтому **и 56 смешан**
(schema-2 header + variant-6). Оба НЕ quarantine-able чисто (несут И pre-M18 данные, И L2Delta);
pre-M18 бинарь на них захлебнётся. При откате трактовать {55, 56} как ПОЛНОСТЬЮ tainted. **Изоляция
чиста начиная с segment-57** (первый, открытый fix-бинарём schema-3 — машинный schema-гейт); §8
подтвердил живое BTC L2Delta в 57, не в 55/56. Это цена однократного дефекта; данные в 55/56 не
потеряны, но эпоха-граница внутри них грязная (forensic-долг — TD-032).
