...
**⚠ segment-55 — одноразовый СМЕШАННЫЙ сегмент (артефакт TD-031).** До fix'а первое живое L2Delta
ушло в pre-M18 `segment-00000055` (schema-2 заголовок, но содержит variant-6 внутри). Он НЕ
quarantine-able чисто (несёт И pre-M18 данные, И L2Delta); pre-M18 бинарь на нём захлебнётся. При
откате segment-55 трактовать как ПОЛНОСТЬЮ tainted (весь сегмент). Начиная с fix-деплоя (schema-3)
новые сегменты (56+) чисты по эпохе — смешения больше не будет (машинный schema-гейт). Это цена
однократного дефекта, зафиксирована; данные в 55 не потеряны, но эпоха-граница внутри него грязная.
