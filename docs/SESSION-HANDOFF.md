...
> **Обновлено 2026-07-14.** ⛔ **НЕ диспетчеризовать risk / killswitch / oms / runner / testnet.**
> Прежняя формулировка этого раздела предлагала «M-08 = risk + killswitch + oms» — она УСТАРЕЛА
> (под номером M-08 сделано другое: data durability) и провела бы агента мимо фазы P2.5 прямо в
> торговый стек. Источник правды по очереди — `milestones/BACKLOG.md` + `docs/DESIGN.md` §10.

### Сейчас в работе

**M-08 «Data durability» (rev 6) — IN_PROGRESS, close-out невозможен.** Код в проде (`b7721d1`),
прод здоров: ротация работает, legacy-сегмент (15.0 GB) заморожен и цел, `seq` сшит через границы,
полосы OBI не деградировали. **Но две цели НЕ достигнуты** (§8 eyes-on, 2026-07-14):

- **TD-020 (MAJOR, главная цель):** ретеншен написан как библиотека, но **его никто не вызывает** —
  ни recorder, ни CLI, ни cron. «Сбор не остановится НИКОГДА» не выполнено: диск растёт ~2.8 GB/сут,
  **~40 дней до disk-guard**. Ветка `feat/M-08-closeout`: спека операторского пути (бинарь
  `journal-retention` + cron, `DryRun` по умолчанию) + RED R1-R7 → engine-dev.
- **TD-019 (MINOR):** heartbeat = 13 байт таймстампа; наблюдаемость `storage_status` обещана E4 и
  не сделана. RED написан.
- **TD-021 (MAJOR, процессный):** метрика, по которой был заведён TD-016, **врала** — `docker stats`
  включает page cache журнала. Реальный рост кучи +1 MiB/ч, а не +8. TD-016 переспецифицирован:
  **точность данных > экономия памяти**; книга, возможно, СХОДИТСЯ (бутстрап top-5000 + дальние
  уровни из diff), а не течёт. Проверяется асимптотой и recon'ом (P2.5).

### Что дальше — решает founder ★ (не агент)

1. **P2.5 «Data safety net»** (`docs/fa/ops.md`, `BACKLOG` M-09) — **PROPOSED**, проходит doc-гейт
   (`gates.md` §9). Пока не принята founder'ом, работа по ней НЕ стартует. Содержание: бэкап + 
   restore-drill, **recon книги с биржей** (единственная проверка правильности данных), метрики+алерты.
2. **Развилка HL-глубины ★** (`BACKLOG` M-13): либо закрыть глубину HL (полосы OBI там невычислимы,
   TD-005), либо осознанно сменить первый сигнал/площадку. Пока не решено — **M-11/M-12 не могут
   назвать свой первый live-сигнал**, поэтому торговый стек не диспетчеризуется.
3. **M-10 (отчёт R-001, валидация OBI)** — независим от прод-стека, можно запускать параллельно
   (compute на записанных данных; TD-015: только записи кода `>= 5141fd9`).

**Порядок из `BACKLOG` (приоритет = цена ошибки):** M-08 close-out → P2.5/M-09 (данные) →
M-10 (доказательство сигнала) → M-11 (барьер к деньгам) → M-12 (исполнение).
