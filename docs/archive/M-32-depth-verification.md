...
| 4 (Q2в) | ✅ N/A | **cross-source recon** — ТОЛЬКО если Q1 (task 1) нашёл независимый deep-источник: сверить нашу реконструкцию против него. Если источника нет → задача N/A с явной записью «эталона нет, Q2в закрыт отрицательно» | research-dev | Либо recon-числа, либо документированное N/A с обоснованием из Q1 |
| 5 (verdict) | ✅ (ждёт founder-подпись) | **Вердикт-memo** → `research/data-quality/depth-verdict.md`: синтез Q1+Q2 → 3 founder-решения (эталон глубже 1.3% y/n; полосы достоверны y/n по staleness+order-flow; TPP на эталоне ИЛИ diff-provenance). Гейт: TPP-полосы НЕ в контракт до подписи. | architect (синтез) → **founder** | Вердикт называет каждый Q1/Q2 результат; §D handoff — founder-подпись (что именно решает) |
