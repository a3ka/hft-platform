...
# Считаем строки ТОЛЬКО внутри секции M-58. Первая редакция этой проверки грепала по ВСЕМУ
# файлу и дала ложный PASS («строк = 32») при шести строках в самой секции: в файле лежит ещё
# и таблица M-32 на 24 строки. Проверка мерила окружение, а не свой инвариант (testing.md,
# «целостность гейта», свойство 2) — поймано первым же прогоном.
SECTION="$(awk '/УСЛОВИЯ ПРОГОНА M-58/,0' "$RESULTS")"
ROWS="$(printf '%s\n' "$SECTION" | grep -cE '^\| *(bid|ask) *\|' || true)"
if [ -z "$SECTION" ]; then
  fail "задача 3 — секция «УСЛОВИЯ ПРОГОНА M-58» не найдена: считать нечего (setup не состоялся)"
elif [ "${ROWS:-0}" -ge 14 ]; then
  pass "задача 3 — таблица §M-58 полна: строк bid/ask внутри секции = ${ROWS} (7 полос × 2 стороны)"
else
  fail "задача 3 — таблица §M-58 НЕПОЛНА: строк bid/ask внутри секции = ${ROWS:-0}, ожидалось >= 14. \
