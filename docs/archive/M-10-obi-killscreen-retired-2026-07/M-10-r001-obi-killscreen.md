<!-- ═══════════════ АРХИВНАЯ ВРЕЗКА — добавлена при архивации, не часть оригинала ═══════════
     ...
     Сдан в архив 2026-08-15 решением founder'а; носитель — мёртвая ветка `feat/M-10-rebased`
     (последний коммит `51c21dc`, 2026-07-22). Документ НЕ активен: по нему не исполняют и его
     не правят.

     ⛔ КОД ЭТОГО МИЛЕСТОУНА К CI НЕ ПОДКЛЮЧАЕТСЯ. Он несёт дефект `KS-I-1` (`sharpe_se`
        считался от числа шагов, а не от календарной длины окна → ложный PASS достижим), и
        `C-020` прямо блокирует merge. Код сохранён инертным патчем `crates-research-cli.patch`.

     ПОЛНЫЙ КОНТЕКСТ, ВЕРДИКТЫ И УРОКИ — в `README.md` этого каталога. Читать его первым.

     Ниже маркера конца врезки — ТЕЛО ОРИГИНАЛА БАЙТ-В-БАЙТ.
     ═════════════════════════════════════════════════════════════════════════════════════ -->
<!-- Этот файл: спека милестоуна (rev3) -->
