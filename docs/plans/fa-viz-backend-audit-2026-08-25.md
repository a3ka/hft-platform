...
- **(b) новый `docs/fa/gateway.md` (+`gateway-serve`)** — честный дом. Цена: новый FA = новый носитель под полный круг §9 + правка `reading-map` §2 и `DESIGN` §22 тем же коммитом; идёт против решения founder'а «ничего нового» от 24.08 — требует его явного слова. Выгода: слой 8 расщеплён по крейтам, как весь остальной корпус.
...
ПОЧЕМУ: `docs/archive/M-36-gateway-snapshot-prod.md`; `grep -n "session-reset СНЯТ" crates/gateway/src/lib.rs` → :403; `grep -in hlc3 crates/gateway/src/lib.rs` → пусто.
